...
- A benchmark promising a new report file must use exclusive creation (`Path.open("x")`), not only an earlier `exists()` check: another writer can create the file during the benchmark. Test a file appearing after argument validation and verify that its contents survive (review on PR #1070).
