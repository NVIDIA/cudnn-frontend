...
def extract_seq_and_ragged(payload: dict, seed: int) -> dict:
    ...
    if ragged_q_entry is None:
        ragged_q_entry = utils.ragged_offset_entry(
            tensors, utils.tensor_entry(tensors, inputs.get("Q"))
        )
    if ragged_kv_entry is None:
        ragged_kv_entry = utils.ragged_offset_entry(
            tensors,
            utils.tensor_entry(tensors, inputs.get("K")),
            utils.tensor_entry(tensors, inputs.get("V")),
        )
