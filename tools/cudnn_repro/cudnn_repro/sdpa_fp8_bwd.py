...
def extract_seq_and_ragged(payload: dict, seed: int) -> dict:
    ...
    k_entry = utils.tensor_entry(tensors, inputs.get("K"))
    v_entry = utils.tensor_entry(tensors, inputs.get("V"))
    ragged_q_entry = utils.tensor_entry(
        tensors, ragged_offset_q
    ) or utils.ragged_offset_entry(tensors, q_entry)
    ragged_kv_entry = utils.tensor_entry(
        tensors, ragged_offset_kv
    ) or utils.ragged_offset_entry(tensors, k_entry, v_entry)
    return {
