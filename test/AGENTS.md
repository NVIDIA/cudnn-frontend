...
- **Build the reference in fp64 when the bound is tighter than ~1e-3.** The DLFW CI containers run fp32 matmul in TF32 (`TORCH_ALLOW_TF32_CUBLAS_OVERRIDE=1`; recent torch also defaults `fp32_precision` to `tf32` on Blackwell+), a ~3e-4 relative error per `Q @ K^T` logit. A 1024-column log-sum-exp averages it down to ~2e-5, a causal row with ONE valid column keeps it whole: `test_fp8_stats_is_the_exact_softmax_lse[causal]` read max|dLSE| 1.3e-4 against its 1e-4 bound on the sm107 lane (2026-09-15) with an exact kernel, and passed on a 208-SM node whose draws happened to be kinder. `sdpa/fp8_ref.compute_ref_forward(dtype=torch.float64)` is the existing knob; in a hand-rolled reference `.double()` the operands before the matmul, not just the `logsumexp`.
- **An fp8 midpoint flip has a SHAPE -- check it before touching a cap.** One flipped P/dS code moves one output d-row by `ulp x operand_row` (Q for dK, K for dQ, dO for dV, V for O), a rank-1 update along a single operand row with a power-of-two step times the intermediate's descale; a defect (a tile, a band, a mask off by one) is a sum over many rows and fits none. `assert_close_fp8_grad(..., operand=, flip_unit=)` verifies exactly that and prints the fitted step (`2^-4 x flip_unit`); `sdpa/test_fp8_flip_budget.py` pins what it accepts and rejects. The negative-score q rows have amplitude 8 at d192, so a legitimate flip moves a dK row by 0.5 -- the fixed `4 * atol` cap (0.32) alone would call that a defect (sm107 212-SM lane, test310, 2026-09-15).
