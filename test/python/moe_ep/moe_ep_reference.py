...
class WgradOperandsReference:
    ...
    def dense_wgrads(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """Return dense ``dW1=x.T@dC`` and ``dW2=(p*h).T@dY`` per expert."""
...
class MoeEpReference:
    ...
    def __init__(
        ...
        *,
        ...
        self.intermediate_format = None if intermediate_format is None else _parse_format(intermediate_format)
        self.backward_operand_format = None if backward_operand_format is None else _parse_format(backward_operand_format)
    ...
    def __call__(
        ...
        forward_activation_float = wgrad_activation_float if wgrad_activation_float is not None else activation_float
    ...
    def backward(
        ...
        *,
        ...
        weighted_h_rows = torch.zeros(
            (local_routes, self.intermediate_size),
            dtype=torch.float32,
            device=device,
        )
        wgrad_dy_rows = torch.zeros(
        ...
        for expert in range(self.experts_per_rank):
            ...
            s = g * sig
            h = s * u
            weighted_h_rows.index_copy_(0, positions, h * w)
            wgrad_dy_rows.index_copy_(0, positions, d_y)
        ...
        if self.backward_wgrad_mode == "operands":
            ...
            padded_weighted_h = _padded_expert_rows(
                weighted_h_rows,
                expert_rows,
                valid_counts,
                padded_ends,
            )
            padded_wgrad_dy = _padded_expert_rows(
                wgrad_dy_rows,
                expert_rows,
                valid_counts,
                padded_ends,
            ...
            operands = WgradOperandsReference(
                ...
                fc2_a=quantize_blockwise(
                    padded_weighted_h.transpose(0, 1),
                    MoeFormat.MXFP8,
                    axis=1,
                ),
