...
_SPECS = [
    ...
    SuiteSpec(
        ...
        rng_seed=445,
        knobs=knobs.chunked,
        fuzzed=_COMMON_FUZZ + _MASK_FUZZ + _THD_FUZZ + ("layout ragged/cu_ragged/padded/full",),
        pinned=("infer", "s_q<=1024 chunks", "s_kv up to 16k"),
        notes="chunked prefill: query chunks against a long KV history (was generation.f16.thd_chunked)",
    ),
    SuiteSpec(
        name="context.fp8.chunked",
        phase="context",
        dtype="fp8",
        level="L0",
        num_tests=192,
        rng_seed=991,
        knobs=knobs.fp8_chunked,
        exec_kind="fp8",
        fuzzed=_COMMON_FUZZ
        + _MASK_FUZZ
        + (
            "e4m3/e5m2 in",
            "out fp8/fp16",
            "layout ragged/cu_ragged/padded/full",
            "sink",
            "capacity slack",
            "declared totals",
        ),
        pinned=("infer", "s_q<=1024 chunks", "s_kv up to 16k", "d<=128 (fp8 ragged d>128 hangs the backend)"),
        notes="chunked prefill, fp8 (was generation.fp8.thd_chunked)",
    ),
    SuiteSpec(
