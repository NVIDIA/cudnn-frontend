...
def test_fully_masked_kv_tiles_overwrite_outputs(monkeypatch, out, persistent, b, hq, hkv, sq, skv):
    ...
    if persistent:
        # The dK/dV head/sequence split is not implemented on the persistent
        # scheduler, and the heuristic would pick a head split for the GQA shapes.
        from cudnn.sdpa.bwd.api_dsl_mxfp8_sm100 import SdpaBwdDslSm100Mxfp8

        def unsplit(init):
            def wrapped(self, *args, **kwargs):
                kwargs["dkdv_head_split"] = 1
                kwargs["dkdv_seq_split"] = 1
                init(self, *args, **kwargs)

            return wrapped

        monkeypatch.setattr(SdpaBwdDslSm100Mxfp8, "__init__", unsplit(SdpaBwdDslSm100Mxfp8.__init__))
