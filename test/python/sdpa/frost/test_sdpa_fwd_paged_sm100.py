...
from frost_test_utils import launch_f16, launch_combine, requires_dsl, requires_pre_rubin_blackwell, select_engine
...
def _run_kernel(B, H, KH, P, max_pages, lens, hnd, splits, *, cta_mma=1, dtype=torch.float16, d=128, expect_pack_g=None):
    ...
    fn = mod.compile(d_qk=d, d_v=d, has_lse=True, lse_kind="dense", paged_hnd=k_view.stride(1) < k_view.stride(2))
    ...
    if splits == 1:
        ...
        launch_combine(cfn, o_p, lse_p, o_out, lse_out, None, None, (B, H, 1, d), cutlass.Int32(splits), stream=stream)
    torch.cuda.synchronize()
