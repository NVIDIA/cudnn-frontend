...
# --- MXFP8 pools (sdpa_mxfp8) --------------------------------------------------
#
# The same paged contract on the block-scale node. The descales are page pools too
# (descale_k [num_pages, H_kv, page_size, D/32], descale_v [num_pages, H_kv, page_size/32, D],
# F8_128x4-reordered, so page_size % 128 == 0) and page with K/V through the same block
# tables. Reference: dequantize the pools, gather the live tokens, fp32 attention. Dead pool
# pages are NaN-filled as in the fp8 section.
...
def _ref_mxfp8(qd, kd_pool, vd_pool, bt_k, bt_v, seq_lens, scale, *, causal_br=False, window_left=None, sinks=None):
...
def _pools_mxfp8(B, KH, P, max_pages, hnd, fp8, *, d_qk=D_MXFP8, d_v=D_MXFP8, seed=0, separate_v=False, batch_inner=False):
...
def _run_graph_mxfp8(B, H, KH, P, max_pages, lens, hnd, *, want_split=None, **build_kw):
    import cudnn
    import cudnn.sdpa  # noqa: F401 — registers the FROST engines
    from cudnn.sdpa.fwd.engines import engine_name
...
def test_paged_mxfp8_graph_prefill_shaped_s_q():
    ...
    _run_graph_mxfp8(2, 4, 2, 128, 8, [700, 300], False, s_q=64, causal_br=True, stats=True)
...
def test_paged_mxfp8_graph_split_kv():
    ...
    plan = _run_graph_mxfp8(1, 4, 1, 128, 64, [8000], False, stats=True, want_split=True)
...
def test_paged_mxfp8_adapter_cuda_graph_replay_no_host_sync():
    ...
    dev = "cuda"
    fp8 = _FP8["e4m3"]
    pools = _pools_mxfp8(B, KH, P, max_pages, False, fp8, seed=1)
    Qf = torch.randn(B, H, 1, D_MXFP8, device=dev) * 0.5
    Q8, sfq, dqq, sfq_dims = _quantize(Qf, B, H, 1, D_MXFP8, fp8, columnwise=False)
...
def test_paged_mxfp8_graph_e5m2_sink_swa():
    ...
    _run_graph_mxfp8(3, 8, 2, 128, 9, [1000, 1, 513], False, in_key="e5m2", out_dt=torch.float8_e4m3fn, stats=True, sink=True, window_left=300)
...
def test_paged_mxfp8_graph_other_flavors(dims, hnd):
    d_qk, d_v = dims
    _run_graph_mxfp8(3, 8, 2, 128, 6, [700, 2, 129], hnd, d_qk=d_qk, d_v=d_v, stats=True, s_q=2, causal_br=True)  # L >= s_q: no keyless rows
...
def test_paged_mxfp8_adapter_rejects_unequal_table_extents():
    ...
    fp8 = _FP8["e4m3"]
    pools = _pools_mxfp8(B, KH, P, max_pages, False, fp8, seed=1)
    Q8, sfq, _, sfq_dims = _quantize(torch.randn(B, H, 1, D_MXFP8, device=dev) * 0.5, B, H, 1, D_MXFP8, fp8, columnwise=False)
    Qb = Q8.permute(0, 2, 1, 3).contiguous().transpose(1, 2)
    Ob = torch.empty(B, 1, H, D_MXFP8, device=dev, dtype=torch.bfloat16).transpose(1, 2)
    lse = torch.empty(B, H, 1, device=dev, dtype=torch.float32)
    amax = torch.zeros(1, 1, 1, 1, device=dev, dtype=torch.float32)
