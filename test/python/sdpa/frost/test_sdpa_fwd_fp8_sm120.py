...
no THD. Everything outside that envelope is a capability-row decline, checked by
the *_not_offered tests below.
...
def test_fp8_sm120_gqa():
    scale = 1.0 / math.sqrt(128)
    res = _run(2, 8, 2, 256, 256, scale=scale, sdpa_kwargs=dict(use_causal_mask=True))
    _check(*res)
