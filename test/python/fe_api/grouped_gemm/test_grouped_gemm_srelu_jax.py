...
"""
JAX coverage for the SM100 grouped GEMM SReLU wrapper.
...
def make_jax_inputs(m=256, n=256, k=128, experts=2):
    ...
    return a_j, sfa_j, offsets_j, alpha_j, prob_j


@pytest.mark.L0
def test_grouped_gemm_srelu_jax_discrete_rejected_with_clear_error():
    from cudnn import grouped_gemm_srelu_wrapper_sm100

    m, n, k, experts = 256, 256, 128, 2
    a_j, sfa_j, offsets_j, alpha_j, prob_j = make_jax_inputs(m, n, k, experts)
...
@pytest.mark.L0
def test_grouped_gemm_srelu_jax_api_class_rejected():
    from cudnn.gemm.cutedsl.grouped.srelu.api import GroupedGemmSreluSm100
...
@pytest.mark.L0
def test_grouped_gemm_srelu_unknown_framework_rejected():
    from cudnn import grouped_gemm_srelu_wrapper_sm100
