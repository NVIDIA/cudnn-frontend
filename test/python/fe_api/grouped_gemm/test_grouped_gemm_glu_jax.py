...
"""
JAX coverage for the SM100 grouped GEMM GLU forward wrapper.

JAX contract: BF16 backend, discrete weight mode only (dense mode's expert-outermost
strided B and the column-major bias layout are not expressible as row-major JAX arrays,
and the block-scaled backend's MMA-interleaved scale-factor layouts are likewise
inexpressible), with b_ptrs built from per-expert weight pointers -- as a packed uint8
array (8 bytes per pointer) since JAX truncates int64 without x64 mode. Outputs are
checked bit-identical against the torch wrapper run on identical input bytes (both
paths share one compiled kernel).
"""
...
def make_problem(m=512, n_full=256, k=128, experts=2):
    rng = np.random.default_rng(20260808)
    a_np = (rng.standard_normal((m, k, 1), dtype=np.float32) * 0.125).astype(ml_dtypes.bfloat16)
    b_storage_np = (rng.standard_normal((experts, n_full, k), dtype=np.float32) * 0.125).astype(ml_dtypes.bfloat16)
    group_m = m // experts
...
def test_grouped_gemm_glu_jax_discrete_matches_torch(act_func):
    skip_unless_sm100()
    ...
    # ---- torch run (discrete BF16 mode, no bias) ----
    a_t = torch.from_numpy(a_np.view(np.uint8)).view(torch.bfloat16).reshape(m, k, 1).cuda()
    b_storage_t = torch.from_numpy(b_storage_np.view(np.uint8)).view(torch.bfloat16).reshape(experts, n_full, k).cuda()
    b_ptrs_t = torch.tensor([b_storage_t[i].data_ptr() for i in range(experts)], dtype=torch.int64, device="cuda")
    result_t = grouped_gemm_glu_wrapper_sm100(
        a_tensor=a_t,
        sfa_tensor=None,
    ...
    result_j = grouped_gemm_glu_wrapper_sm100(
        a_tensor=a_j,
        sfa_tensor=None,
    ...
    for key in ("c_tensor", "d_tensor"):
        np.testing.assert_array_equal(
            ...
            err_msg=f"grouped GLU {key} ({act_func}): JAX output differs from torch output on identical input bytes",
        )
...
def test_grouped_gemm_glu_jax_errors():
    ...
    # Column-major (n, experts) bias is inexpressible.
    bias_j = jnp.zeros((n_full, experts), dtype=ml_dtypes.bfloat16)
