...
from cudnn import block_sparse_attention_forward as forward
from cudnn import block_sparse_attention_backward as backward
from cudnn.block_sparse_attention import block_sparse_attention as attention
...
assert callable(cudnn.BSA.block_sparse_attention)
assert callable(cudnn.block_sparse_attention_backward)
assert cudnn.block_sparse_attention_forward is bsa.block_sparse_attention_forward
assert cudnn.BSA.block_sparse_attention_backward is bsa.block_sparse_attention_backward
for name in ("block_sparse_attention_forward_jax", "block_sparse_attention_backward_jax", "block_sparse_attention_jax"):
    for namespace in (cudnn, cudnn.BSA, bsa):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"pack_gqa": True}, NotImplementedError),
        ({"kv_splits": 2}, NotImplementedError),
        ({"use_clc": False}, NotImplementedError),
    ],
)
...
def test_jax_rejects_caller_gradient_outputs(name):
    ...
    with pytest.raises(NotImplementedError, match="caller-provided gradient outputs"):
