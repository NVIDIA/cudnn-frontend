...
def test_sm107_block_scale_flexible_cga(combo, config_name):
    """Flexible CGA rides along with no caller change: any config whose cluster is
    ...
    assert C._flex_cga_fallback(cfg, cta_group, f"sm107_block_scale_matmul_{cta_group}ctamma.py") == (cta_group, 1)
    _run_bs_numeric(combo, config_name, 1920, 1920, 512)


@requires_sm107
@pytest.mark.parametrize("cta_group", [1, 2])
def test_flex_cga_fallback_is_the_mma_mode_minimum(cta_group):
    ...
    assert C._flex_cga_fallback(wide, cta_group, tmpl) == (cta_group, 1)
    minimal = by_name(f"CONFIG_sm107_128x128x128_128x128x64_cluster{cta_group}x1")
    assert C._flex_cga_fallback(minimal, cta_group, tmpl) is None


@requires_sm107
def test_flex_cga_is_off_where_it_cannot_be_honored(monkeypatch):
    ...
    assert C._flex_cga_fallback(wide, 2, sm107_tmpl) == (2, 1)

    # Template that never reads the constant -> no fallback attached.
    assert not C._template_reads_fallback_cluster("sm100_block_scale_matmul_2ctamma.py")
    assert C._flex_cga_fallback(wide, 2, "sm100_block_scale_matmul_2ctamma.py") is None

    # Substitution is a floor, not a range: every part from SM 10.0 up can do it.
    assert C._flex_cga_supported(100) and C._flex_cga_supported(110)
    # A pre-Blackwell part -> plain fixed cluster, as before.
    monkeypatch.setattr(C, "_current_arch", lambda: 90)
    assert not C._flex_cga_supported()
    assert C._flex_cga_fallback(wide, 2, sm107_tmpl) is None
    monkeypatch.undo()

    # A pinned N-super-block walk -> skipped rather than silently mis-tiled.
    pinned = dataclasses.replace(wide, tile_swizzle_n=8)
    assert C._flex_cga_fallback(pinned, 2, sm107_tmpl) is None

    # The escape hatch for A/B measurement.
    monkeypatch.setenv("CUDNN_FROST_DISABLE_FLEX_CGA", "1")
    assert C._flex_cga_fallback(wide, 2, sm107_tmpl) is None


@requires_sm107
def test_flex_cga_leaves_unported_templates_byte_identical():
    """An sm100 template on this GPU gets no fallback in its launch — so its
    generated kernel, including the L2 swizzle knob the flexible path pins, is
    exactly what it was before flexible CGA existed."""
    ...
    compiled = _plan(g, **_kw(sm100_cfg))
    src = compiled.generated_path.read_text()
    assert "fallback_cluster=" not in src
