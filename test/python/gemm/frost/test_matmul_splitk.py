...
def _mk_graph(B, M, N, K, io_dt, out_dt):
...
_DT = {
    "bf16": (cudnn.data_type.BFLOAT16, torch.bfloat16, torch.bfloat16),
    "fp16": (cudnn.data_type.HALF, torch.float16, torch.float16),
    "bf16->fp32": (cudnn.data_type.BFLOAT16, torch.bfloat16, torch.float32),
}
_OUT_DT = {"bf16": cudnn.data_type.BFLOAT16, "fp16": cudnn.data_type.HALF, "bf16->fp32": cudnn.data_type.FLOAT}


def _data(B, M, N, K, torch_in, torch_out):
    torch.manual_seed(0)
...
# Shapes chosen for the axes split-K perturbs: K tails, M tails, batch, deep K.
_SHAPES = ((1, 256, 256, 4096), (1, 256, 256, 4160), (1, 300, 256, 8192), (3, 256, 512, 4096), (1, 128, 128, 16384))


@requires_sm100
@pytest.mark.parametrize("dt", sorted(_DT))
@pytest.mark.parametrize("S", (2, 3, 8))
@pytest.mark.parametrize("shape", _SHAPES, ids=lambda s: "x".join(map(str, s)))
def test_splitk_parity(shape, S, dt):
    B, M, N, K = shape
    io_dt, torch_in, torch_out = _DT[dt]
    g, A, Bt, C = _mk_graph(B, M, N, K, io_dt, _OUT_DT[dt])
    compiled = _jit(g, S)
    a, b, c, ref = _data(B, M, N, K, torch_in, torch_out)
...
def test_splitk_n_not_multiple_of_4(N):
    ...
    g, A, Bt, C = _mk_graph(1, 256, N, 4096, cudnn.data_type.BFLOAT16, cudnn.data_type.FLOAT)
    compiled = _jit(g, 8)
    a, b, c, ref = _data(1, 256, N, 4096, torch.bfloat16, torch.float32)
    _run(compiled, {A: a, Bt: b, C: c})
...
def test_splitk_cta_group1():
    ...
    g, A, Bt, C = _mk_graph(1, 256, 256, 8192, cudnn.data_type.BFLOAT16, cudnn.data_type.BFLOAT16)
    ...
    a, b, c, ref = _data(1, 256, 256, 8192, torch.bfloat16, torch.bfloat16)
    _run(compiled, {A: a, Bt: b, C: c})
    assert torch.equal(c, ref)
...
def test_splitk_deterministic():
    g, A, Bt, C = _mk_graph(1, 256, 256, 8192, cudnn.data_type.BFLOAT16, cudnn.data_type.BFLOAT16)
    compiled = _jit(g, 8)
    a, b, c1, _ = _data(1, 256, 256, 8192, torch.bfloat16, torch.bfloat16)
...
def test_splitk_workspace_size_and_missing_workspace():
    ...
    g, A, Bt, C = _mk_graph(B, M, N, K, cudnn.data_type.BFLOAT16, cudnn.data_type.BFLOAT16)
    compiled = _jit(g, S)
    assert compiled.workspace_bytes == -(-S * B * M * N * 4 // 128) * 128
    a, b, c, _ = _data(B, M, N, K, torch.bfloat16, torch.bfloat16)
...
def test_splitk_more_slices_than_k_tiles():
    ...
    g, A, Bt, C = _mk_graph(1, 256, 256, 128, cudnn.data_type.BFLOAT16, cudnn.data_type.BFLOAT16)
    compiled = _jit(g, 4)
    a, b, c, _ = _data(1, 256, 256, 128, torch.bfloat16, torch.bfloat16)
    from cudnn.frost.workspace import Workspace
...
def test_splitk_rejects_epilogue_fusion():
    ...
    A = g.tensor(name="A", dim=[1, 256, 512], stride=[256 * 512, 512, 1])
    Bt = g.tensor(name="B", dim=[1, 512, 256], stride=[512 * 256, 1, 512])
    Bias = g.tensor(name="bias", dim=[1, 1, 256], stride=[256, 256, 1])
    mm = g.matmul(name="mm", A=A, B=Bt)
    out = g.add(name="add", a=mm, b=Bias)
...
def test_splitk_auto_select():
    """Pure-function checks of the cuDNN-derived auto-split heuristic (fixed
    sm_count so no GPU is needed)."""
    from cudnn.gemm.frost.compiler import _auto_split_k
    from cudnn.gemm.frost.graph_analyzer import analyze_with_binding
    from cudnn.gemm.frost.tile_config import DEFAULT_CONFIG

    def slices(B, M, N, K, cfg=DEFAULT_CONFIG, sm=148):
        chain, _ = analyze_with_binding(_mk_graph(B, M, N, K, cudnn.data_type.BFLOAT16, cudnn.data_type.BFLOAT16)[0])
        return _auto_split_k(chain, cfg, sm_count=sm).split_k_slices

    # Latency-bound + deep K: one full wave (148 // 2 = 74) hits the S=32 cap
    # (the reducer unrolls chained loads only up to S=32).
    assert slices(1, 256, 256, 16384) == 32
    # Compute-bound: grid already fills the GPU.
    assert slices(1, 4096, 4096, 8192) == 1
    # Shallow K: per-slice floor (2 CTA-K tiles of 64 bf16 elems) wins.
    assert slices(1, 256, 256, 1024) == 8
    # K below the 2048-byte enable threshold (measured: S>1 loses at K<=512 bf16).
    assert slices(1, 256, 256, 512) == 1
    # grid.z cap: batch * S <= 65535.
    assert 60000 * slices(60000, 128, 128, 65536) <= 65535
    # Fused graph never auto-splits (gate reused).
    g = cudnn.pygraph(io_data_type=cudnn.data_type.BFLOAT16, intermediate_data_type=cudnn.data_type.FLOAT, compute_data_type=cudnn.data_type.FLOAT)
    A = g.tensor(name="A", dim=[1, 256, 8192], stride=[256 * 8192, 8192, 1])
    Bt = g.tensor(name="B", dim=[1, 8192, 256], stride=[8192 * 256, 1, 8192])
    Bias = g.tensor(name="bias", dim=[1, 1, 256], stride=[256, 256, 1])
    out = g.add(name="add", a=g.matmul(name="mm", A=A, B=Bt), b=Bias)
    out.set_output(True).set_data_type(cudnn.data_type.BFLOAT16)
    chain, _ = analyze_with_binding(g)
    from cudnn.gemm.frost.compiler import _auto_split_k as auto
    from cudnn.gemm.frost.tile_config import DEFAULT_CONFIG as dc

    assert auto(chain, dc, sm_count=148).split_k_slices == 1
