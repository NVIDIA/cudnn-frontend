...
def test_reordered_scale_blob_keeps_physical_capacity(capacity):
    """Per-group scale padding can exceed the graph's global logical padding."""
    ...
    dequant = g.block_scale_dequantize(token, scale, block_size=[1, 32])
    dequant.set_output(True).set_data_type(cudnn.data_type.FLOAT)
    ...
    assert list(operand.shape) == [1, capacity, 1]
...
@pytest.mark.L0
def test_reordered_batched_flat_scale_blob_preserves_batch_count():
    g = cudnn.pygraph(intermediate_data_type=cudnn.data_type.FLOAT, compute_data_type=cudnn.data_type.FLOAT)
    ...
    blob = torch.empty(4096, dtype=torch.uint8, device="cuda")
    pack = g._normalize({scale.get_uid(): blob}, None)
    operand = pack.operand(pack.index_of(scale))
    assert list(operand.shape) == [4, 1024, 1]
    assert list(operand.stride()) == [1024, 1, 1]
    assert operand.data_ptr() == blob.data_ptr()


@pytest.mark.L0
def test_reordered_scale_blob_does_not_flatten_overlapping_view():
    g = cudnn.pygraph(intermediate_data_type=cudnn.data_type.FLOAT, compute_data_type=cudnn.data_type.FLOAT)
    ...
    g.block_scale_dequantize(token, scale, block_size=[1, 32]).set_output(True).set_data_type(cudnn.data_type.FLOAT)
    # Span equals numel, but the view repeats offsets and is not contiguous.
    blob = torch.empty(4, dtype=torch.uint8, device="cuda").as_strided((2, 2), (3, 0))
    pack = g._normalize({scale.get_uid(): blob}, None)
    operand = pack.operand(pack.index_of(scale))
    assert list(operand.shape) == [2, 2]
    assert list(operand.stride()) == [3, 0]
