...
def _run_fprop_case(
    ...
    o, fs = run_gdn(
        q, k, v, g, b, scale=scale, initial_state=S0, output_final_state=True, cu_seqlens=cu_seqlens
    )
    torch.cuda.synchronize()

    with torch.no_grad():
        o_ref, fs_ref = gdn_reference(
            q, k, v, g, b, scale=scale, initial_state=S0, cu_seqlens=cu_seqlens
        )
...
def _dummy_inputs(H=2, HV=2, B=1, T=64, K=128, V=128):
    q, k, v = gen_qkv(B, T, H, HV, K, V, torch.bfloat16)
    g, b = gen_gates(B, T, HV, torch.bfloat16)
    return q, k, v, g, b


def test_invalid_qk_head_mismatch():
    from cudnn.experimental.ops.linear_attention._gdn_chunk_cutile import chunk_gated_delta_rule

    q, k, v, g, b = _dummy_inputs()
    with pytest.raises(ValueError, match="same number of heads"):
        chunk_gated_delta_rule(q, k[:, :, :1], v, g, b)
...
def test_invalid_gva_head_ratio():
    ...
    with pytest.raises(ValueError, match="divisible"):
        chunk_gated_delta_rule(q, k, v.repeat(1, 1, 3, 1)[:, :, :3], g, b)


def test_invalid_varlen_batch():
    from cudnn.experimental.ops.linear_attention._gdn_chunk_cutile import chunk_gated_delta_rule

    q, k, v, g, b = _dummy_inputs(B=2)
    cu = torch.tensor([0, 64], dtype=torch.int32, device="cuda")
    with pytest.raises(ValueError, match="Batch size must be 1"):
        chunk_gated_delta_rule(q, k, v, g, b, cu_seqlens=cu)


def test_invalid_initial_state_count():
    from cudnn.experimental.ops.linear_attention._gdn_chunk_cutile import chunk_gated_delta_rule

    q, k, v, g, b = _dummy_inputs(B=1, T=128)
    ...
    with pytest.raises(ValueError, match="initial states"):
        chunk_gated_delta_rule(q, k, v, g, b, cu_seqlens=cu, initial_state=S0)
