...
class MhaTestSuite:
    ...
    def test_gnn_mha_forward_backward(self, graph_data, dtype, concat_heads, with_edge, deterministic):
    ...
    @pytest.mark.L0
    @pytest.mark.parametrize("deterministic", [False, True])
    def test_gnn_mha_bipartite_forward_backward(self, deterministic):
        variant = self.variant
        _require_gnn_mha(variant)
        ...
        edge_features = torch.randn((5, 8 if variant == "gat_v2" else 4), device="cuda", requires_grad=True)
        attn_weights = torch.randn((8 if variant == "gat_v2" else 20,), device="cuda", requires_grad=True)
        op = self.op
        ...
        torch.autograd.backward((actual_output, actual_attention), (grad_output, grad_attention))
        torch.autograd.backward((expected_output, expected_attention), (grad_output, grad_attention))
        torch.testing.assert_close(src_features.grad, reference_src.grad, atol=6e-3, rtol=6e-3)
        torch.testing.assert_close(dst_features.grad, reference_dst.grad, atol=6e-3, rtol=6e-3)
        torch.testing.assert_close(edge_features.grad, reference_edge.grad, atol=6e-3, rtol=6e-3)
        torch.testing.assert_close(attn_weights.grad, reference_weights.grad, atol=6e-3, rtol=6e-3)

    @pytest.mark.L0
    def test_gnn_mha_default_returns_only_output(self):
        variant = self.variant
        op = self.op
    ...
    @pytest.mark.L0
    def test_gnn_mha_rejects_invalid_inputs(self):
        op = self.op
        graph = CscGraph(
            torch.tensor([0, 1], device="cuda", dtype=torch.int32),
    ...
    @pytest.mark.L0
    def test_gnn_mha_torch_compile(self):
        variant = self.variant
        op = self.op
        weight_size = self.weight_size
        _require_gnn_mha(variant)
    ...
    def test_gnn_mha_activations(self, activation):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
            torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32),
    ...
    def test_gnn_mha_attention_edge_order(self, mapped):
        variant = self.variant
        _require_gnn_mha(variant)
        ...
        expected_output, expected_attention = _reference(variant, graph, node, node, edge, weights, dropout_mask, 2, True, "leaky_relu", 0.2)
        torch.testing.assert_close(actual_output, expected_output, atol=5e-4, rtol=5e-4)
        torch.testing.assert_close(actual_attention, expected_attention, atol=5e-4, rtol=5e-4)
    ...
    def test_gnn_mha_selective_gradients(self, deterministic, grad_target):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
            torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32),
    ...
    @pytest.mark.L0
    @pytest.mark.parametrize("deterministic", [False, True])
    def test_gnn_mha_empty_destination_set(self, deterministic):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
            torch.tensor([0], device="cuda", dtype=torch.int32),
    ...
    @pytest.mark.L0
    @pytest.mark.parametrize("deterministic", [False, True])
    def test_gnn_mha_zero_degree_destination(self, deterministic):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
        ...
        reference_node = node.detach().clone().requires_grad_()
        reference_weights = weights.detach().clone().requires_grad_()
        expected, _ = _reference(variant, graph, reference_node, reference_node, None, reference_weights, None, 2, True, "leaky_relu", 0.2)
        torch.testing.assert_close(actual, expected, atol=5e-4, rtol=5e-4)
    ...
    @pytest.mark.L0
    def test_gnn_mha_uses_current_stream(self):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
        ...
        stream.synchronize()
        expected, _ = _reference(variant, graph, node, node, None, weights, None, 2, True, "leaky_relu", 0.2)
        torch.testing.assert_close(actual, expected, atol=5e-4, rtol=5e-4)

    @pytest.mark.L0
    def test_gnn_mha_backward_initializes_cuda_context_on_new_thread(self):
        variant = self.variant
        _require_gnn_mha(variant)
    ...
    @pytest.mark.L0
    def test_gnn_mha_deterministic_gradients_are_repeatable(self):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
            torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32),
    ...
    def test_gnn_mha_head_dimensions(self, num_heads, dim_head):
        variant = self.variant
        _require_gnn_mha(variant)
        graph = CscGraph(
            torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32),
