...
from gnn._mha_test_utils import _require_gnn_mha, _reference_gat_v2, _tolerances
...
def test_gnn_mha_gat_v2_forward_backward(small_reverse_csc_graph, dtype, concat_heads, with_edge, deterministic):
    ...
    expected_output, expected_attention = _reference_gat_v2(
...
@pytest.mark.L2
def test_gnn_mha_gat_v2_medium_graph(medium_reverse_csc_graph):
    _require_gnn_mha("gat_v2")
    ...
    edge = torch.randn((graph.num_edges, dim_node), device="cuda", requires_grad=True)
    weights = torch.randn((dim_node,), device="cuda", requires_grad=True)
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_default_returns_only_output():
    _require_gnn_mha("gat_v2")
    ...
    weights = torch.randn((8,), device="cuda", requires_grad=True)
    output = mha_gat_v2(graph, node, weights, num_heads=2)
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_rejects_invalid_inputs():
    ...
    with pytest.raises(TypeError, match="features must be a Tensor or a .* tuple"):
        mha_gat_v2(graph, (node,), weights)
    with pytest.raises(ValueError, match="dst_features must have shape"):
        mha_gat_v2(graph, (node, torch.randn((2, 8), device="cuda")), weights)
    with pytest.raises(ValueError, match="divisible by num_heads"):
        mha_gat_v2(graph, node, weights, num_heads=3)
    with pytest.raises(ValueError, match="attn_weights must have shape"):
        mha_gat_v2(graph, node, weights[:-1])
    with pytest.raises(ValueError, match="Unsupported activation"):
        mha_gat_v2(graph, node, weights, activation="gelu")
    with pytest.raises(TypeError, match="dropout_mask must have dtype float32"):
        mha_gat_v2(graph, node, weights, dropout_mask=torch.ones((1, 1), device="cuda", dtype=torch.float16))
    with pytest.raises(ValueError, match="deterministic backward requires"):
        mha_gat_v2(graph, node, weights, deterministic=True)
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_torch_compile():
    _require_gnn_mha("gat_v2")
    graph = CscGraph(torch.tensor([0, 2], device="cuda", dtype=torch.int32), torch.tensor([0, 1], device="cuda", dtype=torch.int32), num_src_nodes=2)

    def fn(node, weights):
        return mha_gat_v2(graph, node, weights, num_heads=2)
...
def test_gnn_mha_gat_v2_activations(activation):
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
    ...
    edge = torch.randn((5, 8), device="cuda", requires_grad=True)
    weights = torch.randn((8,), device="cuda", requires_grad=True)
    actual = mha_gat_v2(graph, node, weights, edge_features=edge, num_heads=2, activation=activation, activation_alpha=0.3)
    reference_node = node.detach().clone().requires_grad_()
    reference_edge = edge.detach().clone().requires_grad_()
    reference_weights = weights.detach().clone().requires_grad_()
    expected, _ = _reference_gat_v2(graph, reference_node, reference_node, reference_edge, reference_weights, None, 2, True, activation, 0.3)
...
def test_gnn_mha_gat_v2_attention_edge_order(mapped):
    _require_gnn_mha("gat_v2")
    ...
    edge = torch.randn((5, 8), device="cuda")
    weights = torch.randn((8,), device="cuda")
    dropout_mask = torch.tensor([[0.0, 1.25, 1.25, 1.25, 0.0], [1.25, 0.0, 1.25, 1.25, 1.25]], device="cuda")
    actual_output, actual_attention = mha_gat_v2(
        graph, node, weights, edge_features=edge, dropout_mask=dropout_mask, num_heads=2, return_attention_weights=True
    )
    expected_output, expected_attention = _reference_gat_v2(graph, node, node, edge, weights, dropout_mask, 2, True, "leaky_relu", 0.2)
    torch.testing.assert_close(actual_output, expected_output, atol=0.0005, rtol=0.0005)
    torch.testing.assert_close(actual_attention, expected_attention, atol=0.0005, rtol=0.0005)
...
def test_gnn_mha_gat_v2_selective_gradients(deterministic, grad_target):
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
        torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32), torch.tensor([0, 2, 1, 2, 3], device="cuda", dtype=torch.int32), num_src_nodes=4
    ).with_reverse_csc()
    node = torch.randn((4, 8), device="cuda", requires_grad=grad_target == "node")
    edge = torch.randn((5, 8), device="cuda", requires_grad=grad_target == "edge")
    weights = torch.randn((8,), device="cuda", requires_grad=grad_target == "weights")
    mha_gat_v2(graph, node, weights, edge_features=edge, num_heads=2, deterministic=deterministic).sum().backward()
...
@pytest.mark.L0
@pytest.mark.parametrize("deterministic", [False, True])
def test_gnn_mha_gat_v2_empty_destination_set(deterministic):
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
    ...
    weights = torch.randn((8,), device="cuda", requires_grad=True)
    output, attention = mha_gat_v2(graph, node, weights, num_heads=2, return_attention_weights=True, deterministic=deterministic)
...
@pytest.mark.L0
@pytest.mark.parametrize("deterministic", [False, True])
def test_gnn_mha_gat_v2_zero_degree_destination(deterministic):
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
    ...
    node = torch.randn((3, 8), device="cuda", requires_grad=True)
    weights = torch.randn((8,), device="cuda", requires_grad=True)
    actual = mha_gat_v2(graph, node, weights, num_heads=2, deterministic=deterministic)
    reference_node = node.detach().clone().requires_grad_()
    reference_weights = weights.detach().clone().requires_grad_()
    expected, _ = _reference_gat_v2(graph, reference_node, reference_node, None, reference_weights, None, 2, True, "leaky_relu", 0.2)
    torch.testing.assert_close(actual, expected, atol=0.0005, rtol=0.0005)
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_uses_current_stream():
    _require_gnn_mha("gat_v2")
    ...
    stream.synchronize()
    expected, _ = _reference_gat_v2(graph, node, node, None, weights, None, 2, True, "leaky_relu", 0.2)
    torch.testing.assert_close(actual, expected, atol=0.0005, rtol=0.0005)


@pytest.mark.L0
def test_gnn_mha_gat_v2_backward_initializes_cuda_context_on_new_thread():
    _require_gnn_mha("gat_v2")
    ...
    act_scores = torch.empty((2, 8), device="cuda")
    common_forward = (
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_deterministic_gradients_are_repeatable():
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
        torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32),
        torch.tensor([0, 2, 1, 2, 3], device="cuda", dtype=torch.int32),
    ...
    edge_value = torch.randn((5, 8), device="cuda")
    weight_value = torch.randn((8,), device="cuda")
    dropout_mask = torch.tensor([[0.0, 1.25, 1.25, 1.25, 0.0], [1.25, 0.0, 1.25, 1.25, 1.25]], device="cuda")
...
def test_gnn_mha_gat_v2_head_dimensions(num_heads, dim_head):
    _require_gnn_mha("gat_v2")
    graph = CscGraph(
        torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32), torch.tensor([0, 2, 1, 2, 3], device="cuda", dtype=torch.int32), num_src_nodes=4
    )
    ...
    weights = torch.randn((dim_node,), device="cuda", requires_grad=True)
    actual = mha_gat_v2(graph, node, weights, num_heads=num_heads, concat_heads=False)
    reference_node = node.detach().clone().requires_grad_()
    reference_weights = weights.detach().clone().requires_grad_()
    expected, _ = _reference_gat_v2(graph, reference_node, reference_node, None, reference_weights, None, num_heads, False, "leaky_relu", 0.2)
...
@pytest.mark.parametrize("deterministic", [False, True])
@pytest.mark.parametrize(
    ("input_dtype", "high_precision_grad"),
    [(torch.float16, False), (torch.float16, True), (torch.bfloat16, False), (torch.bfloat16, True), (torch.float32, True)],
)
def test_gnn_mha_gat_v2_gradient_precision(deterministic, input_dtype, high_precision_grad):
    _require_gnn_mha("gat_v2")
    grad_dtype = torch.float32 if high_precision_grad else input_dtype
    graph = CscGraph(
        torch.tensor([0, 2, 5], device="cuda", dtype=torch.int32), torch.tensor([0, 2, 1, 2, 3], device="cuda", dtype=torch.int32), num_src_nodes=4
    ).with_reverse_csc()
...
@pytest.mark.L0
def test_gnn_mha_gat_v2_rejects_invalid_precision_option():
    graph = CscGraph(torch.tensor([0, 1], device="cuda", dtype=torch.int32), torch.tensor([0], device="cuda", dtype=torch.int32), num_src_nodes=1)
    node = torch.randn((1, 8), device="cuda", dtype=torch.float16)
    weights = torch.randn((8,), device="cuda", dtype=torch.float16)
    with pytest.raises(TypeError, match="high_precision_grad must be a bool"):
        mha_gat_v2(graph, node, weights, high_precision_grad="true")
