...
"""
Backward (bprop) tests for the KDA cuTile kernel: gradients of q, k, v, g,
beta (and the initial state) are compared against autograd through the fp64
recurrent reference in ``reference_kda`` using a shared random upstream
gradient.
...
on/off, chunked prefill with state carry-over, initial-state (dh0) and
final-state (dht) gradient paths, and head-dim variants including K != V and
K = 256.
"""
...
def _run_bprop_case(
    ...
    g0, b0 = gen_kda_gates(B, Teff, HV, K, dtype, alpha=alpha, beta=beta)
...
def test_bprop_zero_length_sequence(dtype, H, HV, T=64):
    ...
    def run(cu):
        ...
        o, _ = run_kda(leaves["q"], leaves["k"], leaves["v"], leaves["g"], leaves["beta"], cu_seqlens=cu)
...
def test_bprop_chunked_prefill(dtype, T1, T2, H, HV, B=2, K=128, V=128):
    ...
    g0, b0 = gen_kda_gates(B, T, HV, K, dtype)
    ...
    ref_leaves = {n: t.detach().double().requires_grad_() for n, t in leaves.items()}
    o_ref, _ = kda_reference(ref_leaves["q"], ref_leaves["k"], ref_leaves["v"], ref_leaves["g"], ref_leaves["beta"])
