...
"""
Forward (fprop) tests for the KDA cuTile kernel, validated against the fp64
recurrent reference in ``reference_kda``.
...
explicit/auto scale, alpha (per-channel decay gate) and beta (write strength)
on/off, chunked prefill with state carry-over, initial state, and head-dim
variants including K != V and K = 256.
"""
...
def _run_fprop_case(
    ...
    g, b = gen_kda_gates(B, total_T or T, HV, K, dtype, alpha=alpha, beta=beta)
    ...
    o, fs = run_kda(q, k, v, g, b, scale=scale, initial_state=S0, output_final_state=True, cu_seqlens=cu_seqlens)
    torch.cuda.synchronize()

    with torch.no_grad():
        o_ref, fs_ref = kda_reference(q, k, v, g, b, scale=scale, initial_state=S0, cu_seqlens=cu_seqlens)
...
def test_fprop_zero_length_sequence(dtype, H, HV, T=64):
    ...
    g, b = gen_kda_gates(1, T, HV, 128, dtype)
    cu = torch.tensor([0, T], dtype=torch.int32, device="cuda")
    cu_with_empty = torch.tensor([0, T, T], dtype=torch.int32, device="cuda")

    o_ref, fs_ref = run_kda(q, k, v, g, b, output_final_state=True, cu_seqlens=cu)
    o, fs = run_kda(q, k, v, g, b, output_final_state=True, cu_seqlens=cu_with_empty)
    torch.cuda.synchronize()
...
def test_fprop_chunked_prefill(dtype, T1, T2, H, HV, B=2, K=128, V=128):
    ...
    g, b = gen_kda_gates(B, T, HV, K, dtype)
    ...
    with torch.no_grad():
        o_ref, fs_ref = kda_reference(q, k, v, g, b)
...
def test_invalid_qk_head_mismatch():
    ...
    with pytest.raises(Exception, match="head|No valid engine"):
        run_kda(q.unsqueeze(0), k[:, :1].unsqueeze(0), v.unsqueeze(0), g.unsqueeze(0), b.unsqueeze(0), cu_seqlens=_cu(0, 64))
...
def test_invalid_gva_head_ratio():
    ...
    with pytest.raises(Exception, match="divisible|multiple|head|No valid engine"):
        run_kda(q.unsqueeze(0), k.unsqueeze(0), v.repeat(1, 3, 1)[:, :3].unsqueeze(0), g.unsqueeze(0), b.unsqueeze(0), cu_seqlens=_cu(0, 64))
...
def test_invalid_initial_state_count():
    ...
    with pytest.raises(Exception, match="initial"):
        run_kda(q.unsqueeze(0), k.unsqueeze(0), v.unsqueeze(0), g.unsqueeze(0), b.unsqueeze(0), cu_seqlens=_cu(0, 64, 128), initial_state=S0)
