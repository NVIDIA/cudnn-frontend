...
_NAMES = {torch.float32: "float32", torch.bfloat16: "bfloat16", torch.int64: "int64"}
...
def test_resolve_inputs_converts_on_the_default_stream(no_external_stream):
    g = torch.randn(3, 8, device="cuda").to(torch.bfloat16)
    cu = torch.tensor([0, 5, 8], device="cuda", dtype=torch.int64)
    addr, keepalive = marshal.resolve_inputs(["g", "cu_seqlens"], [_view(g), _view(cu)], "test", 0, want={"g": "float32", "cu_seqlens": "int32"})
    torch.cuda.synchronize()
    assert [addr["g"], addr["cu_seqlens"]] == [buf.data_ptr() for buf in keepalive]
    assert torch.equal(keepalive[0], g.float())
    assert torch.equal(keepalive[1], cu.to(torch.int32))
...
def test_staged_output_writes_back_on_the_default_stream(no_external_stream):
    ...
    ptr, staged, destination = marshal.stage_output(_view(final_state), "float32", 0)
    assert staged is not None and ptr == staged.data_ptr() and staged.dtype == torch.float32
    staged.copy_(torch.randn_like(staged))
    marshal.write_back(staged, destination, 0)
    torch.cuda.synchronize()
    assert torch.equal(final_state, staged.to(torch.bfloat16))
