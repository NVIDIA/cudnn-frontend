...
"""Example 04: KDA (Kimi Delta Attention) backward (pure cuDNN frontend API).

The KDA_BWD node takes the forward inputs plus ``dO`` and returns
``(dQ, dK, dV, dG, dBeta)`` (per-key-channel ``dG``). Without the optional
per-chunk ``h`` input the engine recomputes the forward state pass
internally. Gradients are checked against fp64 autograd through the
...
def main(seq_lens=(192, 320), H: int = 2, D: int = 128) -> None:
    ...
    pack = {q_t: q, k_t: k, v_t: v, g_t: gate, beta_t: beta, cu_t: cu, do_t: do}
    pack.update(dict(zip(grads_t, (dq, dk, dv, dg, db))))
    ...
    for name, out, ref in (("dQ", dq, grads[0]), ("dK", dk, grads[1]), ("dV", dv, grads[2]), ("dG", dg, grads[3]), ("dBeta", db, grads[4])):
        r = _rms_ratio(out, ref)
        assert r < 5e-2, f"{name} rms ratio {r:.4g}"
    print(f"[04] PASS  kda backward (recompute)    seq_lens={list(seq_lens)} H={H} D={D}")
