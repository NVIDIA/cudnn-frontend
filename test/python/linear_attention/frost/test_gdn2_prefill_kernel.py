...
"""FROST GDN-2 prefill tests (``pygraph`` + ``Gdn2FrostEngine``) against the
...
from linear_attention.reference_gdn2 import gdn2_reference, rms_ratio
...
def _gen_thd(seq_lens, H, HV, head_size, dtype, HK=None):
    ...
    k = F.normalize(multidist_randu(total * HK, head_size, device="cuda").reshape(total, HK, head_size), p=2.0, dim=-1)
...
def _build_gdn2_engine_graph(
    ...
    *,
    ...
    beta_t = g.tensor([total, HO, D], data_type=io_dt, name="beta")
    w_t = g.tensor([total, HO, D], data_type=io_dt, name="w")
    cu_t = g.tensor([num_seqs + 1], data_type=cudnn.data_type.INT32, name="cu_seqlens")
    t = dict(q=q_t, k=k_t, v=v_t, g=g_t, beta=beta_t, w=w_t, cu=cu_t)
    if with_s0:
    ...
    O_t, fs_t, h_t = g.gdn2(
        ...
        use_beta_w_sigmoid=use_beta_w_sigmoid,
        name="gdn2",
    )
...
def _run_gdn2(q, k, v, gate, beta, w, scale, cu, initial_state=None, output_state=None, out_h=None, every_n=0, use_qk_l2norm=True):
    """Torch adapter over the graph.  ``gate`` is the per-key-
    channel natural-log decay (fp32); ``beta``/``w`` are the io-dtype
    channel-wise gates; state ports are K-major ``[N, HO, K, V]``.  GQA ``k``
    (and ``HV < HQ`` ``v``) are pre-broadcast: the node serves HK == HQ, HV a
    multiple of HQ."""
    device = q.device
...
def _run_and_check(dtype, H, HV, seq_lens, with_s0=False, HK=None, gate_lo=None):
    ...
    beta = (torch.rand(total, HO, head_size, device="cuda").sigmoid() * 2.0).to(dtype)
    w = torch.rand(total, HO, head_size, device="cuda").sigmoid().to(dtype)
    ...
    output, _ = _run_gdn2(q, k, v, gate, beta, w, scale, _cu(seq_lens), initial_state=s0, output_state=output_state)
    torch.cuda.synchronize()

    with torch.no_grad():
        o_ref, fs_ref = gdn2_reference(
    ...
    torch.testing.assert_close(output.float(), o_ref.squeeze(0).float(), atol=tol, rtol=tol)
    assert rms_ratio(output_state, fs_ref) < 5e-2  # engine state ports are K-major, like the reference
...
def test_gdn2_kernel_basic(dtype, H, HV, seq_lens):
...
def test_gdn2_kernel_seqlen_edges(seq_lens):
...
def test_gdn2_kernel_seqlen_edges_fp16(seq_lens):
    _run_and_check(torch.float16, 2, 4, seq_lens)


@requires_runtime
@pytest.mark.parametrize("seq_lens", [[256], [64, 129, 512]])
def test_gdn2_kernel_initial_state(seq_lens):
    _run_and_check(torch.bfloat16, 2, 2, seq_lens, with_s0=True)


@requires_runtime
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gdn2_beta_w_sigmoid_in_kernel(dtype):
    """use_beta_w_sigmoid: io-dtype beta/w logits + in-kernel sigmoid must
    match the host-side sigmoid path (approx-tanh sigmoid and the io-dtype
    roundtrip differ by at most ~1 ulp per gate)."""
    _seed()
    seq_lens, H, D = [256, 129], 2, 128
    total, num_seqs = sum(seq_lens), len(seq_lens)
    q, k, v = _gen_thd(seq_lens, H, H, D, dtype)
    gate = torch.empty(total, H, D, device="cuda").uniform_(0.5, 1.0).log().float().contiguous()
    beta_logits = torch.randn(total, H, D, device="cuda")
    w_logits = torch.randn(total, H, D, device="cuda")
    ...
    for in_kernel in (True, False):
        if in_kernel:
            beta, w = beta_logits.to(dtype).contiguous(), w_logits.to(dtype).contiguous()
        else:
            beta = beta_logits.to(dtype).float().sigmoid().to(dtype).contiguous()
            w = w_logits.to(dtype).float().sigmoid().to(dtype).contiguous()
        g, t = _build_gdn2_engine_graph(total, H, D, num_seqs, scale, io_dt=_DT[dtype], use_beta_w_sigmoid=in_kernel)
        g.build()
        o = torch.empty(total, H, D, dtype=dtype, device="cuda")
        fs = torch.empty(num_seqs, H, D, D, dtype=torch.float32, device="cuda")
        pack = {t["q"]: q, t["k"]: k, t["v"]: v, t["g"]: gate, t["beta"]: beta, t["w"]: w, t["cu"]: cu, t["O"]: o, t["fs"]: fs}
...
def test_gdn2_zero_length_sequence_state_passthrough(with_initial_state):
    ...
    gate = torch.empty(seq_len, H, D, device="cuda").uniform_(0.5, 1.0).log().float().contiguous()
    beta = (torch.rand(seq_len, H, D, device="cuda").sigmoid() * 2.0).to(torch.bfloat16)
    ...
    _run_gdn2(q, k, v, gate, beta, w, 1.0 / math.sqrt(D), _cu([seq_len, 0]), initial_state=s0, output_state=fs)
    torch.cuda.synchronize()
...
def test_gdn2_kernel_head_configs(HQ, HK, HV):
...
def test_gdn2_kernel_head_configs_fp16(HQ, HK, HV):
    _run_and_check(torch.float16, HQ, HV, [64, 128, 512], HK=HK)


@requires_runtime
def test_gdn2_chunked_prefill():
    ...
    beta = (torch.rand(T1 + T2, H, D, device="cuda").sigmoid() * 2.0).to(dtype)
    w = torch.rand(T1 + T2, H, D, device="cuda").sigmoid().to(dtype)
    scale = 1.0 / math.sqrt(D)

    fs_full = torch.full((1, H, D, D), float("nan"), dtype=torch.float32, device="cuda")
    o_full, _ = _run_gdn2(q, k, v, gate, beta, w, scale, _cu([T1 + T2]), output_state=fs_full)

    fs1 = torch.full((1, H, D, D), float("nan"), dtype=torch.float32, device="cuda")
    o1, _ = _run_gdn2(q[:T1], k[:T1], v[:T1], gate[:T1], beta[:T1], w[:T1], scale, _cu([T1]), output_state=fs1)
...
@requires_runtime
def test_gdn2_kernel_state_dtype_bf16():
    ...
    beta = (torch.rand(total, H, D, device="cuda").sigmoid() * 2.0).to(dtype)
    w = torch.rand(total, H, D, device="cuda").sigmoid().to(dtype)
    scale = 1.0 / math.sqrt(D)
    s0 = (torch.randn(num_seqs, H, D, D, dtype=torch.float32, device="cuda") * 0.05).to(dtype).contiguous()
    fs = torch.full((num_seqs, H, D, D), float("nan"), dtype=dtype, device="cuda")
    o, _ = _run_gdn2(q, k, v, gate, beta, w, scale, _cu(seq_lens), initial_state=s0, output_state=fs)
    torch.cuda.synchronize()
...
# ---------------------------------------------------------------------------
# Gdn2FrostEngine: graph-level coverage through the router
# ---------------------------------------------------------------------------


def _gdn2_engine_inputs(seq_lens, H, D):
    from linear_attention.conftest import gen_gdn2_gates, gen_qkv
    ...
    return (x.squeeze(0).contiguous() for x in (q, k, v, gate, beta, w)), cu
...
def test_gdn2_frost_engine_matches_reference(seq_lens, H=2, D=128):
    ...
    g, t = _build_gdn2_engine_graph(total, H, D, num_seqs, scale)
    g.build()
    assert isinstance(g.selected_engine, Gdn2FrostEngine)
    ...
    pack = {t["q"]: q, t["k"]: k, t["v"]: v, t["g"]: gate, t["beta"]: beta, t["w"]: w, t["cu"]: cu, t["O"]: o_buf, t["fs"]: fs_buf}
    g.execute(pack, torch.empty(g.get_workspace_size(), dtype=torch.uint8, device="cuda"))
    torch.cuda.synchronize()

    # the kernel L2-normalizes q/k internally
    with torch.no_grad():
        o_ref, fs_ref = gdn2_reference(
            F.normalize(q.float(), dim=-1).unsqueeze(0),
            F.normalize(k.float(), dim=-1).unsqueeze(0),
...
def test_gdn2_frost_engine_no_l2norm_matches_reference(seq_lens=(256,), H=2, D=128):
    ...
    g, t = _build_gdn2_engine_graph(total, H, D, num_seqs, scale, use_qk_l2norm=False)
    g.build()
    assert isinstance(g.selected_engine, Gdn2FrostEngine)
...
def test_gdn2_frost_engine_initial_state(seq_lens=(128, 256), H=2, D=128):
    ...
    g, t = _build_gdn2_engine_graph(total, H, D, num_seqs, scale, with_s0=True)
    g.build()
    assert isinstance(g.selected_engine, Gdn2FrostEngine)

    o_buf = torch.empty(total, H, D, dtype=torch.bfloat16, device="cuda")
    fs_buf = torch.empty(num_seqs, H, D, D, dtype=torch.float32, device="cuda")
    pack = {t["q"]: q, t["k"]: k, t["v"]: v, t["g"]: gate, t["beta"]: beta, t["w"]: w, t["cu"]: cu, t["s0"]: s0, t["O"]: o_buf, t["fs"]: fs_buf}
    g.execute(pack, torch.empty(g.get_workspace_size(), dtype=torch.uint8, device="cuda"))
    torch.cuda.synchronize()
...
def test_gdn2_frost_engine_declines_bwd():
    """GDN2_BWD declines (stub backward kernel on this branch)."""
    g, _t = _build_gdn2_engine_graph(256, 2, 128, 1, 0.125, bwd=True)
    assert_engine_declines(g, "gdn2_frost")


@requires_runtime
def test_gdn2_frost_engine_declines_wrong_head_dim():
    g, _t = _build_gdn2_engine_graph(256, 2, 64, 1, 0.125)
    assert_engine_declines(g, "gdn2_frost")
...
def _det_launch(seq_lens, H, HV, with_s0=False, stream=None):
    ...
    beta = (torch.rand(total, HO, 128, device="cuda").sigmoid() * 2.0).to(torch.bfloat16).contiguous()
    w = torch.rand(total, HO, 128, device="cuda").sigmoid().to(torch.bfloat16).contiguous()
    ...
    def launch():
        if stream is not None:
            with torch.cuda.stream(stream):
                return _run_gdn2(q, k, v, gate, beta, w, scale, cu, initial_state=s0)
        return _run_gdn2(q, k, v, gate, beta, w, scale, cu, initial_state=s0)
...
def test_gdn2_prefill_determinism(seq_lens):
    _seed()
    assert_bitwise_runs(_det_launch(seq_lens, 2, 4), label="gdn2")


@requires_runtime
def test_gdn2_prefill_determinism_initial_state():
    _seed()
    assert_bitwise_runs(_det_launch([256, 640, 0, 33], 2, 2, with_s0=True), label="gdn2+s0")


@requires_runtime
def test_gdn2_concurrent_streams_determinism():
    _seed()
...
# (the per-channel gate scan itself is covered in test_kda_prefill_kernel.py)
# ---------------------------------------------------------------------------
...
def test_gdn2_prefill_split_strong_decay(heads, with_s0):
...
@requires_runtime
def test_gdn2_prefill_cuda_graph_replay():
    ...
    beta = torch.zeros(T_cap, H, D, dtype=torch.bfloat16, device=dev)
    w = torch.zeros(T_cap, H, D, dtype=torch.bfloat16, device=dev)
    ...
    g, t = _build_gdn2_engine_graph(T_cap, H, D, B_cap, scale)
    g.build()
    ws = torch.empty(max(g.get_workspace_size(), 1), dtype=torch.uint8, device=dev)
    base = {t["q"]: q, t["k"]: k, t["v"]: v, t["g"]: gate, t["beta"]: beta, t["w"]: w, t["cu"]: cu}
    ...
    def fill(seq_lens):
        ...
        beta[:total] = (torch.rand(total, H, D, device=dev).sigmoid() * 2.0).bfloat16()
        w[:total] = torch.rand(total, H, D, device=dev).sigmoid().bfloat16()
