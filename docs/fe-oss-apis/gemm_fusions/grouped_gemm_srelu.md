...
from cudnn.jax import grouped_gemm_srelu_wrapper_sm100
...
`(L, K, N)`. The `C`, `D`, and `D_col` matrix results are fixed to
`output_layout="LMN"` and use shape `(1, valid_m, N)`. The expert mode `L`
must remain outermost in these strings.

Packed scale tensors, `prob`, and reduction outputs retain their specialized
shapes documented below. The JAX path supports dense FP8 A/B with E8M0 scales
and `sf_vec_size=32`; the FP8 D configuration above returns both quantized
layouts and their scale factors, with `amax=None`. Discrete weight pointers and
packed FP4 are not exposed. Shapes, dtypes, layouts, and configuration
