...
- **Maturity is per engine** (`EngineSlot.opt_in`, gated by
  ...
  withholds an optimization, never an operation: when the backend validates a
  graph but proposes no plan for it, or every plan it proposed declines at
  `check_support()` / build time, the family's opt-in engines are admitted as
  candidates without the flag (`pygraph._admit_opt_in_engines`; an explicit
  `create_execution_plan(engine_id, knobs)` replay of such a plan resolves the
  engine the same way) — the per-graph form of never gating a
  sole-implementation family. A graph the installed backend rejects at
  `validate()` still raises there.
