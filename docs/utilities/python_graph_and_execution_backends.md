...
- Sequence splitting. The main kernels run one persistent CTA per (sequence,
  ...
  wave of `B * P` unit-aligned pieces, `P = num_sm // (B * HO)` (at least 3
  for a forward plan and 2 for a backward plan, at most 16, each at least 4
  chunk units of `lcm(expand_num, checkpoint cadence)` chunks) wherever that
  has room; every piece spans `ceil(total_chunks / (B * P))` chunks whichever
  ...
  passthrough; the summary table only for sequences with two or more filled
  slots); a summary launch produces every multi-piece
