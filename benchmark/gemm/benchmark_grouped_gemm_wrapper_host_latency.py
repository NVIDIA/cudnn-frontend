...
"""Steady-state CPU submission latency for MXFP8 MoE training wrappers.

Run with CUDNN_FE_GROUPED_GEMM_DYNAMIC_MNKL=True and
CUDNNFE_CLUSTER_OVERLAP_MARGIN=8 to exercise common training settings.
Synchronization before each timed call drains prior GPU work; synchronization
and kernel completion are excluded from the measurement. This measures eager
wrapper submission, not training throughput or CUDA graph replay.
"""
...
def main():
    print(f"cuDNN source: {cudnn.__file__}")
    for name, default in (("CUDNN_FE_GROUPED_GEMM_DYNAMIC_MNKL", "1"), ("CUDNNFE_CLUSTER_OVERLAP_MARGIN", "0")):
        print(f"{name}={os.environ.get(name, default)}")
    torch.manual_seed(0)
