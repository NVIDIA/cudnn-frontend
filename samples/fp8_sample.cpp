                   ...
                   void* devPtrScale)
{
    ...
    try {
        ...
        auto afterDescale1Tensor = TensorBuilder()
                            .cloneFrom(dxTensor, 'a')
                            .build();
        
        auto wDescaleTensor = TensorBuilder()
                              .cloneFrom(dyDescaleTensor, 't')
                              .build();

        auto afterDescale2Tensor = TensorBuilder()
                                   .cloneFrom(dxTensor, 'b')
                                   .build();

        auto dxScaleTensor = TensorBuilder()
                             .cloneFrom(dyDescaleTensor, 'u')
                             .build();

        auto fp8OutputTensor = TensorBuilder()
                                .cloneFrom(dxTensor, 'c')
                                .setVirtual(false)
                                .setDataType(dataType)
                                .build();
