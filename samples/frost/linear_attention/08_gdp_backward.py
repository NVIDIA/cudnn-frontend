...
"""Example 08: GDP (Gated DeltaProduct) backward (pure cuDNN frontend API).

The GDP_BWD node takes the forward inputs plus ``dO`` and returns
``(dQ, dK, dV, dG, dBeta, dS0)``. ``dQ`` and ``dG`` live at the real tokens,
``dK``, ``dV`` and ``dBeta`` on the expanded timeline (``total * n`` rows, one
per Householder update). Without the optional per-chunk ``h`` input the
engine recomputes the forward state pass internally. Gradients are checked
against fp64 autograd through the per-token recurrence.
"""
...
def _reference_o(q, k, v, g, beta, cu, n, scale):
    """Differentiable fp64 per-token recurrence, ``n`` updates per token; returns o."""
    total, H, D = q.shape
    V = v.shape[2]
    outs = []
...
def main(seq_lens=(192, 320), H: int = 2, D: int = 128, n: int = 2) -> None:
    ...
    dQ_t, dK_t, dV_t, dG_t, dBeta_t, _dS0_t, _dA_t, _dDt_t = g.gdp_bwd(
        q=q_t,
        k=k_t,
        v=v_t,
        g=g_t,
    ...
    dk = torch.empty(total * n, H, D, dtype=torch.bfloat16, device=device)
    dv = torch.empty(total * n, H, D, dtype=torch.bfloat16, device=device)
    dg = torch.empty(total, H, dtype=torch.float32, device=device)
    db = torch.empty(total * n, H, dtype=torch.float32, device=device)
    ...
    o_ref = _reference_o(*leaves, cu, n, scale)
    ...
    print(f"[08] PASS  gdp backward (recompute)    seq_lens={list(seq_lens)} H={H} D={D} n={n}")
