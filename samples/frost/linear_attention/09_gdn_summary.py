...
"""Example 09: GDN (Gated DeltaNet) forward summary (pure cuDNN frontend API).

The GDN_SUMMARY node is the state-only pass behind context parallelism: it
reads no ``q`` and writes no ``o``. For every packed sequence it returns
...
def _build_plans(g) -> None:
    ...
    g.select_plan(names.index("gdn_summary_frost"))
    g.check_support()
    g.build_plans()
...
def main(spans=(128, 192), H: int = 2, D: int = 128) -> None:
    ...
    fs_t, transition_t = g.gdn_summary(
        k=k_t,
        v=v_t,
        g=g_t,
        beta=beta_t,
    ...
    print(f"[09] PASS  gdn summary                 spans={list(spans)} H={H} D={D} (fs rms {r_h:.2e}, transition rms {r_m:.2e}, composed rms {r_x:.2e})")
