...
"""Example 10: GDN (Gated DeltaNet) backward summary (pure cuDNN frontend API).

The GDN_SUMMARY_BWD node is the reverse state-gradient pass behind context
...
def main(spans=(128, 192), H: int = 2, D: int = 128) -> None:
    ...
    dS0_t, transition_t = g.gdn_summary_bwd(
        q=q_t,
        k=k_t,
        g=g_t,
        beta=beta_t,
    ...
    print(f"[10] PASS  gdn summary backward        spans={list(spans)} H={H} D={D} (dS0 rms {r_g:.2e}, transition rms {r_m:.2e}, composed rms {r_x:.2e})")
