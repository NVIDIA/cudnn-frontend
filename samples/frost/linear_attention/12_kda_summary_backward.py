...
"""Example 12: KDA (Kimi Delta Attention) backward summary (pure cuDNN frontend API).

The KDA_SUMMARY_BWD node is the reverse state-gradient pass behind context
parallelism, under KDA's per-key-channel decay. For every packed sequence it
returns ``d_initial_state`` ``[N, H, V, K]``, the gradient of ``sum(o * dO)``
with respect to the span's incoming state from a zero outgoing state gradient
(the optional ``d_final_state`` seeds it instead), and with
``output_transition=True`` the span ``transition`` ``[N, H, K, K]`` in the
backward's orientation, so that state gradients compose in reverse as::
...
state. ``use_qk_l2norm=False`` passes ``q``/``k`` through as given, so this
example feeds pre-normalized rows.
"""
...
def _reference(q, k, v, g, beta, cu, scale, s0):
...
def main(spans=(128, 192), H: int = 2, D: int = 128) -> None:
    ...
    dS0_t, transition_t = g.kda_summary_bwd(
        q=q_t,
        k=k_t,
        g=g_t,
        beta=beta_t,
    ...
    o_ref, _ = _reference(qd, kd, vd, gd, bd, cu, scale, s0)
    (dS0_ref,) = torch.autograd.grad((o_ref * dod).sum(), [s0])
    eye = torch.eye(D, dtype=torch.float64, device=device).expand(num_spans, H, D, D)
    transition_ref = _reference(qd, kd, torch.zeros_like(vd), gd, bd, cu, scale, eye)[1].transpose(-2, -1)
    ...
    o_uncut, _ = _reference(qd, kd, vd, gd, bd, cu_uncut, scale, s0_uncut)
    ...
    print(f"[12] PASS  kda summary backward        spans={list(spans)} H={H} D={D} (dS0 rms {r_g:.2e}, transition rms {r_m:.2e}, composed rms {r_x:.2e})")
