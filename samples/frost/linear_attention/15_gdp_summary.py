...
"""Example 15: GDP (Gated DeltaProduct) forward summary (pure cuDNN frontend API).

The GDP_SUMMARY node is the state-only pass behind context parallelism, with
GDP's ``n = num_householder`` delta-rule updates per token. It reads ``g`` at
the real tokens and ``k``, ``v``, ``beta`` on the expanded timeline
(``total * n`` rows, sub-token ``j`` of token ``t`` at row ``t * n + j``);
``cu_seqlens`` counts real tokens. For every packed sequence it returns
``final_state`` ``[N, H, V, K]``, the state after the span from a zero seed
(or from the optional ``initial_state``), and with ``output_transition=True``
the span ``transition`` ``[N, H, K, K]`` in the stored V-major domain, so that
consecutive spans compose as::
...
state of the uncut sequence. Decays near one keep the transition well away
from zero, so the cross-span term of the composition is exercised.
"""
...
def _reference_state(k, v, g, beta, cu, n, s0):
    """fp64 per-token recurrence over the packed batch from the V-major seed ``s0``, ``n`` updates per token.
    Returns the V-major final state."""
    total, H, D = g.shape[0], k.shape[1], k.shape[2]
    V = v.shape[2]
...
def main(spans=(128, 192), H: int = 2, D: int = 128, n: int = 2) -> None:
    ...
    fs_t, transition_t = g.gdp_summary(
        k=k_t,
        v=v_t,
        g=g_t,
        beta=beta_t,
    ...
    fs_ref = _reference_state(k, v, gate, beta, cu, n, zero)
    transition_ref = _reference_state(k, torch.zeros_like(v), gate, beta, cu, n, eye)
    cu_uncut = torch.tensor([0, total], dtype=torch.int32, device=device)
    composed_ref = _reference_state(k, v, gate, beta, cu_uncut, n, zero[:1])[0]
    ...
    assert r_h < 2e-2, f"final_state rms ratio {r_h:.4g}"
    assert r_m < 2e-2, f"transition rms ratio {r_m:.4g}"
    assert r_x < 5e-2, f"composed final state rms ratio {r_x:.4g}"
    print(f"[15] PASS  gdp summary                 spans={list(spans)} H={H} D={D} n={n} (fs rms {r_h:.2e}, transition rms {r_m:.2e}, composed rms {r_x:.2e})")
