...
/**
 * A mixed-precision residual block for the forward inference pass. This class acts as the basic building block for a Residaul Network such as a ResNet34. The residual forward block consists of a convolutional layer, a batch normalization layer, and a ReLu actuvation stacked together 3 times. There is also a residual connection between the input and the output of the block, which can optionally have a 1x1 convolution + batch normalization layer before the addition, or just a direct connection to the addition.
 */
class ResidualForwardInferenceBlock : public IBlock {
    public:
         ...
         */
        ResidualForwardInferenceBlock(cudnnHandle_t& handle, 
                             ResidualBlockParams const& params)
                             : IBlock(IBlock::FORWARD_INFERENCE), params_(params) {
            (void) handle;
            getLogger() << "[cudnn_frontend] " << "INFO: Creating ResidualForwardInferenceBlock" << std::endl;
                ...
                switch (node.type) {
                    ...
                    case CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR: {
                        ...
                        if(pointwise_descriptor->getPointWiseMode() == CUDNN_POINTWISE_IDENTITY || 
                           ...
                           pointwise_descriptor->getPointWiseMode() == CUDNN_POINTWISE_RSQRT) {
