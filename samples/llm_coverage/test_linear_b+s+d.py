...
def get_cudnn_matmul(x_dim, x_stride, w_dim, w_stride, dtype):
    ...
    Create a cudnn graph for matmul between a tensor "x" of shape (m, n, k) and a
    tensor "w" of shape (1, k, h) to produce a tensor "y" of shape (m, n, h).
    """
...
def test_linear_bfloat16():
    ...
    with pytest.raises(cudnn.cudnnGraphNotSupportedError):
        _ = compare_linear(torch.bfloat16)
