...
def get_cudnn_swiglu(x_dim, x_stride, w1_dim, w1_stride, w2_dim, w2_stride, dtype):
    """Compute Y = (X @ W1) . swish(X @ W2) where the two components are element-wise multiplied."""
    ...
    with cudnn.Graph(
        ...
        Y = graph.mul(name="mul", a=Z1, b=swish)
...
class CudnnSwigluFunction(Function):
    @staticmethod
    def forward(x, w1, w2):
        """Swiglu function: y = (x @ w1) . swish(x @ w2)
        ...
        print(f"CudnnSwiglu: x.shape: {x.shape}, w1.shape: {w1.shape}, w2.shape: {w2.shape}, x.dtype: {x.dtype}, w1.dtype: {w1.dtype}, w2.dtype: {w2.dtype}")
        X = x.view(1, b * s, m)
        W1 = w1.T.unsqueeze(0)
        W2 = w2.T.unsqueeze(0)
...
class CudnnSwiGLUModule(nn.Module):
    ...
    def forward(self, x):
        print(
            f"SwiGLU: x.shape: {x.shape}, linear1.shape: {self.linear1.shape}, linear2.shape: {self.linear2.shape}, x.dtype: {x.dtype}, linear1.dtype: {self.linear1.dtype}, linear2.dtype: {self.linear2.dtype}"
        )
        return CudnnSwigluFunction.apply(x, self.linear1, self.linear2)
...
def test_swiglu_bfloat16():
    """Test SwiGLU with bfloat16 data type for PyTest"""
    ...
    assert max_diff < 1e-2  # wider tolerance for bfloat16


def test_swiglu_float16():
    """Test SwiGLU with float16 data type for PyTest"""
...
def test_swiglu_float32():
    """Test SwiGLU with float32 data type for PyTest"""
