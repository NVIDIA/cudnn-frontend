    ...
    m.def("sdpa_fp8",
          ...
          R"pbdoc(
                ...
                Args:
                    ...
                    unfuse_fma (Optional[bool]): For SM100: use unfused __fmul_rn + __fadd_rn instead of ffma2 in softmax. Default is False.
                    implementation (Optional[cudnn.attention_implementation]): Which underlying implementation to use in the cuDNN backend. Default is AUTO (recommended).
                    cu_seq_len_q (Optional[cudnn_tensor]): Cumulative sequence length of the query, shape (b+1, 1, 1, 1) or 1-D (b+1,) (promoted automatically), int32 or int64. Mutually exclusive with seq_len_q (a side cannot use both forms); must be paired with a KV-side length tensor (seq_len_kv or cu_seq_len_kv) and requires use_padding_mask=True. The two sides may use different forms (e.g. cu_seq_len_q with seq_len_kv). Requires cuDNN 9.25.0 or newer and the UNIFIED implementation.
                    cu_seq_len_kv (Optional[cudnn_tensor]): Cumulative sequence length of the key, shape (b+1, 1, 1, 1) or 1-D (b+1,) (promoted automatically), int32 or int64. Mutually exclusive with seq_len_kv (a side cannot use both forms); must be paired with a Q-side length tensor (seq_len_q or cu_seq_len_q) and requires use_padding_mask=True. The two sides may use different forms (e.g. seq_len_q with cu_seq_len_kv). Requires cuDNN 9.25.0 or newer and the UNIFIED implementation.
                Preferred masking Args:
