...
def _grouped_gemm_srelu_impl(
    ...
    *,
    ...
    """Compute a dense-weight block-scaled grouped GEMM and squared-ReLU fusion.

    Native FP4 inputs cover MXFP4/NVFP4 without raw-byte reinterpretation.
    Low-precision outputs from FP8 inputs return row/column scale factors;
    FP16/BF16 outputs return an initialized per-expert amax reduction.
    """
    ...
    outputs = [
        ...
        make_buffer_desc("d_tensor", (1, m, n), d_dtype, tensor_spec=output_spec),
    ]
    if generate_sfd:
        outputs.append(
            make_buffer_desc(
                "d_col_tensor", (1, m, n), d_dtype, tensor_spec=output_spec
            )
        )
    if has_amax:
        outputs.append(
