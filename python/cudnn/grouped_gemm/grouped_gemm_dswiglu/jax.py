...
def _launch(
    ...
    *args,
    ...
    a, b, c, sfa, sfb, padded_offsets, alpha, beta, prob = (take() for _ in range(9))
    ...
    if arg_idx != len(args):
        raise RuntimeError(
            f"Unexpected grouped dSwiGLU argument count: consumed {arg_idx}, received {len(args)}"
        )
...
def _grouped_gemm_dswiglu_impl(
    ...
    *,
    ...
    """Compute the contiguous block-scaled grouped dSwiGLU fusion.
    ...
    Native FP4 inputs use logical JAX shapes; raw-byte reinterpretation is not
    part of this adapter. Configuration values are static under JAX tracing.
    """
    ...
    input_descs = [
        ...
        as_gemm_tensor_desc("beta_tensor", beta_tensor),
    ...
    outputs = [
        make_buffer_desc(
            "d_row_tensor", (1, m, output_n), d_dtype, mode=output_mode
        ),
        make_buffer_desc(
    ...
    result_idx = 3
    d_row_tensor, d_col_tensor, dprob_tensor = results[:3]
...
class GroupedGemmDswigluSm100(ApiBaseJax):
    ...
    def __init__(
        ...
        self._sample_descs = {
            ...
            "beta_tensor": self.make_optional_tensor_desc(
