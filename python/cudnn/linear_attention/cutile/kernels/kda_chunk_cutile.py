...
def autotuned_launch(kernel, cache_key, grid, args, occ_choices=(1, 2, 3, 4), nww_choices=(4, 8), timeout=30, stream=None):
    ...
    fixed). Falls back to the base kernel when DISABLE_TUNE is set or tuning
    fails / times out. Default config (occ=1, nww=4) is explored first so a
...
def cast(bufs, name, src, ref, stream=None):
    ...
    if str(src.dtype).split(".")[-1] == str(ref.dtype).split(".")[-1]:
        return src
    from .common import cast_copy, reshaped
...
@ct.kernel
def l2norm_bwd_kernel1(y, rstd, dy, dx, eps, D, BD: ConstInt):
    i_t = ct.bid(0)
    cols = ct.arange(BD, dtype=ct.int32)
    mask = cols < D
...
def l2norm_fwd_kernel(x, y, rstd, eps, T, D: ConstInt, BD: ConstInt, BT: ConstInt):
    ...
    # ct.load with block index + ZERO padding (boundary-checked block load).
    i_t = ct.bid(0)
...
# The (T, S) sub-view has stride (H*S, 1).
...
# ===========================================================================
# Token-parallel: one block per (token, head-group). BH heads at a time, each
# head contributes a scalar Aqk/Akk per partner token j. GATHER RATIONALE:
# per-token head-interleaved scalar reads/writes -> structured gather/scatter.
...
# ===========================================================================
# chunk_intra (diag compute+solve / intra sub-chunk)
# ===========================================================================
...
# ===========================================================================
# chunk_bwd (wy + dqkg fused) -- main backward kernel
# ===========================================================================
...
# ===========================================================================
# wy_fast (prepare_wy_repr_bwd) -- NOT on the reachable chunk_kda
# path (no launch site); kept for module parity.
# ===========================================================================
...
def prepare_wy_repr_bwd_kda_kernel(
    ...
    v_base = (bos * HV + i_hv) * V
    beta_base = bos * HV + i_hv
    gk_base = (bos * HV + i_hv) * K
    A_base = (bos * HV + i_hv) * BT
    dA_base = (bos * HV + i_hv) * BT
    dk_base = (bos * HV + i_hv) * K
    dk2_base = (bos * HV + i_hv) * K
    dw_base = (bos * HV + i_hv) * K
    du_base = (bos * HV + i_hv) * V
    dv_base = (bos * HV + i_hv) * V
    db_base = bos * HV + i_hv
    dg_base = (bos * HV + i_hv) * K
    dg2_base = (bos * HV + i_hv) * K
    ...
    b_b = ct.astype(ct.gather(beta, beta_base + o_t * HV, mask=m_t, check_bounds=False, padding_value=0.0), ct.float32)
    # b_A: view (BT, T) stride (1, HV*BT), block (BT, BT) at (0, i_t*BT)
    A_off = A_base + o_bt[:, None] * 1 + o_t[None, :] * (HV * BT)
    A_mask = (o_bt[:, None] < BT) & m_t[None, :]
    b_A = ct.gather(A, A_off, mask=A_mask, check_bounds=False, padding_value=0.0)
    ...
    for i_k in range(ct.cdiv(K, BK)):
        ...
        b_k = ct.gather(k, k_base + rows * (H * K) + kk[None, :], mask=m_tk, check_bounds=False, padding_value=0.0)
        b_gk_exp = exp2(
            ct.astype(
                ct.gather(gk, gk_base + rows * (HV * K) + kk[None, :], mask=m_tk, check_bounds=False, padding_value=0.0),
                ct.float32,
            )
        )
        b_dw = ct.gather(dw, dw_base + rows * (HV * K) + kk[None, :], mask=m_tk, check_bounds=False, padding_value=0.0)
        b_kbg = ct.astype(b_k, ct.float32) * b_b[:, None] * b_gk_exp

        b_dA = safe_mma(b_dw, ct.transpose(ct.astype(b_kbg, b_dw.dtype)), b_dA)
        b_dkbg = safe_matmul(b_A, b_dw)
        b_dk_prev = ct.astype(
            ct.gather(dk, dk_base + rows * (HV * K) + kk[None, :], mask=m_tk, check_bounds=False, padding_value=0.0),
            ct.float32,
        )
        b_dg_prev = ct.astype(
            ct.gather(dg, dg_base + rows * (HV * K) + kk[None, :], mask=m_tk, check_bounds=False, padding_value=0.0),
            ct.float32,
        )
        b_dk = ct.astype(b_dkbg, ct.float32) * b_gk_exp * b_b[:, None] + b_dk_prev
        b_dg = b_kbg * ct.astype(b_dkbg, ct.float32) + b_dg_prev
        b_db = b_db + ct.sum(ct.astype(b_dkbg, ct.float32) * ct.astype(b_k, ct.float32) * b_gk_exp, axis=1)
        ct.scatter(dk2, dk2_base + rows * (HV * K) + kk[None, :], ct.astype(b_dk, dk2.dtype), mask=m_tk, check_bounds=False)
        ct.scatter(dg2, dg2_base + rows * (HV * K) + kk[None, :], ct.astype(b_dg, dg2.dtype), mask=m_tk, check_bounds=False)
    ...
    for i_v in range(ct.cdiv(V, BV)):
        ...
        b_v = ct.gather(v, v_base + rows * (HV * V) + vv[None, :], mask=m_tv, check_bounds=False, padding_value=0.0)
        b_du = ct.gather(du, du_base + rows * (HV * V) + vv[None, :], mask=m_tv, check_bounds=False, padding_value=0.0)
        b_vb = ct.astype(ct.astype(b_v, ct.float32) * b_b[:, None], b_v.dtype)
        b_dA = safe_mma(b_du, ct.transpose(b_vb), b_dA)
        b_dvb = safe_matmul(b_A, b_du)
        b_dv = ct.astype(b_dvb, ct.float32) * b_b[:, None]
        b_db = b_db + ct.sum(ct.astype(b_dvb, ct.float32) * ct.astype(b_v, ct.float32), axis=1)
        ct.scatter(dv, dv_base + rows * (HV * V) + vv[None, :], ct.astype(b_dv, dv.dtype), mask=m_tv, check_bounds=False)
...
# ===========================================================================
# Host wrappers (fixed reasonable tile sizes chosen;
# None optionals -> dummy tensor + flag).
# ===========================================================================
...
def kda_gate_bwd(g, A_log, dt_bias=None, dyg=None, lower_bound=None, dg_out=None, dA_out=None, dbias_out=None, bufs=None, stream=None):
    ...
    if dt_bias is not None:
        sum_leading(reshaped(dbias_out, (H * K,)), db_nt, NT, H * K, stream=stream)
...
# ---------------------------------------------------------------------------
# chunk_gated_delta_rule_fwd_h / bwd_dhu (shared chunk_delta_h)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# chunk_kda_fwd_intra_token_parallel / chunk_kda_fwd_intra
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# chunk_kda_bwd_dAv / chunk_kda_bwd_wy_dqkg_fused
# ---------------------------------------------------------------------------
...
def chunk_kda_bwd_wy_dqkg_fused(
    ...
    dg = reshaped(bufs["dg"], (T, HV, K))
    db = reshaped(bufs["db"], (T, HV))
    dA = reshaped(bufs["dAkk"], (T, HV, BT))
...
# ===========================================================================
# chunk_fwd / chunk_bwd orchestration
# ===========================================================================
...
def chunk_kda_bwd(
    ...
    **kwargs,
    ...
    if HV > H:
        ...
        dk_r = reshaped(bufs["dk_hred"], (T_, H, K_))
...
def chunk_kda_grad(
    ...
    if use_qk_l2norm_in_kernel:
        dq = l2norm_bwd(q_in, q_rstd, dq, out=bufs["dq_l2"], stream=stream)
        dk = l2norm_bwd(k_in, k_rstd, dk, out=bufs["dk_l2"], stream=stream)
    if use_beta_sigmoid_in_kernel:
