...
GDN2_ALIGN = {"a_log": 4, "beta": 4, "cu_seqlens": 8}
GDN2_COMPACT = ("dt_bias",)
...
class Gdn2FrostEngine(BaseEngine):
    ...
    def check_support(self, graph) -> None:
        ...
        if ckpt and (facts.is_bwd or ckpt != 16):
            raise NotImplementedError(f"Gdn2FrostEngine: checkpoint_every_n_tokens must be 16 (per-chunk) on the forward node (got {ckpt})")
...
class CompiledGdn2:
    ...
    def __init__(self, node, kernel_mod):
        ...
        self.expect = expect_table(node, GDN2_ALIGN)
    ...
    def __call__(self, node_buffers, *, workspace=None, stream=None) -> Any:
        ...
        check_layouts(
            "Gdn2FrostEngine (GDN2)",
            expect=self.expect,
            compact=GDN2_COMPACT,
            q=q,
            k=k,
            v=v,
            g=g,
        ...
        ws = Workspace(workspace, self.ws_bytes, "Gdn2FrostEngine (GDN2)")
...
class CompiledGdn2Bwd:
    ...
    def __call__(self, node_buffers, *, workspace=None, stream=None) -> Any:
        ...
        B = cu.shape[0] - 1
        ws = Workspace(workspace, self.ws_bytes, "Gdn2FrostEngine (GDN2_BWD)")
