...
"""
GDN-2 prep for SM100 / SM103 / SM107 (Cutlass primitives): the per-(chunk, head) prep records of the BT = 16 schedule,
computed once ahead of the prep-fed prefill (gdn2_prep_prefill_f16); kda_prep_f16 is the KDA twin.

Per chunk c of head h (tokens [16c, 16c + 16) of its sequence), with the prefill's compute-mode arithmetic; Beta is the
per-channel erase gate, a [16, DK] tile like K:
  g[t, d]        = cumulative log2 gate per key channel (safe-gate / natural-log / linear, padded rows carry none)
  K decay[t, d]  = Beta[t, d] * K_n[t, d] * exp2(+g[t, d])       record k_decay   (the prefill's sK_decay image; the erase
                                                                  gate folded in, optionally through the guard of
                                                                  common/beta_guard.py)
  ...
  KK = K decay @ K inv^T;  L = tril(KK, -1);  inverse = (I + L)^-1 (the 4 -> 8 -> 16 blockwise inverse)
  T[j, k]      = sum_i inverse[i, j] * K restore[i, k]                record t    (the prefill's sK_restore image)
  A              = tril(Q decay @ K inv^T);  A[t, j] = sum_i A[t, i] * inverse[i, j]        record a (SW32 image, 512 B)
...
prefill's CG0 operand materialization (the four warps own the whole chunk), phase 2 = the record math split FlashInfer-style:
  warp 0 : KK / L / inverse, and the raw TMA loads (Q, K, Gate and the beta tile) of item j + SMEM_RAW_STAGES once item j's
...
from .gdn2_prep_config import CFG
...
def make_bars(cfg) -> Gdn2PrepBars:
    """Gdn2PrepBars constructor."""
    ONE_LANE = 1
    ...
    return Gdn2PrepBars(
        mb_q_ready=MBarrier(alloc(RS), try_wait=True, stages=RS, init_count=ONE_LANE, producer=Producer.TMA_LOAD),
        mb_k_ready=MBarrier(alloc(RS), try_wait=True, stages=RS, init_count=ONE_LANE, producer=Producer.TMA_LOAD),
        mb_gate_ready=MBarrier(alloc(RS), try_wait=True, stages=RS, init_count=ONE_LANE, producer=Producer.TMA_LOAD),
        mb_beta_ready=MBarrier(alloc(RS), try_wait=True, stages=RS, init_count=ONE_LANE, producer=Producer.TMA_LOAD),
    )
...
def compute_warp_group(
    ...
    """The CTA's four warps over its items: phase 1 materializes the chunk's operands (the prefill's CG0 body, the erase gate
    folded into K decay), phase 2 folds the records: warp 0 KK -> L -> inverse (it also issues the raw loads of item j +
    SMEM_RAW_STAGES, the beta tile among them), warp 2 A then A, warps 1 and 3 the two key halves of T (lane 0 of warp 1
    issues the record stores)."""
    RS = cfg.smem_raw_stages
        ...
        if warp_local == 0:
            if nvvm.elect_sync():
                for b in cutlass.range(batch_first, batch_last + cutlass.Int32(1), unroll=1):
                    ...
                    tma_tensormap_acquire((desc_beta_base + b * cutlass.Int32(TENSOR_MAP_QWORDS)).tospace(cutlass.AddressSpace.generic))
            for s0 in cutlass.range(cutlass.min(cutlass.Int32(RS), n_items), unroll=1):
                ...
                load_desc_beta = (desc_beta_base + load_batch).tospace(cutlass.AddressSpace.generic)
                if nvvm.elect_sync():
                    bars.mb_beta_ready[load_stage].arrive(n_bytes=cfg.tma_beta_bytes)
                tma_load_tile(
                    sBeta_tma[load_stage],
                    tma_slice_runtime_desc(load_desc_beta, zero, load_head, load_token),
                    bars.mb_beta_ready[load_stage].smem_ptr,
                    acquire=False,
                )
        if warp_local == 1:
            if lane_idx == 0:
    ...
    for j in cutlass.range(n_items):
        ...
        for dim_half in cutlass.range_constexpr(dk_halves):
            ...
            raw_beta_vec_f32 = (sBeta_ptr + raw_f16_idx).load(count=8, alignment=16).to(cutlass.Float32)
            for dim_offset in cutlass.range_constexpr(8):
        ...
        if warp_local == 0:
            ...
            # ---- L = tril(KK, -1) (the erase gate is already folded into K decay) -------------
            ...
            # ---- the inverse into the group's intermediate tile ------------------------------
            nvvm.stmatrix(
...
def build_descs_body(
    ...
    """Per-batch descriptor-array build, one warp per array: Q, K, Gate, k_decay, q_decay, t, beta."""
    ...
    desc_beta_arr = cute.make_tensor(desc_workspace.iterator + 6 * arr_words, cute.make_layout((arr_words,), stride=(1,)))
    if widx == 0:
...
def prep_base_maps(cfg, q, k, gate, beta, k_decay, q_decay, t):
    """Trace-time helper: the prep's base TensorMaps (Q, K, Gate, beta tile, k_decay, q_decay, t) whose per-batch arrays the
    prep-fed prefill prologue emits for it."""
    ...
    beta_headed = cute.make_tensor(beta.iterator, cute.make_layout((beta.shape[0], beta.shape[1], beta.shape[2]), stride=(beta.stride[0], beta.stride[1], 1)))
    base_desc_beta = tma.create_tensor_map_tiled_from_view(beta_headed, box_dims=(cfg.b_t, 1, box_elems), stride_order=(2, 1, 0), swizzle=swizzle_128b)
    record_maps = []
    ...
    return base_desc_q, base_desc_k, base_desc_gate, base_desc_beta, record_maps
...
def host(
    ...
    cfg.tma_beta_bytes = cfg.d_k * cfg.b_t * bytes_per_element
...
def frost_gdn2_prep(
    ...
    """BT = 16 GDN-2 prep (persistent item blocks, CTAS_PER_SM CTAs per SM)."""
    ...
    desc_beta_base = desc_q_base + cutlass.Int32(6) * arr_words
    ...
    sBeta_raw = cutlass.Array(cfg.io_dtype, RS * cfg.d_k * cfg.b_t, space=SMEM, alignment=cfg.buffer_align_bytes)
    if cutlass.const_expr(cfg.gate_dtype == cutlass.Float32):
...
@dataclass
class Gdn2PrepCfg:
    """Kernel cfg (fixed BT = 16 schedule constants; the raw ring depth is picked by key dim in ``build_cfg``, the derived SMEM
    sizes are stamped by ``host``).  Passed ``cfg``-first (a ``cutlass.Constexpr``) into ``host`` / ``kernel`` and every warp body."""
...
def build_cfg(
    ...
    *,
    ...
    """Build the per-compile ``Gdn2PrepCfg`` (io_dtype in {Float16, BFloat16}; gate fp32 or the io dtype); the raw ring depth
    is the config's constant for the key dim."""
    cfg = Gdn2PrepCfg(
        io_dtype=io_dtype,
        ...
        smem_raw_stages=CFG.SMEM_RAW_STAGES_D_K_128 if d_k == 128 else CFG.SMEM_RAW_STAGES_D_K_64,
    )
    cfg.threads_per_cta = cfg.threads_per_warp * cfg.compute_warps
    bytes_per_element = io_dtype.width // 8
    tile = d_k * cfg.b_t
    stage_bytes = 3 * tile * bytes_per_element + tile * (gate_dtype.width // 8)
...
TENSORMAP_DESC_ARRAYS = 7  # per-batch runtime TMA descriptors: Q, K, Gate, k_decay, q_decay, t, beta tile


frost_gdn2_prep.set_name_prefix("cudnn", remove_cutlass_symbol=False)
