...
"""BT=16 GDN-2 fused state summary: one persistent CTA per (piece, head) runs two recurrences in lockstep on the
same K / gate / beta stream.  H is the state from zero (or ``initial_state``) consuming W * V; M is seeded with the
identity and treats V as zero, giving the piece transition in the stored domain (``M_buf = M^T``,
``X_final = X_init @ M_buf + X_H``).  Beta is folded into the K decay operand, so both chains see it; w scales V for
the H chain only.  The state carry is fp32; every stage runs H first, M second.  Warp roles: CG0 (warps 0-7) stages
the shared operands and the left state halves; CG1 (warps 8-11) seeds, packs / rescales the right halves, stages
Y / U and stores H and M; warps 12 and 15 compute T^-1 (even / odd local chunks); warp 13 issues every tcgen05
GEMM; warp 14 loads K / V / beta / W / gate.  An item seeds when ``compute_start == 0`` and stores when
``write_end == batch_num_chunks``; an empty item passes ``initial_state`` (or zero) through as H and writes M = I.
"""
...
from .gdn2_summary_config import CFG
...
class ChainBars(NamedTuple):
    """The handoffs of one recurrence (H or M); each chain owns its state, input, accumulators and stagings."""
...
class Gdn2SummaryBars(NamedTuple):
    """Every inter-warp handoff as an ``MBarrier`` over its ring: the shared rings, then one ``ChainBars`` per recurrence."""
...
def make_bars(cfg) -> Gdn2SummaryBars:
    """Gdn2SummaryBars factory."""
    ...
    def chain():
        return ChainBars(
            ...
            mb_state_acc_done=MBarrier(alloc(cfg.smem_decay_stages), stages=cfg.smem_decay_stages, init_count=1, producer=Producer.MMA_COMMIT),
        )

    return Gdn2SummaryBars(
...
def super_mma_warp(
    ...
    """Chunk-factor warp role (warps 12 and 15): the register-MMA T_inv, shared by both chains."""
    ...
    while tile_idx < total_tiles:
        ...
        for local_chunk in cutlass.range(chunk_parity, num_chunks_tile, 2, unroll=1):
            ...
            sIntermediate_ptr = sIntermediate_raw.data_ptr() + intermediate_stage * (cfg.b_t * cfg.b_t)

            bars.mb_k_decay_inv_cg0_ready[decay_stage].wait(cutlass.Int32((chunk_count // cfg.smem_decay_stages) % 2))
            ...
            bars.mb_t_inv_done[intermediate_stage].wait(cutlass.Int32(((chunk_count // cfg.smem_intermediate_stages) + 1) % 2))
            nvvm.stmatrix(
...
def tcgen05_mma_warp(
    ...
    """tcgen05-MMA warp role (warp 13): issues every tcgen05 GEMM of both chains (H first, M second at every
    stage) and owns the TMEM lifecycle."""
    elect_one = nvvm.elect_sync()
    nvvm.setmaxregister(cfg.num_regs_other, nvvm.SetMaxRegisterAction.DECREASE)
    ...
    state_input_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.state_input, cutlass.Int8)
    state_k_acc_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.state_k_acc, cutlass.Float32)
    u_acc_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.u_acc, cutlass.Float32)
    y_input_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.y_input, cutlass.Int8)
    u_input_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.u_input, cutlass.Int8)
    state_dst_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.state_acc, cutlass.Float32)
    state_dst_r_ptr_h = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_h.state_acc + state_update_n, cutlass.Float32)
    state_input_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.state_input, cutlass.Int8)
    state_k_acc_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.state_k_acc, cutlass.Float32)
    u_acc_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.u_acc, cutlass.Float32)
    y_input_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.y_input, cutlass.Int8)
    u_input_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.u_input, cutlass.Int8)
    state_dst_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.state_acc, cutlass.Float32)
    state_dst_r_ptr_m = nvvm.make_tmem_ptr(tmem_base + cfg.tmem_m.state_acc + state_update_n, cutlass.Float32)
    state_input_index_h = PipelineState.start(phase=0)
    state_input_l_index_h = PipelineState.start(phase=0)
    state_input_index_m = PipelineState.start(phase=0)
    state_input_l_index_m = PipelineState.start(phase=0)
    ...
    STATE_K_STEPS_L = bmm_state_k_decay_desc_h.num_k_steps // 2
    global_chunk_base = cutlass.Int32(0)
    scheduler_state = PipelineState.start(phase=0)
    tile_idx = cutlass.Int32(bidx)
    while tile_idx < total_tiles:
        ...
        for local_chunk in cutlass.range(num_chunks_tile, unroll=1):
            ...
            if cutlass.const_expr(cfg.use_initial_state):
                have_state_h = local_chunk > 0 or seed_state
            else:
                have_state_h = local_chunk > 0
            have_state_m = local_chunk > 0 or seed_state
            chunk_count = cutlass.Uint32(global_chunk)
            decay_stage = cutlass.Int32(chunk_count % cfg.smem_decay_stages)
            decay_parity = cutlass.Int32((chunk_count // cfg.smem_decay_stages) % 2)
            intermediate_stage = cutlass.Int32(chunk_count % cfg.smem_intermediate_stages)
            intermediate_parity = cutlass.Int32((chunk_count // cfg.smem_intermediate_stages) % 2)
            ...
            while not nvvm.mbarrier_wait_parity(bars.mb_k_decay_inv_cg0_ready[decay_stage].smem_ptr, decay_parity, nvvm.MBarrierWait.TRY):
                pass
            if have_state_h:
                while not nvvm.mbarrier_wait_parity(bars.chain_h.mb_state_input_ready_l.smem_ptr, state_input_l_index_h.phase, nvvm.MBarrierWait.TRY):
                    pass
                state_input_l_index_h = advance(state_input_l_index_h, 1)
                ...
                for f in cutlass.range_constexpr(bmm_state_k_decay_desc_h.num_k_steps):
                    if cutlass.const_expr(f == STATE_K_STEPS_L):
                        while not nvvm.mbarrier_wait_parity(bars.chain_h.mb_state_input_ready.smem_ptr, state_input_index_h.phase, nvvm.MBarrierWait.TRY):
                            pass
                        state_input_index_h = advance(state_input_index_h, 1)
                        nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
                ...
                if elect_one:
                    bars.chain_h.mb_state_k_acc_ready.arrive(cta_group=1)
            if have_state_m:
                while not nvvm.mbarrier_wait_parity(bars.chain_m.mb_state_input_ready_l.smem_ptr, state_input_l_index_m.phase, nvvm.MBarrierWait.TRY):
                    pass
                state_input_l_index_m = advance(state_input_l_index_m, 1)
                nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)

                for f in cutlass.range_constexpr(bmm_state_k_decay_desc_m.num_k_steps):
                    if cutlass.const_expr(f == STATE_K_STEPS_L):
                        while not nvvm.mbarrier_wait_parity(bars.chain_m.mb_state_input_ready.smem_ptr, state_input_index_m.phase, nvvm.MBarrierWait.TRY):
                            pass
                        state_input_index_m = advance(state_input_index_m, 1)
                        nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
                ...
                if elect_one:
                    bars.chain_m.mb_state_k_acc_ready.arrive(cta_group=1)
            ...
            # ---- u acc H = Y H(T) @ (T^-1)^T, u acc M = Y M(T) @ (T^-1)^T ------------
            while not nvvm.mbarrier_wait_parity(bars.mb_t_inv_ready[intermediate_stage].smem_ptr, intermediate_parity, nvvm.MBarrierWait.TRY):
                pass
            while not nvvm.mbarrier_wait_parity(bars.chain_h.mb_y_input_ready.smem_ptr, y_input_index.phase, nvvm.MBarrierWait.TRY):
                pass
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
            mma_ts_step(bmm_y_t_inv_desc_h, y_input_ptr_h, desc_t_inv, u_acc_ptr_h, 0, cutlass.Boolean(False), issue_mma=elect_one)
            if elect_one:
                bars.chain_h.mb_u_acc_ready.arrive(cta_group=1)
            while not nvvm.mbarrier_wait_parity(bars.chain_m.mb_y_input_ready.smem_ptr, y_input_index.phase, nvvm.MBarrierWait.TRY):
                pass
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
            mma_ts_step(bmm_y_t_inv_desc_m, y_input_ptr_m, desc_t_inv, u_acc_ptr_m, 0, cutlass.Boolean(False), issue_mma=elect_one)
            if elect_one:
                bars.chain_m.mb_u_acc_ready.arrive(cta_group=1)
            ...
            while not nvvm.mbarrier_wait_parity(bars.chain_h.mb_u_input_ready.smem_ptr, u_input_index.phase, nvvm.MBarrierWait.TRY):
                pass
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
            mma_ts_step(bmm_u_k_restore_desc_h, u_input_ptr_h, desc_k_restore, state_dst_ptr_h, 0, have_state_h, issue_mma=elect_one)
            if elect_one:
                bars.chain_h.mb_state_acc_done_l[decay_stage].arrive(cta_group=1)
            if cutlass.const_expr(state_update_split):
                mma_ts_step(bmm_u_k_restore_desc_h, u_input_ptr_h, desc_k_restore_right, state_dst_r_ptr_h, 0, have_state_h, issue_mma=elect_one)
            if elect_one:
                bars.chain_h.mb_state_acc_done[decay_stage].arrive(cta_group=1)
            while not nvvm.mbarrier_wait_parity(bars.chain_m.mb_u_input_ready.smem_ptr, u_input_index.phase, nvvm.MBarrierWait.TRY):
                pass
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
            mma_ts_step(bmm_u_k_restore_desc_m, u_input_ptr_m, desc_k_restore, state_dst_ptr_m, 0, have_state_m, issue_mma=elect_one)
            if elect_one:
                bars.chain_m.mb_state_acc_done_l[decay_stage].arrive(cta_group=1)
            if cutlass.const_expr(state_update_split):
                mma_ts_step(bmm_u_k_restore_desc_m, u_input_ptr_m, desc_k_restore_right, state_dst_r_ptr_m, 0, have_state_m, issue_mma=elect_one)
            if elect_one:
                bars.mb_k_restore_acc_done[decay_stage].arrive(cta_group=1)
                bars.chain_m.mb_state_acc_done[decay_stage].arrive(cta_group=1)
...
def tmaldg_warp(
    ...
    """TMA-LDG warp role (warp 14): the per-chunk K / V / beta / W / gate G->S loads."""
    nvvm.setmaxregister(cfg.num_regs_other, nvvm.SetMaxRegisterAction.DECREASE)
...
@cute.jit
def cg0_state_stage_left(cfg, chain, global_chunk, chunk_count, local_chunk, row_lo_addr, state_col_id, packed_col_id, sGate_exchange_ptr) -> None:
    """One chain's left key half of the state stage: wait for the chain's left delta GEMM of the previous chunk, pack
    the fp32 state to b16 into its state input and publish it, then rescale the fp32 state in place by exp2(g last)
    (row b_t-1 of the exchange tile)."""
    dk_halves = cutlass.const_expr(cfg.d_k // 64)
    if global_chunk > 0:
        update_count = chunk_count - cutlass.Uint32(1)
        chain.mb_state_acc_done_l[cutlass.Int32(update_count % cfg.smem_decay_stages)].wait(cutlass.Int32((update_count // cfg.smem_decay_stages) % 2))
        nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
    if local_chunk > 0:
        l_state_vecs = []
        ...
        if nvvm.elect_sync():
            chain.mb_state_input_ready_l.arrive()
...
def compute0_warp_group(
    ...
    """CG0 warp-group role (warps 0-7): the gate prefix scan, the decay / restore operands and the left key half of
    both states."""
    nvvm.setmaxregister(
    ...
    state_col_id_h = tmem_col + cfg.tmem_h.state_acc
    packed_col_id_h = tmem_col + cfg.tmem_h.state_input
    state_col_id_m = tmem_col + cfg.tmem_m.state_acc
    packed_col_id_m = tmem_col + cfg.tmem_m.state_input
    ...
    while tile_idx < total_tiles:
        ...
        for local_chunk in cutlass.range(cg0_group_id, num_chunks_tile, cfg.cg0_group_count, unroll=1):
            ...
            # ---- state stage, left key half of both chains: pack, publish, fp32 decay ----
            cg0_state_stage_left(cfg, bars.chain_h, global_chunk, chunk_count, local_chunk, row_lo_addr, state_col_id_h, packed_col_id_h, sGate_exchange_ptr)
            cg0_state_stage_left(cfg, bars.chain_m, global_chunk, chunk_count, local_chunk, row_lo_addr, state_col_id_m, packed_col_id_m, sGate_exchange_ptr)
            if nvvm.elect_sync():
                bars.mb_gate_done[raw_stage].arrive()
                bars.chain_h.mb_u_input_ready.arrive()
                bars.chain_m.mb_u_input_ready.arrive()
        global_chunk_base += num_chunks_tile
        tile_idx, scheduler_state = scheduler_next_tile(cfg, bars, sScheduler, scheduler_state, tile_idx, num_ctas, elect_one)


@cute.jit
def cg1_seed_chain(cfg, chain, mState_init, batch_idx, head_o, state_gmem_row, row_lo_addr, state_col_id, packed_col_id, seed_exchange_ptr) -> None:
    """Seed one chain at chunk 0 of an item, one key half at a time: pack the seed row to b16 into the chain's state
    input (the left half published on ``mb_state_input_ready_l``, the right on ``mb_state_input_ready``) and write it
    rescaled by exp2(g last) as the fp32 state; the seed is the ``initial_state`` row or the identity row when ``mState_init`` is None."""
    state_blocks_l = cutlass.const_expr(cfg.d_k // 32)
    if cutlass.const_expr(mState_init is None):
        identity_zero = opaque_f32_zero()
        identity_one = identity_zero + cutlass.Float32(1.0)
        identity_pack_lo = fp32_to_fp16(identity_one, identity_zero, dtype=cfg.io_dtype)
        identity_pack_hi = fp32_to_fp16(identity_zero, identity_one, dtype=cfg.io_dtype)
        identity_pack_zero = fp32_to_fp16(identity_zero, identity_zero, dtype=cfg.io_dtype)
    else:
    ...
    for seed_half in cutlass.range_constexpr(2):
        seed_blocks_lo = seed_half * state_blocks_l
        seed_blocks_hi = seed_blocks_lo + state_blocks_l
        ...
        for i in cutlass.range_constexpr(seed_blocks_lo, seed_blocks_hi):
            ...
            for packed_col in cutlass.range_constexpr(8):
                if cutlass.const_expr(mState_init is None):
                    identity_col = i * 16 + 2 * packed_col
                    state_pack[packed_col] = (
                        identity_pack_lo if state_gmem_row == identity_col else (identity_pack_hi if state_gmem_row == identity_col + 1 else identity_pack_zero)
                    )
                else:
                    state_pack[packed_col] = fp32_to_fp16(
                        state_vecs[i - seed_blocks_lo][2 * packed_col], state_vecs[i - seed_blocks_lo][2 * packed_col + 1], dtype=cfg.io_dtype
                    )
            nvvm.tcgen05_st(
                "32x32b",
        ...
        if cutlass.const_expr(seed_half == 0):
            ...
            if nvvm.elect_sync():
                chain.mb_state_input_ready.arrive()

        # ---- fp32 decay of the seed half: state = seed * exp2(g last) -------------
        for i in cutlass.range_constexpr(seed_blocks_lo, seed_blocks_hi):
            init_scaled = []
            for scale_group in cutlass.range_constexpr(4):
                init_scale_dim = i * 16 + scale_group * 4
                init_scale_segment = init_scale_dim // 32
                init_scale_idx = (
                    init_scale_segment * (cfg.b_t * 32)
                    + (cfg.b_t - 1) * 32
                    + swizzle_xor_128b_seg(cfg.b_t - 1, init_scale_segment, init_scale_dim - init_scale_segment * 32, elem_bytes=4)
                )
                init_scale_frag = (seed_exchange_ptr + init_scale_idx).load(count=4, alignment=16)
                for t in cutlass.range_constexpr(2):
                    if cutlass.const_expr(mState_init is None):
                        init_s0 = init_scale_frag[2 * t] if state_gmem_row == i * 16 + scale_group * 4 + 2 * t else cutlass.Float32(0.0)
                        init_s1 = init_scale_frag[2 * t + 1] if state_gmem_row == i * 16 + scale_group * 4 + 2 * t + 1 else cutlass.Float32(0.0)
                    else:
                        init_s0, init_s1 = fmul2(
                            state_vecs[i - seed_blocks_lo][scale_group * 4 + 2 * t],
                            state_vecs[i - seed_blocks_lo][scale_group * 4 + 2 * t + 1],
                            init_scale_frag[2 * t],
                            init_scale_frag[2 * t + 1],
                        )
                    init_scaled += [init_s0, init_s1]
            nvvm.tcgen05_st(
                "32x32b",
                nvvm.make_tmem_ptr(row_lo_addr + state_col_id + i * 16, cutlass.Float32),
                cutlass.Vector.from_elements(tuple(init_scaled), cutlass.Float32),
            )
    nvvm.tcgen05_wait("store")
    nvvm.tcgen05_fence(nvvm.Tcgen05Fence.BEFORE_THREAD_SYNC)
...
@cute.jit
def cg1_state_stage_right(
    cfg,
    chain,
    wait_exchange: cutlass.Constexpr[bool],
    raw_stage,
    raw_phase,
    k_restore_index,
    row_lo_addr,
    ...
    """One chain's right key half of the state stage: wait for the chain's delta GEMM of the previous chunk, pack the
    fp32 state to b16 into its state input, rescale by exp2(g last) (row b_t-1 of the exchange tile; the first chain of
    a chunk waits for the tile), publish the input and write the rescaled fp32 state back."""
    state_blocks_l = cutlass.const_expr(cfg.d_k // 32)
    chain.mb_state_acc_done[k_restore_index.idx].wait(k_restore_index.phase)
    nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)
    state_vecs = []
    ...
    if cutlass.const_expr(wait_exchange):
...
def cg1_y_stage(
    cfg,
    chain,
    v_is_zero: cutlass.Constexpr[bool],
    two_halves: cutlass.Constexpr[bool],
    row_lo_addr,
    row_hi_addr,
    projection_col_id,
    input_col_id,
    ...
    """Y = W * V - k state of one chain into its packed Y input (the M chain takes the zero word for W * V; without a
    state the k-state term is zero).  ``two_halves``: the chain has 128 rows, so lanes 16-31 of every warp hold rows too."""
    ...
    if have_state:
        chain.mb_state_k_acc_ready.wait(state_k_acc_index.phase)
        ...
        if cutlass.const_expr(two_halves):
    ...
    if cutlass.const_expr(v_is_zero):
    ...
    for reg_idx in cutlass.range_constexpr(4):
        if cutlass.const_expr(v_is_zero):
            y_input_pack_lo[reg_idx] = sub_f16x2(zero_word, state_k_pack_lo[reg_idx], cfg.io_dtype)
            y_input_pack_hi[reg_idx] = sub_f16x2(zero_word, state_k_pack_hi[reg_idx], cfg.io_dtype)
        else:
    ...
    nvvm.tcgen05_fence(nvvm.Tcgen05Fence.BEFORE_THREAD_SYNC)
    if nvvm.elect_sync():
        chain.mb_y_input_ready.arrive()
    return state_k_acc_index


@cute.jit
def cg1_u_stage(cfg, chain, u_acc_addr, u_input_addr, u_acc_phase) -> None:
    """One chain's U accumulator TMEM -> packed b16 U input TMEM."""
    chain.mb_u_acc_ready.wait(u_acc_phase)
    ...
    nvvm.tcgen05_fence(nvvm.Tcgen05Fence.BEFORE_THREAD_SYNC)
    if nvvm.elect_sync():
        chain.mb_u_input_ready.arrive()


@cute.jit
def cg1_store_state(cfg, mOut, batch_idx, head_o, state_gmem_row, row_valid, row_lo_addr, state_col_id) -> None:
    """One chain's final state row of this value dim: state TMEM -> GMEM (``row_valid``: the lane holds a row)."""
    state_vw = 16 // (mOut.element_type.width // 8)
    state_dst = (mOut.iterator + mOut.layout((batch_idx, head_o, state_gmem_row, 0))).raw_ptr()
    for key_block_start in cutlass.range_constexpr(0, cfg.d_k, 32):
        state_vec = nvvm.tcgen05_ld(
            "32x32b",
            nvvm.make_tmem_ptr(row_lo_addr + state_col_id + key_block_start, cutlass.Float32),
            num=32,
        )
            ...
            if row_valid:
                (state_dst + key_block_start + g * state_vw).store(
                    cutlass.Vector.from_elements(
                        tuple(state_vec[g * state_vw + t].to(mOut.element_type) for t in range(state_vw)),
                        mOut.element_type,
                    ),
                    alignment=16,
                )
...
def compute1_warp_group(
    ...
    """CG1 warp-group role (warps 8-11): seeds both states, packs and rescales their right key halves, stages the
    Y / U inputs of both chains (H first, M second at every stage) and stores H and M."""
    nvvm.setmaxregister(
    ...
    # ---- ldmatrix.x4 COL lane decode for the V/W loads -------------------------------
    ...
    state_gmem_row = cg1_warp * row_lanes_h + lane_idx % row_lanes_h
    row_valid_h = lane_idx < cutlass.Int32(row_lanes_h)
    key_row = cg1_warp * row_lanes_m + lane_idx % row_lanes_m
    row_valid_m = lane_idx < cutlass.Int32(row_lanes_m)
    state_col_id_h = tmem_col + cfg.tmem_h.state_acc
    packed_col_id_h = tmem_col + cfg.tmem_h.state_input
    projection_col_id_h = tmem_col + cfg.tmem_h.state_k_acc
    input_col_id_h = tmem_col + cfg.tmem_h.y_input
    u_acc_addr_h = row_lo_addr + tmem_col + cfg.tmem_h.u_acc
    u_input_addr_h = row_lo_addr + tmem_col + cfg.tmem_h.u_input
    state_col_id_m = tmem_col + cfg.tmem_m.state_acc
    packed_col_id_m = tmem_col + cfg.tmem_m.state_input
    projection_col_id_m = tmem_col + cfg.tmem_m.state_k_acc
    input_col_id_m = tmem_col + cfg.tmem_m.y_input
    u_acc_addr_m = row_lo_addr + tmem_col + cfg.tmem_m.u_acc
    u_input_addr_m = row_lo_addr + tmem_col + cfg.tmem_m.u_input
    v_swz_off_lo = (
    ...
    k_restore_index = PipelineState.start(phase=0)
    ...
    while tile_idx < total_tiles:
        ...
        if num_chunks_tile > 0:
            ...
            # ---- first chunk: seed both states (H from mState init, M with the identity) --
            if seed_state:
                ...
                if cutlass.const_expr(mState_init is not None):
                    cg1_seed_chain(
                        cfg, bars.chain_h, mState_init, batch_idx, head_o, state_gmem_row, row_lo_addr, state_col_id_h, packed_col_id_h, seed_exchange_ptr
                    )
                cg1_seed_chain(cfg, bars.chain_m, None, batch_idx, head_o, key_row, row_lo_addr, state_col_id_m, packed_col_id_m, seed_exchange_ptr)

            # ---- Y stage: Y H = W*V - state H*(Beta*K), Y M = 0 - state M*(Beta*K) ----
            ...
            raw_w_frag_lo = nvvm.ldmatrix(sW_ptr + v_swz_off_lo, 4, nvvm.MMALayout.COL)
            raw_w_frag_hi = raw_w_frag_lo
            if cutlass.const_expr(cfg.d_v == 128):
                raw_w_frag_hi = nvvm.ldmatrix(sW_ptr + v_swz_off_hi, 4, nvvm.MMALayout.COL)
            if cutlass.const_expr(mState_init is not None):
                have_state_h = seed_state
            else:
                have_state_h = cutlass.Boolean(False)
            state_k_acc_index_h = cg1_y_stage(
                cfg,
                bars.chain_h,
                False,
                cfg.d_v == 128,
            ...
            state_k_acc_index_m = cg1_y_stage(
                cfg,
                bars.chain_m,
                True,
                cfg.d_k == 128,
            ...
            # ---- U stage: acc TMEM -> packed b16 TMEM, both chains -------------------
            cg1_u_stage(cfg, bars.chain_h, u_acc_addr_h, u_input_addr_h, u_acc_index.phase)
            cg1_u_stage(cfg, bars.chain_m, u_acc_addr_m, u_input_addr_m, u_acc_index.phase)
        ...
        for local_chunk in cutlass.range(1, num_chunks_tile, 1, unroll=1):
            ...
            # ---- state stage, right key half of both chains: pack, fp32 decay, publish ---
            sGate_exchange_ptr = sGate_exchange_raw.data_ptr() + (global_chunk % cfg.gate_exchange_stages) * (cfg.d_k * cfg.b_t)
            cg1_state_stage_right(
                cfg, bars.chain_h, True, raw_stage, raw_phase, k_restore_index, row_lo_addr, state_col_id_h, packed_col_id_h, sGate_exchange_ptr, bars
            )
            cg1_state_stage_right(
                cfg, bars.chain_m, False, raw_stage, raw_phase, k_restore_index, row_lo_addr, state_col_id_m, packed_col_id_m, sGate_exchange_ptr, bars
            )
        ...
        if num_chunks_tile > 0:
            bars.chain_h.mb_state_acc_done[k_restore_index.idx].wait(k_restore_index.phase)
            bars.chain_m.mb_state_acc_done[k_restore_index.idx].wait(k_restore_index.phase)
            k_restore_index = advance(k_restore_index, cfg.smem_decay_stages)
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.AFTER_THREAD_SYNC)

        owns_final = write_end == batch_num_chunks

        # ---- final H and M stores: TMEM -> GMEM; empty items pass the seeds through --
        if batch_seqlen > 0:
            if owns_final:
                cg1_store_state(cfg, mState_out, batch_idx, head_o, state_gmem_row, row_valid_h, row_lo_addr, state_col_id_h)
                cg1_store_state(cfg, mTransition, batch_idx, head_o, key_row, row_valid_m, row_lo_addr, state_col_id_m)
        else:
            h_vw = 16 // (mState_out.element_type.width // 8)
            h_dst = (mState_out.iterator + mState_out.layout((batch_idx, head_o, state_gmem_row, 0))).raw_ptr()
            if cutlass.const_expr(mState_init is not None):
                seed_vw = 16 // (mState_init.element_type.width // 8)
                seed_src = (mState_init.iterator + mState_init.layout((batch_idx, head_o, state_gmem_row, 0))).raw_ptr()
            ...
            m_dst = (mTransition.iterator + mTransition.layout((batch_idx, head_o, key_row, 0))).raw_ptr()
            ...
            if row_valid_m:
                mTransition[batch_idx, head_o, key_row, key_row] = cutlass.Float32(1.0).to(mTransition.element_type)
        global_chunk_base += num_chunks_tile
        tile_idx, scheduler_state = scheduler_next_tile(cfg, bars, sScheduler, scheduler_state, tile_idx, num_ctas, elect_one)
...
def build_descs_body(
    ...
    desc_words_beta = cute.make_tensor(desc_workspace.iterator + 3 * arr_words, cute.make_layout((arr_words,), stride=(1,)))
    desc_words_w = cute.make_tensor(desc_workspace.iterator + 4 * arr_words, cute.make_layout((arr_words,), stride=(1,)))
    ...
    if widx == 2:
        ...
        nvvm.fence_proxy_release(nvvm.MemScope.GPU, from_proxy=nvvm.Proxy.GENERIC, to_proxy=nvvm.Proxy.TENSORMAP)
    if widx == 3:
        emit_seq_descs(base_beta, desc_words_beta, cu_seqlens, beta, n_batch, 2, lanes=32)
        nvvm.fence_proxy_release(nvvm.MemScope.GPU, from_proxy=nvvm.Proxy.GENERIC, to_proxy=nvvm.Proxy.TENSORMAP)
    if widx == 4:
        emit_seq_descs(base_w, desc_words_w, cu_seqlens, w, n_batch, 2, lanes=32)
        nvvm.fence_proxy_release(nvvm.MemScope.GPU, from_proxy=nvvm.Proxy.GENERIC, to_proxy=nvvm.Proxy.TENSORMAP)
...
def frost_gdn2_summary_prologue(
    ...
    table and zero the scheduler rings via :func:`order_body`, then build the per-batch TMA-descriptor arrays via
...
def frost_gdn2_summary(
    ...
    """BT=16 GDN-2 fused H + M summary persistent kernel body: every warp role runs a tile-scheduler loop."""
    if cutlass.const_expr(USE_PDL):
        wait_on_dependent_grids()
    ...
    # per-batch TMA-descriptor arrays (heads are load coordinates): K, V, Gate, Beta, W
    ...
    desc_beta_base = desc_base_words + cutlass.Int32(3) * arr_words
    ...
    sIntermediate = SmemTile(
        ...
        elems_per_stage=(cfg.b_t * cfg.b_t),
    ...
    if warp_idx == cfg.tma_warp_id:
        ...
        if elect_one:
            for chain in (bars.chain_h, bars.chain_m):
                chain.mb_state_k_acc_ready.init()
                chain.mb_u_acc_ready.init()
                chain.mb_state_input_ready.init()
                chain.mb_state_input_ready_l.init()
                chain.mb_y_input_ready.init()
                chain.mb_u_input_ready.init()
                for stage in cutlass.range_constexpr(cfg.smem_decay_stages):
                    chain.mb_state_acc_done_l[stage].init()
                    chain.mb_state_acc_done[stage].init()
            for stage in cutlass.range_constexpr(cfg.smem_decay_stages):
...
class TmemColumns(NamedTuple):
    """TMEM column offsets of one chain; the fp32 state doubles as its final state accumulator."""
...
def chain_tmem_columns(cfg, base: int) -> TmemColumns:
    state_acc = base
    state_input = state_acc + cfg.d_k
    state_k_acc = state_input + (cfg.d_k // 2)
    u_acc = state_k_acc + cfg.b_t
    y_input = u_acc + cfg.b_t
    u_input = y_input + (cfg.b_t // 2)
    return TmemColumns(state_acc, state_input, state_k_acc, u_acc, y_input, u_input)


@dataclass
class Gdn2SummaryCfg:
    """Kernel cfg: fixed BT=16 schedule constants; ``build_cfg`` stamps the derived TMEM column offsets and SMEM
    buffer cosizes.  Passed ``cfg``-first as a ``cutlass.Constexpr`` into every warp body."""
    ...
    tmem_h: TmemColumns = TmemColumns(0, 0, 0, 0, 0, 0)
    tmem_m: TmemColumns = TmemColumns(0, 0, 0, 0, 0, 0)
...
def build_cfg(
    ...
    *,
    ...
    """Build the per-compile ``Gdn2SummaryCfg`` (io_dtype in {Float16, BFloat16}); fills the derived TMEM column offsets and SMEM cosizes."""
    cfg = Gdn2SummaryCfg(
        io_dtype=io_dtype,
    ...
    if cfg.d_k not in STATE_DIMS or cfg.d_v not in STATE_DIMS:
        raise ValueError(f"the fused GDN-2 summary serves DK, DV in {STATE_DIMS}, got DK={cfg.d_k} DV={cfg.d_v}")
    ...
    cfg.tmem_h = chain_tmem_columns(cfg, 0)
    cfg.tmem_m = chain_tmem_columns(cfg, cfg.tmem_h.u_input + (cfg.b_t // 2))
    assert (cfg.tmem_m.u_input + (cfg.b_t // 2)) <= 512
...
TENSORMAP_DESC_ARRAYS = 5  # per-batch runtime TMA descriptors: K, V, Gate, Beta, W
...
def compile(
    ...
    *,
    ...
    """JIT-compile the fused GDN-2 summary kernel for one static config."""
    cfg = build_cfg(
        io_dtype,
        gate_dtype,
        use_initial_state=use_initial_state,
...
def chunk_gdn2_summary_sm100(
    ...
    *,
    ...
    """Blackwell BT=16 fused GDN-2 state summary: one launch writes ``output_state`` (H) and ``output_transition`` (stored domain
    ``M_buf = M^T``, ``X_final = X_init @ M_buf + X_H``) of every work item; an empty sequence passes ``initial_state`` (or zero) through
    ...
    if tuple(beta.shape) != (k.shape[0], HO, DK) or beta.dtype != k.dtype:
        raise ValueError(f"beta must be (total_tokens, HO, DK) in the io dtype, got {tuple(beta.shape)} {beta.dtype}")
    if tuple(w.shape) != (v.shape[0], HO, DV) or w.dtype != v.dtype:
        raise ValueError(f"w must be (total_tokens, HO, DV) in the io dtype, got {tuple(w.shape)} {w.dtype}")
    ...
    if "compiled" not in cache:
        ...
        beta_cute = from_dlpack(beta, assumed_align=16).mark_layout_dynamic(leading_dim=2)
...
frost_gdn2_summary_prologue.set_name_prefix("cudnn", remove_cutlass_symbol=False)
frost_gdn2_summary.set_name_prefix("cudnn", remove_cutlass_symbol=False)
