...
"""
Chunked Gated Delta Net 2 (GDN-2) fused state-summary kernel for SM100 / SM103 / SM107 (Cutlass primitives): the BT = 16
recompute pipeline run twice per chunk on one K / Gate / Beta stream, chain H (the state from zero or initial_state,
consuming W .* V) and chain M (the identity seed with V = 0, the piece transition), one persistent CTA per (piece,
head).

Algorithm overview (per chunk c, tokens [cC, (c+1)C), for each chain X in {H, M}):
  Inputs : K[BT,DK], V[BT,DV] and W[BT,DV] (chain H only), Gate[BT,DK] (per-channel gate), Beta[BT,DK] (erase gate)
  ...
  KK (register MMA) : W_kk[BT,BT] = K decay @ K inv^T;  L = tril(W_kk, -1)   (shared by both chains)
  ...
  U GEMM         : U_X[BT,DV]  = T_inv @ Y_X,  Y_H = W .* V - KS_H, Y_M = 0 - KS_M
  KV update GEMM : S_upd_X[DK,DV] = K restore^T @ U_X   (H first, M second, every stage)
...
SMEM layout (stage counts live in gdn2_summary_config.py; sizes at DK = DV = 128, bf16 io, fp32 Gate):
  Buffer                       Size (B)  Stages
  K / V / Beta / W (raw)       4 x 4096       5
  Gate (raw)                       8192       5    <-- bf16 Gate: 4096 plus a 4-stage fp32 exchange ring
  K decay / K inv / K restore  3 x 4096       2
  T_inv (intermediate)              512       2
...
class Gdn2SummaryBars(NamedTuple):
    """Every inter-warp handoff as an ``MBarrier`` over its ring: the shared rings, then each recurrence's own handoffs,
    chain H then chain M."""
...
def make_bars(cfg) -> Gdn2SummaryBars:
    ...
    return Gdn2SummaryBars(
        mb_k_ready=MBarrier(alloc(cfg.smem_raw_bar_stages), try_wait=True, stages=cfg.smem_raw_bar_stages, init_count=1, producer=Producer.TMA_LOAD),
        ...
        mb_w_done=MBarrier(alloc(cfg.smem_raw_stages), try_wait=True, stages=cfg.smem_raw_stages, init_count=CG1_WARPS, producer=Producer.THREAD),
        ...
        mb_beta_ready=MBarrier(alloc(cfg.smem_raw_bar_stages), try_wait=True, stages=cfg.smem_raw_bar_stages, init_count=1, producer=Producer.TMA_LOAD),
        mb_beta_done=MBarrier(alloc(cfg.smem_raw_stages), try_wait=True, stages=cfg.smem_raw_stages, init_count=CG0_GROUP_WARPS, producer=Producer.THREAD),
        mb_k_decay_inv_cg0_ready=MBarrier(
        ...
        mb_state_k_acc_h_ready=MBarrier(alloc(1), try_wait=True, stages=1, init_count=1, producer=Producer.MMA_COMMIT),
        mb_u_acc_h_ready=MBarrier(alloc(1), try_wait=True, stages=1, init_count=1, producer=Producer.MMA_COMMIT),
        mb_state_input_h_cg1_ready=MBarrier(alloc(1), try_wait=True, stages=1, init_count=CG1_WARPS, producer=Producer.THREAD),
...
def register_mma_warp(
    ...
    """Chunk-inverse warp role (warps 12 and 15): the register-MMA T_inv, shared by both chains."""
...
def compute1_warp_group(
    ...
    state_col_id_h = tmem_col + cfg.tmem_state_acc_h_offset
    packed_col_id_h = tmem_col + cfg.tmem_state_input_h_offset
    projection_col_id_h = tmem_col + cfg.tmem_state_k_acc_h_offset
    input_col_id_h = tmem_col + cfg.tmem_y_input_h_offset
    u_acc_addr_h = row_lo_addr + tmem_col + cfg.tmem_u_acc_h_offset
    u_input_addr_h = row_lo_addr + tmem_col + cfg.tmem_u_input_h_offset
    state_col_id_m = tmem_col + cfg.tmem_state_acc_m_offset
    packed_col_id_m = tmem_col + cfg.tmem_state_input_m_offset
    projection_col_id_m = tmem_col + cfg.tmem_state_k_acc_m_offset
    input_col_id_m = tmem_col + cfg.tmem_y_input_m_offset
    ...
    while tile_idx < total_tiles:
        ...
        if num_chunks_tile > 0:
                ...
                for seed_half in cutlass.range_constexpr(2):
                    ...
                    for i in cutlass.range_constexpr(seed_blocks_lo, seed_blocks_hi):
                        ...
                        for packed_col in cutlass.range_constexpr(8):
                            ...
                            state_pack[packed_col] = (
                                identity_pack_lo if key_row == identity_col else (identity_pack_hi if key_row == identity_col + 1 else identity_pack_zero)
                            )
                        nvvm.tcgen05_st(
                            "32x32b",
                    ...
                    for i in cutlass.range_constexpr(seed_blocks_lo, seed_blocks_hi):
                        ...
                        for scale_group in cutlass.range_constexpr(4):
                            ...
                            init_scale_frag = (seed_exchange_ptr + init_scale_idx).load(count=4, alignment=16)
                            for t in cutlass.range_constexpr(2):
                                init_s0 = init_scale_frag[2 * t] if key_row == i * 16 + scale_group * 4 + 2 * t else cutlass.Float32(0.0)
                                init_s1 = init_scale_frag[2 * t + 1] if key_row == i * 16 + scale_group * 4 + 2 * t + 1 else cutlass.Float32(0.0)
                                init_scaled += [init_s0, init_s1]
                        nvvm.tcgen05_st(
                            "32x32b",
            ...
            raw_w_frag_lo = nvvm.ldmatrix(sW_ptr + v_swizzle_off_lo, 4, nvvm.MMALayout.COL)
            raw_w_frag_hi = raw_w_frag_lo
            if cutlass.const_expr(cfg.d_v == 128):
                raw_w_frag_hi = nvvm.ldmatrix(sW_ptr + v_swizzle_off_hi, 4, nvvm.MMALayout.COL)
            if cutlass.const_expr(mState_init is not None):
            ...
            if have_state_h:
                ...
                state_k_vec_lo = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row_lo_addr + projection_col_id_h, cutlass.Float32), num=2)
                for reg_idx in cutlass.range_constexpr(4):
                    frag_pair = reg_idx * 2
                    state_k_pack_lo[reg_idx] = fp32_to_fp16(state_k_vec_lo[frag_pair], state_k_vec_lo[frag_pair + 1], dtype=cfg.io_dtype)
                if cutlass.const_expr(cfg.d_v == 128):
                    state_k_vec_hi = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row_hi_addr + projection_col_id_h, cutlass.Float32), num=2)
                    for reg_idx in cutlass.range_constexpr(4):
                        frag_pair = reg_idx * 2
            ...
            nvvm.tcgen05_st("16x128b", nvvm.make_tmem_ptr(row_lo_addr + input_col_id_h, cutlass.Int8), y_input_pack_lo[0:4])
            if cutlass.const_expr(cfg.d_v == 128):
                nvvm.tcgen05_st("16x128b", nvvm.make_tmem_ptr(row_hi_addr + input_col_id_h, cutlass.Int8), y_input_pack_hi[0:4])
            nvvm.tcgen05_wait("store")
            if nvvm.elect_sync():
            ...
            if seed_state:
                ...
                state_k_vec_lo = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row_lo_addr + projection_col_id_m, cutlass.Float32), num=2)
                for reg_idx in cutlass.range_constexpr(4):
                    frag_pair = reg_idx * 2
                    state_k_pack_lo[reg_idx] = fp32_to_fp16(state_k_vec_lo[frag_pair], state_k_vec_lo[frag_pair + 1], dtype=cfg.io_dtype)
                if cutlass.const_expr(cfg.d_k == 128):
                    state_k_vec_hi = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row_hi_addr + projection_col_id_m, cutlass.Float32), num=2)
                    for reg_idx in cutlass.range_constexpr(4):
                        frag_pair = reg_idx * 2
                        state_k_pack_hi[reg_idx] = fp32_to_fp16(state_k_vec_hi[frag_pair], state_k_vec_hi[frag_pair + 1], dtype=cfg.io_dtype)
            ...
            nvvm.tcgen05_st("16x128b", nvvm.make_tmem_ptr(row_lo_addr + input_col_id_m, cutlass.Int8), y_input_pack_lo[0:4])
            if cutlass.const_expr(cfg.d_k == 128):
                nvvm.tcgen05_st("16x128b", nvvm.make_tmem_ptr(row_hi_addr + input_col_id_m, cutlass.Int8), y_input_pack_hi[0:4])
            nvvm.tcgen05_wait("store")
            if nvvm.elect_sync():
        ...
        for local_chunk in cutlass.range(1, num_chunks_tile, 1, unroll=1):
            ...
            bars.mb_state_acc_h_cg1_done[k_restore_index.idx].wait(k_restore_index.phase)
            state_vecs = []
            ...
            # ---- state stage, right key half: pack, publish, fp32 decay --------------
            bars.mb_state_acc_m_cg1_done[k_restore_index.idx].wait(k_restore_index.phase)
            state_vecs = []
            for i in cutlass.range_constexpr(state_blocks_per_half, cfg.d_k // 16):
        ...
        if batch_seqlen > 0:
            if owns_final:
                ...
                state_dst = (mState_out.iterator + mState_out.layout((batch_idx, head_o, state_gmem_row, 0))).raw_ptr()
                        ...
                        if row_valid_h:
                            (state_dst + key_block_start + g * state_vw).store(
                                cutlass.Vector.from_elements(
                                    tuple(state_vec[g * state_vw + t].to(mState_out.element_type) for t in range(state_vw)),
                ...
                state_dst = (mTransition.iterator + mTransition.layout((batch_idx, head_o, key_row, 0))).raw_ptr()
                for key_block_start in cutlass.range_constexpr(0, cfg.d_k, 32):
                    state_vec = nvvm.tcgen05_ld(
                        "32x32b",
                    ...
                    for g in cutlass.range_constexpr(32 // state_vw):
                        if row_valid_m:
                            (state_dst + key_block_start + g * state_vw).store(
                                cutlass.Vector.from_elements(
                                    tuple(state_vec[g * state_vw + t].to(mTransition.element_type) for t in range(state_vw)),
...
def chunk_gdn2_summary(
    ...
    *,
    ...
    """BT=16 fused GDN-2 state summary: one launch writes ``output_state`` (H) and ``output_transition`` (stored domain
