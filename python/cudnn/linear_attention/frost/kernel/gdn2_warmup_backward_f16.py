...
"""One compiled launch for the GDN-2 warmup and uncut backward: the split-K table (warmup only), the recompute prologue and
the checkpoint-series recompute (unless the forward's per-chunk series is passed back), the bprop prologue and the bprop,
issued from a single host at ``--opt-level 2``, the level of every GDN-2 module, its prologues and the split table
...
(the table's 4-byte compact views of work_items, work_count, item_scratch and cu_seqlens; ``beta`` at 16 bytes in the
prologues and 4 in the recompute and the bprop's guarded read; the gate when the bprop reads it as a linear alpha)."""
...
def build_warmup_backward(
    *,
    ...
    bwd_order_gen = not (bwd_orders and split)
    key = (
        str(q.dtype),
    ...
    if key not in warmup_backward_cache:
        ...
        warmup_backward_cache[key] = cute.compile(
            ...
            from_dlpack(a_log, assumed_align=4) if a_log is not None else None,
