...
def build_cfg(
    ...
    *,
    ...
    if d_k != 128:
        raise ValueError(f"d_k={d_k} not supported; the q/k head dim must be 128")
    if d_v not in (64, 128):
        raise ValueError(f"d_v={d_v} not supported; the v head dim must be 64 or 128")
