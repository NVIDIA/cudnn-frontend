...
Pipeline (direct CUTLASS primitives, chunk_idx-size 16 KDA schedule):
  ...
  tcgen05-MMA: State*K/new-v/kv-update
...
SMEM layout:
  ...
  A_inv                      2048  2       <-- SW32 16x16 register-MMA tiles
...
TMEM layout (240 of 512 columns):
  ...
  rhs input       224-231  packed b16 A operand: Beta * (V - State*K)
...
def super_mma_warp(
    ...
    register-MMA Neumann-series A_inv."""
    nvvm.setmaxregister(cfg.num_regs_other, nvvm.SetMaxRegisterAction.DECREASE)
...
def tcgen05_mma_warp(
    ...
    while tile_idx < total_tiles:
        ...
        for local_chunk_idx in cutlass.range(num_chunks_tile, unroll=1):
            ...
            # ---- final_state += update(T) @ K_restore ----------------------------
...
def compute1_warp_group(
    ...
    upd_acc_addr = row_addr + tmem_col + cfg.tmem_update_acc_offset
    upd_inp_addr = st_row_addr + tmem_col + cfg.tmem_update_inp_offset
    v_swz_off0 = (
    ...
    while tile_idx < total_tiles:
        ...
        if num_chunks_tile > 0:
            ...
            state_k0 = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row_addr + statek_col_id, cutlass.Float32), num=2)

            state_k1 = nvvm.tcgen05_ld("16x256b", nvvm.make_tmem_ptr(row16_addr + statek_col_id, cutlass.Float32), num=2)
            ...
            update = nvvm.tcgen05_ld(
                "32x32b",
                nvvm.make_tmem_ptr(upd_acc_addr, cutlass.Float32),
                num=cfg.b_t,
            )
            ...
            nvvm.tcgen05_st(
                "32x32b",
                nvvm.make_tmem_ptr(upd_inp_addr, cutlass.Int8),
            ...
            if cutlass.const_expr(cfg.enable_checkpoints):
                if do_checkpoint:
                    ...
                    for slab in cutlass.range_constexpr(cfg.d_k // 16):
                        ...
                        checkpoint_row = slab * 16 + frag_row_coord
                        nvvm.stmatrix(
                            sCheckpoint_ptr
...
def kernel(
    ...
    #   K_decay            : tcgen05 SW128 operand shared with super-MMA
...
def build_all_descs_kernel(
    ...
    """Single-launch builder kernel: one warp emits each per-batch TMA
    descriptor array."""
    tidx, _, _ = cute.arch.thread_idx()
...
def build_descs(
    ...
    n_warps = 4 if state_checkpoints is not None else 3
