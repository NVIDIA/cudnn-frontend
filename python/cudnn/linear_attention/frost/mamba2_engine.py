...
class Mamba2FrostEngine(BaseEngine):
    ...
    def check_support(self, graph):
        ...
        if facts.head_dim != 64 or facts.state_dim != 128 or facts.chunk_size != 32:
            raise NotImplementedError("Mamba2FrostEngine requires head_dim=64, state_dim=128 (Nemotron), and chunk_size=32")
        ...
        if facts.intermediate_dtype == "bfloat16" and "z" in facts.tensors:
            raise NotImplementedError("Mamba2FrostEngine: SiLU gate requires intermediate_dtype=float32")
...
class CompiledMamba2:
    ...
    def __init__(self, node, facts):
        ...
        if not facts.is_bwd:
            raw = scratch("_raw", "float32", bc_partials_shape)
            ...
            step("gate_forward", (raw, present("z"), "O", present("ungated_out")))
        else:
            dy = "dO"
            ddp = scratch("_ddp", "float32", blocks)
            if present("z"):
                dy = scratch("_dy", intermediate, xshape)
