...
"""Torch API for block-scaled dense GEMM + amax on SM100."""
...
class GemmAmaxSm100(APIBase):
    def __init__(
        ...
        self.amax_desc = self._pad_tensor_to_ndim(
            self._make_tensor_desc(sample_amax, name="sample_amax"),
            3,
            "sample_amax",
        )
        ...
        self._logger.debug(
            "setting num_cluster_overlap_margin: %s",
            self.num_cluster_overlap_margin,
        )

        # Torch uint8 is the legacy packed-FP4 storage alias for this API.
        ...
        self._logger.debug(
            "__init__ completed with args: sample_a %s, sample_b %s, "
            "sample_sfa %s, sample_sfb %s, sample_c %s, sample_amax %s, "
            "acc_dtype %s, mma_tiler_mn %s, cluster_shape_mn %s, sf_vec_size %s",
            self.a_desc.shape,
            self.b_desc.shape,
            self.sfa_desc.shape,
            self.sfb_desc.shape,
    ...
    def check_support(self) -> bool:
        ...
        self.a_major = self._op.a_major
        self.b_major = self._op.b_major
        self.c_major = self._op.c_major
        ...
        if self.sfa_desc.dtype == torch.int8:
