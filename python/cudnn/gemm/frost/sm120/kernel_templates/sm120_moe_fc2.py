...
"""Experimental SM120 BF16 grouped projection.

Derived from NVIDIA Frost's generated SM120 grouped GEMM implementation.
This work incorporates Yanqin Zhai's FE PR #1090
(068ffd87f053543a63ddf929aeb87fcc8b231380) and NVIDIA CuTeDSL MegaMoE
(bangyus/cutedsl_megamoe snapshot d19d30a), together with our research
and implementation. Inputs and outputs use the declared canonical layouts.
"""
...
moe_static_sched = FROST_TEMPLATE_PARAMS.static_sched  # Public SCHED_POLICY: 0 dynamic (default), 1 static.

# Kernel geometry.
mma_inst_shape_mnk = (16, 8, 16)
cgrp_tile_mnk = (128, 8, 64)
cta_tile_mnk = (128, 8, 64)
threads_per_cta = 256
...
tile_swizzle_n = 0
swizzle_l2_budget_bytes = 33554432
num_a_operands = 1
num_b_operands = 1
num_gemms = 1
vec_bytes_epi = 16
epi_chunk_elems = 8
split_k_slices = 1
frost_compile_options = "--enable-tvm-ffi --gpu-arch sm_120a"
grid_num_clusters = FROST_TEMPLATE_PARAMS.grid_ctas
offset_cutlass_dtype = cutlass.Int32
moe_static_sched = FROST_TEMPLATE_PARAMS.static_sched
vec_bytes_tap_0 = 16
...
_STG_EPI_WARP_ELEMS = 128  # One m16n8 accumulator, four f32 values per lane.
...
@cute.kernel
def frost_pathfinding_sm120_swap_fc2_m128n8k64(
    m: cutlass.Int64,
    n: cutlass.Int64,
    k: cutlass.Int64,
    ...
    if warp_idx < NUM_COMPUTE_WARPS:
        ...
        while is_valid != 0:
            ...
            # Transpose the m16n8 register fragment into the existing FI output.
            # PTX lane mapping is also documented in MegaMoE's sm120_mma.py.
            # Four adjacent BF16 channels per lane; 8 token rows per atom.
            _stg_stage = smem_stg_epi.subview(warp_idx * _STG_EPI_WARP_ELEMS)
            for mf in cutlass.range_constexpr(_M_FRAGS):
                for nf in cutlass.range_constexpr(_N_FRAGS):
                    _o = (mf * _N_FRAGS + nf) * 4
                    (_stg_stage.data_ptr() + lane * 4).store(accumulators[0][_o:4], alignment=16)
                    nvvm.bar_warp_sync(0xFFFFFFFF)
                    token = coord_n + warp_col * _WARP_TILE_N + nf * 8 + lane_div4
                    channel = coord_m + warp_row * _WARP_TILE_M + mf * 16 + lane_mod4 * 4
                    if token < group_end and channel + 4 <= M:
                        _values = cutlass.Array(cd_dtype, 4, alignment=8)
                        for j in cutlass.range_constexpr(4):
                            _m = lane_mod4 * 4 + j
                            _src_lane = (_m % 8) * 4 + lane_div4 // 2
                            _src_reg = (_m // 8) * 2 + lane_div4 % 2
                            _value = (_stg_stage.data_ptr() + _src_lane * 4 + _src_reg).load()
                            _values[j] = cd_dtype(_value)
                        (gC_tap_0_ptr + channel * out_stride_m_0 + token * out_stride_n_0).store(_values[0:4], alignment=8)
                    nvvm.bar_warp_sync(0xFFFFFFFF)
...
frost_pathfinding_sm120_swap_fc2_m128n8k64.set_name_prefix("cudnn", remove_cutlass_symbol=True)
...
def _host(
    ...
    frost_pathfinding_sm120_swap_fc2_m128n8k64(
        problem_size[0],
        problem_size[1],
        problem_size[2],
...
def compile() -> Callable:
    ...
    sym_m = cute.sym_int64(divisibility=out_vec_elems)
    ...
    fake_c_tap_0 = cute.runtime.make_fake_tensor(
        cutlass.BFloat16,
        (sym_m, sym_n, 1),
        stride=(1, cute.sym_int64(), cute.sym_int64()),
        assumed_align=16,
    )
    problem_size = (
        sym_m,
...
assert not a_is_m_major and not b_is_n_major and num_gemms == 1
