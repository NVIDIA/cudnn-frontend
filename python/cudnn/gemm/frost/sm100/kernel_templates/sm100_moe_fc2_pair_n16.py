...
mma_k_dim = 0
cta_group = 1
cgrp_tile_mnk = (16, 64, 64)
cta_tile_mnk = (16, 64, 64)
epi_tile_mn = (16, 64)
threads_per_cta = 256
...
@cute.kernel
def frost_sm100_moe_fc2_pair_m128n16k16_sched_static_stages_early_pdl_compact_resources(
    m: cutlass.Int64,
    n: cutlass.Int64,
    k: cutlass.Int64,
    ...
    # A contains128 weight rows; B contains16 routed token rows.
    ...
    if warp_idx < num_epilogue_warps:
        ...
        while is_valid != 0:
            ...
            acc_base_col = base_col_id_root + acc_stage * acc_region_cols
            acc_vecs = []
            for token_subtile in cutlass.range_constexpr(cta_tile_mnk[0] // 8):
                tmem = cutlass.inttoptr(
                    (row_id_with_warp_offset << 16) | (acc_base_col + token_subtile * 8),
                    6,
                    mma_c_dtype,
                )
                acc_vecs.append(nvvm.tcgen05_ld(shape, tmem, num=8))
            ...
            for token_subtile in cutlass.range_constexpr(cta_tile_mnk[0] // 8):
                for token_idx in cutlass.range_constexpr(8):
                    value = acc_vecs[token_subtile][token_idx].to(cutlass.BFloat16)
                    row = coord_m_tile + token_subtile * 8 + token_idx
...
frost_sm100_moe_fc2_pair_m128n16k16_sched_static_stages_early_pdl_compact_resources.set_name_prefix(
...
def _host(
    ...
    frost_sm100_moe_fc2_pair_m128n16k16_sched_static_stages_early_pdl_compact_resources(
        problem_size[0],
        problem_size[1],
        problem_size[2],
