...
mma_inst_shape_mnk = (64, 8, 16)
mma_k_dim = 0
cta_group = 1
cgrp_tile_mnk = (8, 32, 64)
cta_tile_mnk = (8, 32, 64)
epi_tile_mn = (8, 32)
threads_per_cta = 256
...
epi_stage_rows = 8
epi_chunk_elems = 8
ab_stages = 12  # 12 * (M64xK64 weights + N8xK64 tokens) = 110,592B
...
# Eight-word aligned stride with a compact six-word scheduler payload.
SCHED_SLOT_WORDS = 8
...
@cute.kernel
def frost_sm100_moe_swiglu_pair_m64n8k16_sched_static_s12_early_pdl_compact_resources(
    m: cutlass.Int64,
    n: cutlass.Int64,
    k: cutlass.Int64,
    ...
    if warp_idx < num_epilogue_warps:
        ...
        # M64's widest drain gives each lane two token values from one gate
        # row and its paired up row, eliminating cross-lane gate/up shuffles.
        ...
        while is_valid != 0:
            ...
            acc_vec = nvvm.tcgen05_ld(shape, tmem, num=1)
            nvvm.tcgen05_wait(kind=nvvm.Tcgen05Wait.LOAD)
            nvvm.tcgen05_fence(nvvm.Tcgen05Fence.BEFORE_THREAD_SYNC)
            if elect_one:
                nvvm.mbarrier_arrive(acc_empty_mbar_ptr.subview(acc_stage))

            # Rank-5 row order is gate[8],up[8].  A 16x256b load places two
            # token columns for a gate row in words 0:2 and its paired up row
            # in words 2:4 of the same lane.
            channel = coord_n_c + warp_idx * 8 + lane // 4
            token_pair = (lane & 3) * 2
            for token_in_pair in cutlass.range_constexpr(2):
                gate = acc_vec[token_in_pair].to(cutlass.Float32)
                up = acc_vec[token_in_pair + 2].to(cutlass.Float32)
                row = coord_m_tile + token_pair + token_in_pair
                if row < group_end:
                    silu = gate * cute.math.rcp(
                        cutlass.Float32(1.0)
                        + cute.math.exp2(
                    ...
                    value = (up * silu).to(cutlass.BFloat16)
                    linear_idx = tile_l * out_stride_l_0 + row * out_stride_m_0 + channel * out_stride_n_0
...
frost_sm100_moe_swiglu_pair_m64n8k16_sched_static_s12_early_pdl_compact_resources.set_name_prefix(
    "cudnn_kblock64_m64" if moe_kblocked64 else "cudnn_m64",
...
def _host(
        ...
        if cutlass.const_expr(b_is_n_major):
            ...
            tma_b_desc_list.append(
                _tma.create_tensor_map_tiled(
                    ...
                    # paired output channels into the physical MMA-M.
    ...
    frost_sm100_moe_swiglu_pair_m64n8k16_sched_static_s12_early_pdl_compact_resources(
        problem_size[0],
        problem_size[1],
        problem_size[2],
