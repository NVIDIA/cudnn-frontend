...
"""Compact causal dS and grouped dK/dV producer.
...
def _hd256_bwd_sparse_head_state(
    ...
    """Analytic causal row state."""
    first = cutlass.max(n_block, m_block_max[0])
    count = cutlass.max(m_block_max[1] - first, 0)
    partial_count = cutlass.min(count, 1) if first == n_block else Int32(0)
    return first, first + partial_count, partial_count, count - partial_count, count
...
def _hd256_bwd_sparse_m_block(
    ...
    """Map an iteration to its causal query tile."""
    is_partial_block = iter_idx < partial_count
    m_block = partial_base + iter_idx
    return m_block, cutlass.min(m_block, m_block_max[1] - 1), not is_partial_block, is_partial_block, Int32(0), Int32(0)
...
def _hd256_bwd_sparse_head_loop_count(
    ...
    """Return the causal query-tile count."""
    return cutlass.max(m_block_max[1] - cutlass.max(n_block, m_block_max[0]), 0)
...
class BlackwellFusedMultiHeadAttentionBackwardDKDVKernel:
    ...
    def __init__(
        ...
        """Initialization."""
        self.packed_lengths = False
        self.ds_maxlen = ds_maxlen
        self.ds_bin = ds_bin
        assert ds_bin >= 128 and ds_bin % 128 == 0
        ...
        self.threads_per_cta = 384  # compute WG + dQ drain WG + loader/MMA WG
    ...
    def _setup_attributes(self):
        ...
        self.load_compute_sum_OdO_stage = 2
    ...
    def __call__(
        ...
        @cute.struct
        class SharedStorage:
            # SM107: persistent transposed K and independent dQ staging.
            # Pipeline barriers
        ...
        K_val = cutlass.min(K_val, query_start + ds_global.shape[2])
        grid_problem_shape = (
            problem_shape[0],
            K_val,
            problem_shape[2],
    ...
    def dkdv_bwd(
        ...
        if warp_idx < 8:
            ...
            cute.arch.warpgroup_reg_dealloc(104)
        varlen = cumulative_s_q is not None
        ...
        load_compute_LSE_producer, load_compute_LSE_consumer = pipeline.PipelineAsync.create(
        ...
        load_compute_sum_OdO_producer, load_compute_sum_OdO_consumer = pipeline.PipelineAsync.create(
        ...
        tmem_alloc_barrier = pipeline.NamedBarrier(barrier_id=self.tmem_alloc_sync_bar_id, num_threads=self.threads_per_cta)
        ...
        if cutlass.const_expr(varlen):
            ...
            if cutlass.const_expr(self.packed_lengths):
                seqlen_q_cur_batch = cumulative_s_k[bidz]
                seqlen_k_cur_batch = cumulative_s_k[bidz]
            else:
                seqlen_k_cur_batch = cumulative_s_k[bidz + 1] - cumulative_s_k[bidz]
            blk_offset = (
                cumulative_s_q[bidz],
                cumulative_s_q[bidz] if cutlass.const_expr(self.packed_lengths) else cumulative_s_k[bidz],
                Int32(0),
                ((Int32(0), Int32(0)), Int32(0)),
            )
        iter_start, iter_end = self.get_Q_block_min_max(seqlen_q_cur_batch)
        m_block_max = (query_start // 128, cute.ceil_div(cutlass.min(query_start + ds_global.shape[2], seqlen_q_cur_batch), 128))
        ...
        problem_shape_cur_batch = (seqlen_q_cur_batch, seqlen_k_cur_batch, problem_shape[2], problem_shape[3])
        if iter_count <= 0:
            pass
        ...
        elif warp_idx == self.stats_warp_id:
            self.load_stats(
                LSE,
                sum_OdO,
                sLSE,
            ...
            cute.arch.barrier(barrier_id=self.epilogue_sync_bar_id, number_of_threads=self.num_compute_warps * self.threads_per_warp)
        else:
            pass
        cute.arch.cluster_arrive()
        cute.arch.cluster_wait()
    ...
    def load(
        ...
        m_block_max: tuple[Int32, Int32] = (Int32(0), Int32(0)),
    ):
        """TMA load."""
        ...
        if cutlass.const_expr(varlen):
            cuseqlen_q_stats = cute.assume((blk_offset[0] + blk_coord_b * self.tile_shape_Q) // self.tile_shape_Q * self.tile_shape_Q, divby=self.tile_shape_Q)
            blk_offset_stats = (cuseqlen_q_stats, blk_offset[1], blk_offset[2], blk_offset[3])
        ...
        cute.copy(tma_atom_K, tKgK_mkl[None, mma_tile_coord_m, 0, (blk_coord_h, blk_coord_b)], tKsK[None, 0], tma_bar_ptr=k_handle.barrier)
        m_block_for_load = iter_index
        blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
        ...
        _, m_block_for_load, _, _, _, _ = _hd256_bwd_sparse_m_block(
            block_sparse_tensors, iter_index, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count, self.subtile_factor, m_block_max
        )
        q_handle = load_mma_Q_producer.acquire_and_advance()
        cute.copy(tma_atom_Q, tQgQ_mkl[None, m_block_for_load, 0, (blk_coord_h, blk_coord_b)], tQsQ[None, q_handle.index], tma_bar_ptr=q_handle.barrier)
        v_handle = load_mma_V_producer.acquire_and_advance()
        cute.copy(tma_atom_V, tVgV_mkl[None, mma_tile_coord_m, 0, (blk_coord_h, blk_coord_b)], tVsV[None, 0], tma_bar_ptr=v_handle.barrier)
        do_handle = load_mma_dO_producer.acquire_and_advance()
        cute.copy(tma_atom_dO, tdOgdO_mkl[None, m_block_for_load, 0, (blk_coord_h, blk_coord_b)], tdOsdO[None, do_handle.index], tma_bar_ptr=do_handle.barrier)
        iter_count -= 1
        if iter_count > 0:
            blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
        ...
        while iter_count > 0:
            ...
            do_handle = load_mma_dO_producer.acquire_and_advance()
            cute.copy(
                tma_atom_dO, tdOgdO_mkl[None, m_block_for_load, 0, (blk_coord_h, blk_coord_b)], tdOsdO[None, do_handle.index], tma_bar_ptr=do_handle.barrier
            )
            iter_count -= 1
            if iter_count > 0:
                blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
    ...
    def load_transposed(
        ...
        cute.copy(
            tma_atom_dOT, tdOTgdOT_mkl[None, 0, m_block_for_load, (blk_coord_h, blk_coord_b)], tdOTsdOT[None, dot_handle.index], tma_bar_ptr=dot_handle.barrier
        )
        qt_handle = load_mma_QT_producer.acquire_and_advance()
        cute.copy(tma_atom_QT, tQTgQT_mkl[None, 0, m_block_for_load, (blk_coord_h, blk_coord_b)], tQTsQT[None, qt_handle.index], tma_bar_ptr=qt_handle.barrier)
        iter_count -= 1
        if iter_count > 0:
            blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
                _hd256_bwd_sparse_group_next_work_item(
        ...
        while iter_count > 0:
            ...
            qt_handle = load_mma_QT_producer.acquire_and_advance()
            cute.copy(
                tma_atom_QT, tQTgQT_mkl[None, 0, m_block_for_load, (blk_coord_h, blk_coord_b)], tQTsQT[None, qt_handle.index], tma_bar_ptr=qt_handle.barrier
            )
            iter_count -= 1
            if iter_count > 0:
                blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
    ...
    def load_stats(
        ...
        thread_idx = tidx % 32
        async_copy_num_elts = sLSE.shape[0] // 32
        atom_stats_copy = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), self.acc_dtype, num_bits_per_copy=32)
        blk_coord_h_r, iter_index, sparse_head_count, sparse_partial_base, sparse_full_base, sparse_partial_count, sparse_full_count = (
            _hd256_bwd_sparse_group_first_work_item(
                block_sparse_tensors,
                blk_coord_b,
        ...
        while iter_count > 0:
            ...
            idx = cute.assume(self.tile_shape_Q * m_block_for_load + thread_idx * 4, divby=4)
            dst = cute.assume(thread_idx * 4, divby=4)
            if idx + 3 < problem_shape[0]:
                src = LSE_for_copy[None, idx, (blk_coord_h, blk_coord_b)]
                out = sLSE_for_copy[None, dst, lse_handle.index]
                src4 = cute.make_tensor(src.iterator, cute.make_layout(4))
                out4 = cute.make_tensor(out.iterator, cute.make_layout(4))
                vector_copy = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), self.acc_dtype, num_bits_per_copy=128)
                cute.copy(vector_copy, src4, out4)
            else:
                for i in cutlass.range_constexpr(4):
                    if idx + i < problem_shape[0]:
                        cute.copy(atom_stats_copy, LSE_for_copy[None, idx + i, (blk_coord_h, blk_coord_b)], sLSE_for_copy[None, dst + i, lse_handle.index])
                    else:
                        sLSE_for_copy[None, dst + i, lse_handle.index].fill(0.0)
            lse_handle.commit()
            sum_odo_handle = load_compute_sum_OdO_producer.acquire_and_advance()
            idx = cute.assume(self.tile_shape_Q * m_block_for_load + thread_idx * 4, divby=4)
            dst = cute.assume(thread_idx * 4, divby=4)
            if idx + 3 < problem_shape[0]:
                src = sum_OdO_for_copy[None, idx, (blk_coord_h, blk_coord_b)]
                out = sSum_OdO_for_copy[None, dst, sum_odo_handle.index]
                src4 = cute.make_tensor(src.iterator, cute.make_layout(4))
                out4 = cute.make_tensor(out.iterator, cute.make_layout(4))
                vector_copy = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), self.acc_dtype, num_bits_per_copy=128)
                cute.copy(vector_copy, src4, out4)
                ...
                for i in cutlass.range_constexpr(4):
                    if idx + i < problem_shape[0]:
                        cute.copy(
                            atom_stats_copy, sum_OdO_for_copy[None, idx + i, (blk_coord_h, blk_coord_b)], sSum_OdO_for_copy[None, dst + i, sum_odo_handle.index]
                        )
                    else:
                        sSum_OdO_for_copy[None, dst + i, sum_odo_handle.index].fill(0.0)
    ...
    def mma_2cta(
        ...
        p_releaser = compute_mma_P_consumer.clone()
        qt_releaser = load_mma_QT_consumer.clone()
        ...
        if is_leader_cta:
            ...
            dot_handle.release()
            p_handle.release()  # Only dV reads P; dQ now has independent shared storage.
    ...
    def compute(
        ...
        while iter_count > 0:
            ...
            # This standalone prototype supports causal attention only.
            # A single compute WG owns both original mask-payload fragments.
            if is_partial_block or (dq_m_block + 1) * 128 > Q:
                for i in cutlass.range_constexpr(cute.size(tTR_rST)):
                    key_pos = blk_coord_k * 64 + cute.get(tTR_cST[i], mode=[0])
                    query_pos = dq_m_block * 128 + cute.get(tTR_cST[i], mode=[1])
                    if key_pos > query_pos or key_pos >= problem_shape[1] or query_pos >= Q:
                        tTR_rST[i] = cutlass.Float32(-float("inf"))
            ...
            tTR_rPT = self.quantize(tTR_rST, cutlass.BFloat16)  # tTR_rST is ST in fp32 in RF.
            ...
            # Power-of-two scale is included in the materialized derivative.
            for i in cutlass.range(0, cute.size(tTR_rdPT), 2, unroll_full=True):
                tTR_rdPT[i], tTR_rdPT[i + 1] = cute.arch.mul_packed_f32x2((tTR_rdPT[i], tTR_rdPT[i + 1]), (scale_softmax, scale_softmax))
            # convert fp32 dS to fp16 dS which will be used in the computation of dK and DQ
            tTR_rdST = self.quantize(tTR_rdPT, cutlass.BFloat16)
            self.reg_to_smem_mma128x128_2cta(tTR_rdST, sdST, ds_handle.index, (self.tile_shape_K, self.tile_shape_Q), dp_idx, wg_idx)
            cute.arch.fence_view_async_shared()
            ds_handle.commit()
            ...
            # Every query in the 128-column tile belongs to the same coarse bin.
            query_base = dq_m_block * 128
            leading = ds_global.shape[3]
            bh = blk_coord[3][1] * 8 + blk_coord[3][0][1] * self.qhead_per_kvhead + sparse_qhead_offset
            head_stride = cutlass.Int64(ds_global.shape[2]) * ds_global.shape[3]
            base_offset = cutlass.Int64(bh) * head_stride + cutlass.Int64(query_base - m_block_max[0] * 128) * leading
            base_offset += cutlass.Int64(blk_coord_k * 64)
            local_ds = cute.make_tensor(ds_global.iterator + base_offset, cute.make_layout((64, 128), stride=(1, leading)))
            if cutlass.const_expr(self.fast_ds_store):
                if blk_coord_k * 64 + 64 <= problem_shape[1] and query_base + 128 <= Q:
                    target = thr_t2r.partition_D(local_ds)
                    target = split_wg(target, num_warp_groups, wg_idx)
                    cute.autovec_copy(tTR_rdST, target)
                else:
                    for i in cutlass.range_constexpr(cute.size(tTR_rdST)):
                        key_local = cute.get(tTR_cdPT[i], mode=[0])
                        query_local = cute.get(tTR_cdPT[i], mode=[1])
                        if blk_coord_k * 64 + key_local < problem_shape[1] and query_base + query_local < Q:
                            local_ds[key_local, query_local] = tTR_rdST[i]
            else:
                for i in cutlass.range_constexpr(cute.size(tTR_rdST)):
                    key_local = cute.get(tTR_cdPT[i], mode=[0])
                    query_local = cute.get(tTR_cdPT[i], mode=[1])
        ...
        mma_compute_dKdV_consumer = self.epilogue(
            ...
            sdOT,
            sP,
            blk_coord[1] * self.tile_shape_K < m_block_max[0] * 128,
        )
    ...
    def store(
        ...
        for i in cutlass.range(cute.size(coord, mode=[2]), unroll_full=True):
            ...
            if cute.elem_less(coord_i[0], tensor_shape):
                values = regs_i.load()
                if accumulate:
                    values += gmem_i.load().to(cutlass.Float32)
                gmem_i.store(values.to(gmem.element_type))
    ...
    def epilogue(
        ...
        if blk_coord_k * self.tile_shape_K < problem_shape_k_cur_batch:
            ...
            if cutlass.const_expr(not varlen):
                ...
                self.store(tTR_gdV, tTR_rdV, tTR_cdV, (K, D), accumulate)
        ...
        if blk_coord_k * self.tile_shape_K < problem_shape_k_cur_batch:
            ...
            # The materialized/shared dS already includes softmax scaling.
            ...
            if cutlass.const_expr(not varlen):
                ...
                self.store(tTR_gdK, tTR_rdK, tTR_cdK, (K, D), accumulate)
