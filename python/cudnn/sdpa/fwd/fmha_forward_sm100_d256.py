...
class BlackwellFusedMultiHeadAttentionForward:
    ...
    def __call__(
        ...
        class SharedStorage:
            ...
            load_kv_mbar_ptr: cute.struct.MemRange[Int64, self.kv_stage * 2]  # load_kv_{producer,consumer}
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.use_clc_dynamic_scheduler):
            ...
            num_clc_consumer_threads = self.threads_per_warp * (
                1 + cluster_size * (len(self.softmax_warp_ids) + len(self.correction_warp_ids) + 1 + 1)  # sched_warp (CTA 0 only)  # mma_warp  # load_warp
            )
    ...
    def store_sum_max(
        ...
        if cutlass.const_expr(mLSE is not None):
            ...
            hb_idx = (current_block_coord[2][0], Int32(0)) if cutlass.const_expr(cum_seqlen_q is not None) else current_block_coord[2]
            lse_value = scale_softmax * row_max + cute.math.log(row_sum, fastmath=True)
            if cute.elem_less(q_idx, seqlen_q):
                global_q_idx = q_idx + cuseqlen_q if cutlass.const_expr(cum_seqlen_q is not None) else q_idx
