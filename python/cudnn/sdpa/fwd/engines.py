...
def mismatch(capabilities: Capabilities, facts: "ga.SdpaGraphFacts", knobs: Optional[SdpaFwdKnobs] = None) -> Optional[str]:
    ...
    if facts.has_paged_kv:
        ...
        # runs unsplit. The FP8 kernel's sink fold over pools is not validated,
        # so that pair stays declined on the fp8 row.
        if facts.is_mxfp8:
...
def _sm100_fp8_spec(*, arch: str = "sm100") -> EngineSpec:
    ...
    return EngineSpec(
        ...
        capabilities=Capabilities(
            ...
            # mismatch() keeps it to dense, sink-free Q (the fp8 THD path
            # clamps runtime K/V descriptors to a packed total a pool does not
            # have; the sink fold over pools is not validated on this kernel).
