...
- ``s_q == 1``, d256: b = 1 loses everywhere (1.3-5.4x); with ``units = b * h_kv``, ``units >= 32``
  ...
  pack the GQA group, so FROST also wins at b = 1 from 32 query heads (0.33-0.86) -> LEAD on those,
  TRAIL otherwise.
...
SQ1_MQA_MIN_Q_HEADS = 32  # s_q == 1, d512 (one KV head): FROST packs the group, the backend does not -> wins from 32 q heads at b = 1
    ...
    if not facts.thd and facts.s_q == 1:
        ...
        if _selected_d_shape(caps, facts) == (512, 512):
            if facts.h_q // max(facts.h_kv, 1) > SM120_SQ1_MAX_GQA_GROUP:
                return TRAIL
            # one KV head: FROST packs the query group, so at b = 1 it wins from 32 query heads
            # (0.44-0.85) and loses below (1.26-1.30), as on SM100
            return LEAD if units >= SM120_SQ1_MIN_KV_UNITS or facts.h_q >= SQ1_MQA_MIN_Q_HEADS else TRAIL
...
def _place_sm100_f16(caps: Capabilities, facts) -> str:
    ...
    if dense and facts.s_q == 1:
        units = facts.b * facts.h_kv
        if flavor == (512, 512):
            return LEAD if units >= SQ1_MIN_KV_UNITS or facts.h_q >= SQ1_MQA_MIN_Q_HEADS else TRAIL
        if flavor == (256, 256):
