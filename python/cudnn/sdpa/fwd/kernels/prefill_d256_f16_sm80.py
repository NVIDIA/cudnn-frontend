...
Pipeline shape (qwen, d_qk = d_v = 256):
  ...
  │  8 warps × 32 threads = 256 threads/CTA, no warp specialization.│
  │  TILE_M = 128 (Q rows / CTA — each warp owns 16, m_blocks = 1)  │
  │  TILE_N = 64  (KV rows / iter)                                  │
  │  D_QK   = 256                                                   │
  │  D_V    = 256                                                   │
  │                                                                 │
  │  SMEM (tile_m=128, num_warps=8):                                │
  │         sQ_buf    64 KiB  (Q during prologue; splits into sV0 + │
  │                            sV1 = double-buffered V ring after   │
  │                            ldmatrix Q to regs)                  │
  │         sK_buf    64 KiB  (K[i] / K[i-1] double-buffered ring;  │
  │                            stage = i & 1 at runtime)            │
  │         total =  128 KiB  (well under A100's 164 KiB opt-in)    │
  │                                                                 │
  │  Pipeline timeline (K AND V prefetched one iter ahead):         │
  │    prologue:    cp.async Q + K[N-1]            (commits)        │
  │                 wait_group(1) → Q done, K still in flight       │
  │                 ldmatrix Q → regs                               │
  │                 barrier (Q reads done; sQ_buf free for sV ring) │
  │                 cp.async V[N-1] → sV[0] = sQ_buf  (commit)      │
  │                                                                 │
  │    loop iter i: cp.async K[i-1] → sK_next       (commit g_K)    │
  │                 cp.async V[i-1] → sV_next       (commit g_V)    │
  │                 wait_group(3) → drain 1 = prev K (or K-prol)    │
  │                 barrier (sK_cur = K[i] visible)                 │
  │                 QK mma (ldmatrix.x4 for K)                      │
  │                 softmax                                         │
  │                 wait_group(2) → drain 1 = prev V (or V-prol)    │
  │                 barrier (sV_cur = V[i] visible)                 │
  │                 SV mma (ldmatrix.x4 for V, b_trans)             │
  │                 barrier (sync warps; next iter's V_pref         │
  │                          overwrites sV_cur(i) slot)             │
  │                                                                 │
  │    post-loop:   barrier (all warps finished last SV mma)        │
  │                 epilogue: fp32→fp16 cast + STG.128 via sO=sQ    │
  └─────────────────────────────────────────────────────────────────┘

Pending-group accounting:
  After V-prologue commit (no drain): 2 groups (K-prologue, V-prologue).
  After iter top K + V prefetch commits: 4 groups.
  ``wait_group(3)`` at iter top drains 1 oldest (K-prologue / prev K-pref);
  ``wait_group(2)`` mid-iter drains 1 more (V-prologue / prev V-pref).
  At iter end: 2 groups left = this iter's K-pref + V-pref (drained next
  iter).  Splitting the drain across iter top and mid-iter lets V cp.async
  overlap the QK k-loop's MIO traffic.

Layout choices (same as the shared kernel):
- Swz128B XOR swizzle on K / V SMEM for bank-conflict-free ldmatrix.
- ldmatrix.x4 .row for Q (A operand at QK time, loaded once in prologue).
- ldmatrix.x4 for K and V (B operands) via ``load_b_smem_x4`` — halves
...
def _sdpa_kernel(
    ...
    io_dtype: cutlass.Constexpr,  # cutlass.Float16 / cutlass.BFloat16 (bf16 reuses
    # the fp16 pipeline; only mma variant + casts flip).
    ...
    causal_bottom_right: cutlass.Constexpr[bool],  # MASK_CAUSAL: bottom-right diagonal
    # (k <= q + (SKV - SQ)) vs top-left (k <= q).
    has_seq_kv_lens: cutlass.Constexpr[bool],  # read per-batch KV length & mask cols >= it.
    ...
    # Causal diagonal offset: top-left ⇒ 0; bottom-right ⇒ SKV-SQ.  The runtime
    # ``causal_band_right`` widens the band by that many future tokens
    # (k <= q + base + right), generalizing causal to an arbitrary right band.
    # br_base shifts BOTH the causal upper bound AND the SWA lower bound to the
    # bottom-right corner (0 for top-left); causal_band_right widens only the
    # causal upper bound.
    # Effective per-batch KV / Q length for the PADDED + BR mask (defaults to
    # the physical SKV / SQ).  THD pulls them from the cu_* diffs.
    if cutlass.const_expr(THD_VARLEN):
    ...
    # br_base shifts the band right by (eff_SKV - eff_SQ) — PER-BATCH under THD
    # (cu_* diffs) or per-batch padding (seq_kv_lens / seq_len_q); folds to the
    # global SKV - SQ when neither is supplied (eff_skv→skv_runtime,
    # eff_sq→sq_runtime).
    br_base = (cutlass.Int32(eff_skv) - cutlass.Int32(eff_sq)) if cutlass.const_expr(causal_bottom_right) else cutlass.Int32(0)
    ...
    if cutlass.const_expr(mask_flags & MASK_CAUSAL):
        ...
        # kv_right = ceil_div(q_row_base + tile_m + causal_diag, tile_n).
    ...
    if cutlass.const_expr(mask_flags & MASK_SWA):
        ...
        # CTA-base Q row), not q_row_coord + TILE_M - 1.
    ...
    if cutlass.const_expr(mask_flags & MASK_CAUSAL):
        # Mask needed when MAX_k_in_tile > MIN_q_in_CTA, i.e.,
        #   (kv_iter + 1) * tile_n > q_row_base + 1
        #   kv_iter >= (q_row_base + 1) // tile_n
    ...
    if cutlass.const_expr((mask_flags & MASK_SWA) != 0):
        ...
        # I.e., kv_iter < ceil((q_row_base + tile_m - W) / tile_n).
    ...
    # PADDED is implicit when ~is_even_mn: only the LAST iter has OOB cols
    # (kv_iter + 1 == kv_right with kv_right * tile_n > skv).  Pull
    # unmasked_hi back by 1 so the last iter applies the mask.
    ...
    # Q is rotated ONCE in the prologue (one CTA owns one Q tile); the extra
    # barrier fences the in-place rotation writes against the ldmatrix reads.
    if cutlass.const_expr(has_rope):
    ...
    # End-of-prologue barrier: all warps must finish ldmatrix Q before the
    # V prologue cp.async overwrites sQ_buf (= sV[0]).
    nvvm.barrier_cta_sync()

    # ---- Prologue V[kv_right-1] load ------------------------------------
    # Pair K and V in the prologue so the mainloop's V-pref pattern mirrors
    # the K-pref pattern: each iter prefetches BOTH K and V for the NEXT
    # iter, with the current iter's K/V already resident from prev iter
    # (or this prologue for iter 0).  sV[0] = sQ_buf + 0 (Q-aliased, free
    # after the end-of-prologue barrier above).
    v_prologue_gmem = v_gmem_batch_head + v_prologue_off64
    load_tile_2d(
        sQ_buf,
        v_prologue_gmem,
        rows=tile_n,
    ...
    # K and V prefetch pointers both start at the kv_right - 2 block (=
    # prologue's K/V minus one stride).  At iter 0 we prefetch K[N-2] and
    # V[N-2] (used at iter 1); iter 0's QK + SV consume the K[N-1] / V[N-1]
    # loaded by the prologue.
    k_next_gmem = k_gmem_batch_head + k_prologue_off64 - K_STRIDE_64
    v_next_gmem = v_gmem_batch_head + v_prologue_off64 - V_STRIDE_64
    ...
    for i in cutlass.range(cutlass.Int32(0), iter_count, cutlass.Int32(1), unroll=1):
        ...
        # ---- Step 1: prefetch K[i-1] AND V[i-1] for NEXT iter ------------
        # Both prefetches issue at iter top into the OTHER stage; current
        # iter's K[i] and V[i] are already resident (from prev iter's
        # prefetches or prologue for iter 0).  This lets the K + V cp.async
        # latency hide over the WHOLE current iter (QK + softmax + SV),
        # not just the QK-time window the old "V[i]-this-iter" pattern
        # afforded.  Each prefetch is predicated with cp_size_bytes=0 on
        # the last iter (kv_iter == kv_left) so the OOB K[-1] / V[-1] reads
        # collapse to benign SMEM zero-fills.
        ...
        # Each K tile is loaded once and consumed once → rotating sK_cur in
        # place per kv-iter is correct (the prefetch wrote sK_next).  Position
        # of K row 0 in this tile = kv_iter*tile_n; barrier fences vs ldmatrix.
        if cutlass.const_expr(has_rope):
            rope_rotate_smem_tile(
                sK_cur,
                rope_cs_ptr,
                kv_iter * cutlass.Int32(tile_n),
                rows=tile_n,
                d_qk=d_qk,
                tidx=tidx,
                threads=threads,
                    ...
                    for k in cutlass.range_constexpr(QK_N_FRAGS):
                        ...
                        def _mask_term(col, q_abs, q_mw):
                            ...
                            if cutlass.const_expr(_effective_mask & MASK_CAUSAL):
                                ...
                                # runtime right band; plain top-left causal ⇒ 0.
        ...
        # ---- Step 5: SV mma (V[i] already resident in sV_cur) -----------
        # With the V-prefetch pattern there's no mid-iter cp_async_wait /
        # barrier needed — V[i] was drained by the wait(2) at iter top
        # (alongside K[i]).  K[i-1] + V[i-1] keep streaming into the OTHER
        # stage; next iter's wait(2) drains them.
        ...
        # this iter's SV just finished reading.  Without this barrier a
        # fast warp at iter (i+1) could issue its V_pref cp.async while
        # a slow warp is still in this iter's SV ldmatrix on the same
        # slot.  (Same hazard applies to K_pref vs QK, but QK reads
        # finish strictly before SV reads via mma_step's k-loop ordering;
        # the slow-warp case is the SV reads that overlap V_pref writes.)
        nvvm.barrier_cta_sync()

        # EXP-A: advance running pointers for iter (i+1).  Int64 add only
        # (K/V stride already cast outside the loop).  K and V have separate
        # strides for asymmetric d_qk != d_v (DSv3).  Both pointers track
        # the NEXT iter's prefetch position.
        k_next_gmem = k_next_gmem - K_STRIDE_64
        v_next_gmem = v_next_gmem - V_STRIDE_64
