...
def compile(  # noqa: A001
    ...
    PAGED_KV: ``skv`` is IGNORED — the kernel derives the static KV maximum
    from the block table (``max_pages * PAGE_SIZE``) at launch; ``k_stride`` /
    ``v_stride`` are the pools' strides in ``[num_pages, page_size, H_kv, D]``
    order (required), ``block_table_stride`` / ``block_table_v_stride`` the
    tables' declared ``(batch, page)`` strides (None = compact row-major).
    ``num_pages`` and ``max_pages`` compile DYNAMIC (``cute.sym_int``), so one
    artifact serves every pool / table size (plan-time-only key, Rule 4).
