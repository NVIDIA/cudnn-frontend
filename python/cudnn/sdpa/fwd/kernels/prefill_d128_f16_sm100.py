...
def compile(
    ...
    at all. THD Stats layouts: ``lse_token_major=True`` = packed rank-2 (T, H);
    default head-major = rank-3 [1, QH, head_stride], where ``lse_stride`` is
    the caller-declared head-row stride (0 → compact, i.e. ``sq``). All three
    are shapes/specializations of the traced code, so they are part of this
    cache key.
