...
def _sdpa_kernel(
    ...
    for i in cutlass.range(cutlass.Int32(0), iter_count, cutlass.Int32(1), unroll=1):
        ...
        # ``block_on`` is a Python ``True`` → ``if block_on:`` below is a
        # pure-Python if (traces the body unconditionally, dense path is
        # byte-identical).
        ...
        # ``block_on`` is Python-True (unconditional, dense byte-identical).
        if block_on:
...
def forward(
    ...
    *,
    ...
    # never touches (seq_len_q trim, zero-length padded batches) rely on
    # zero-init, so zero a bound buffer only when such a path is active —
    # the plain / causal / SWA dense paths write every valid element.
