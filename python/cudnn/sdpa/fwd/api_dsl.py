...
class SdpaFwdDslSm100(SdpaFwdDsl):
        ...
        if _explicit:
            ...
            if self._pertensor:
                ...
                if self.paged:
                    ...
                    # / max_pages are dynamic extents of the artifact (Rule 4),
                    # and the logical KV maximum leaves the key (_PAGED_COMPILE_SKV).
                    fp8_kwargs.update(self._paged_compile_kwargs(), skv=_PAGED_COMPILE_SKV)
    ...
    def _execute_fp8(
        ...
        # Epilogue gate (bf16, O's shape): a view at the compiled strides, never a copy.
        G = self._checked_gate_view(gate, o_tensor) if gate is not None else None
        # Trailing paged block tables (after the o_partial_f32 slot); omitted
        # entirely on dense builds, whose compiled signature ends before them.
        paged_kwargs = {"block_table_tensor": block_table, "block_table_v_tensor": block_table_v} if self.paged else {}
