...
class RmsNormRhtAmaxSm100(APIBase):
    ...
    def __init__(
        ...
        self._kernel = RMSNormRHTAmaxKernel(
            x=self.x_desc,
            weight=self.w_desc,
            eps=eps,
    ...
    def compile(self) -> None:
        ...
        o_desc, amax_desc = self._kernel.infer_output()
        valid_m = cute.sym_int(divisibility=self._kernel.rows_per_cta)

        fake_x_tensor = self._make_fake_cute_compact_tensor(
            dtype=self.x_desc.dtype,
            shape=(valid_m, self._kernel.n),
            stride_order=self.x_desc.stride_order,
            assumed_align=_TENSOR_ALIGNMENT,
            dynamic_mode=None,
            divisibility=self._kernel.rows_per_cta,
        )
        ...
        fake_o_tensor = self._make_fake_cute_compact_tensor(
            dtype=o_desc.dtype,
            shape=(valid_m, self._kernel.n),
            stride_order=o_desc.stride_order,
            assumed_align=_TENSOR_ALIGNMENT,
            dynamic_mode=None,
            divisibility=self._kernel.rows_per_cta,
        )
        ...
        fake_amax_tensor = self._make_fake_cute_tensor(
            dtype=amax_desc.dtype,
            shape=(fake_num_ctas,),
            stride=amax_desc.stride,
            assumed_align=_TENSOR_ALIGNMENT,
        )
    ...
    def execute(
        ...
        self._check_tensor_signature(x_tensor, self.x_desc)
        self._check_tensor_signature(w_tensor, self.w_desc)

        if current_stream is None:
            current_stream = cuda.CUstream(torch.cuda.current_stream(x_tensor.device).cuda_stream)

        o_desc, amax_desc = self._kernel.infer_output()
        o_tensor = o_desc.materialize(stream=current_stream)
        amax_tensor = amax_desc.materialize(stream=current_stream)
