...
def _compile(quantization, heads, position_dtype, device_index, capability):
    import triton
    from triton.backends.compiler import GPUTarget
    from triton.compiler import ASTSource

    kernel = _kernel()
    rows = 8 if quantization == "fp4" else 2
    artifacts = {}
    ...
    with torch.cuda.device(device_index):
        for alignment in (16, 4, 2):
            constants = dict(HEADS=heads, FP4=quantization == "fp4", ROWS=rows, PACKED_IO=alignment >= 4)
...
class RopeQDQInplace(APIBase):
    """Rotate the last 64 channels, then quantize/dequantize groups of 32.

    FP4 accepts BF16 [N,H,128], H in {1,4,32}; FP8 accepts BF16 [...,512].
    The output aliases x. Cache is FP32 [P,64] (32 cosines then 32 sines).
    Position IDs are contiguous int32/int64, one per token (FP4) or row (FP8).
    Token counts, cache length and pointer alignment may change after compile.
    """

    def __init__(self, x, cache, positions, *, quantization, backend="frost"):
        super().__init__()
        ...
        self._heads = self._descs["x"].shape[1] if quantization == "fp4" and self._descs["x"].ndim == 3 else 1
    ...
    def _validate_layouts(self, tensors):
        ...
        if self._quantization == "fp4":
            ...
            if x.ndim < 2 or x.shape[-1] != 512:
                raise ValueError("FP8 x must be [...,512] with at least two dimensions")
            tokens = math.prod(x.shape[:-1])
        rows = tokens * self._heads
        tile_rows = 8 if self._quantization == "fp4" else 2
    ...
    def compile(self):
        ...
        self._compiled_kernel = _compile(self._quantization, self._heads, self._position_dtype, self._device.index, self._capability)
...
def rope_qdq_inplace(x, cache, positions, *, quantization, backend="frost", stream=None):
    """Convenience wrapper returning ``TupleDict(out=x)``; prewarm before capture."""
    op = RopeQDQInplace(x, cache, positions, quantization=quantization, backend=backend)
    op.compile()
