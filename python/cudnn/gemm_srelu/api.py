...
class GemmSreluSm100(APIBase):
    def __init__(
        ...
        torch = torch_dep.require("cudnn.gemm_srelu.api.__init__")
    ...
    def check_support(self) -> bool:
        torch = torch_dep.require("cudnn.gemm_srelu.api.check_support")
...
def _allocate_dense_output(shape: Tuple[int, int, int], major: str, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    torch = torch_dep.require("cudnn.gemm_srelu.api._allocate_dense_output")
    m, n, l = shape
    if major == "m":
...
def gemm_srelu_wrapper_sm100(
    ...
    torch = torch_dep.require("cudnn.gemm_srelu.api.gemm_srelu_wrapper_sm100")
    if c_dtype is None:
