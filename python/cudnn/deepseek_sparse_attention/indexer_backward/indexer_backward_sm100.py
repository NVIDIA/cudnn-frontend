...
class ScoreGradSm100:
    """CuTe DSL kernel for sparse score-gradient precompute.

    By default the results alias the two score inputs for compatibility with
    the Torch pipeline. Functional callers may provide separate gradient and
    sum-gradient output tensors.
    """
    ...
    def __call__(
        ...
        # Supplying a distinct grad-signal output selects the functional path.
        # The sum is not consumed by indexer backward, so avoid materializing it
        # unless the caller explicitly requests it. With neither output passed,
        # preserve the Torch pipeline's historical in-place behavior.
    ...
    def kernel_score_grad(
        ...
        mSumGrad,
        grad_scale: Float32 | float,
        write_sum_grad: cutlass.Constexpr[bool],
    ):
        tidx = cute.arch.thread_idx()[0]
        seq_idx = cute.arch.block_idx()[0]
        batch_idx = cute.arch.block_idx()[1]
