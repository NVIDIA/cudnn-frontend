...
LSE, and the empty-row sentinel (O=0, max_logits=-inf, LSE=-inf per the
generic sparse_attention_forward_wrapper contract; the separate internal
``lse_indexer`` prefix statistic keeps its own +inf empty sentinel).

DSL-style note (PR2 of the fwd-API roadmap): the newer frost tile_dsl house
style used by python/cudnn/sdpa/fwd/kernels/prefill_d*_f16_sm100.py and
python/cudnn/linear_attention/frost/kernel/gdn_prefill_f16.py (raw
cutlass.experimental.primitives / cudnn.frost.tile_dsl.{mma,tma,tmem,barrier,
scheduler,mask} primitives) was evaluated for this kernel and NOT adopted in
this pass. This mainloop is a persistent, TMA-gather4-driven kernel over
aliased MLA K/V latents with per-warp dynamic top-k lengths, duplicate-index
handling, and a hand-tuned 3-stage NoPE/V pipeline built directly on
``cutlass.pipeline`` / ``cutlass.utils.blackwell_helpers`` primitives (see
``_tcgen05_mma_ws.py``, ``_tcgen05_sync.py``, ``_tma_gather4.py``). None of
the frost tile_dsl house primitives currently express irregular/gathered TMA
descriptors with a per-row-dynamic valid-length mask, or a 3-stage async
pipeline depth that differs from the fixed K/V staging in
``cudnn.frost.tile_dsl.tma`` / ``barrier`` (those assume the dense, statically
tiled K/V layout of SDPA and the linear-attention GDN chunk recurrence, not
sparse-gathered rows). Porting the mainloop would mean adding a genuinely new
gather-with-validity primitive to ``cudnn.frost.tile_dsl`` first -- out of
scope for this pass -- rather than a mechanical swap of existing primitives.
Per the subtask contract, the resolution taken here is: keep this kernel's
existing ``cutlass.pipeline`` / ``blackwell_helpers`` implementation as the
PR2 backend, and put the frost-tile_dsl-styled uniformity at the dispatch
boundary instead -- see ``_interface_sm100.py``'s ``DsaSm100TemplateParams``,
which mirrors the compile-time-facts-only shape of
``cudnn.sdpa.fwd.config_sm100.TemplateParams`` so PR4's GQA substrate
dispatch and this PR2 envelope's dispatch present one uniform surface to
``fwd/api.py``'s ``_get_dsa_prefill_kernel()``, even though the two kernel
families are written in different CuTe DSL styles today. A follow-up pass
that first lands a validity-aware TMA-gather primitive in
``cudnn.frost.tile_dsl.tma`` is the natural place to revisit a real port.
"""
    ...
    def _epilogue_stats(
        ...
        if warp_idx < Int32(2):
            ...
            if has_valid:
                ...
                # The generic sparse_attention_forward_wrapper contract (see
                # python/cudnn/sparse_attention/fwd/api.py) fixes the dead-row
                # sentinel at ``lse = -inf`` (the LSE-merge identity:
                # ``logaddexp(-inf, x) == x``), matching cuDNN's own SDPA
                # convention. ``lse_indexer`` below is the DSA indexer's own
                # internal prefix statistic, not the wrapper's public LSE, and
                # keeps its historical FA2-style ``+inf`` empty sentinel.
