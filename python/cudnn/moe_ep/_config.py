...
class MoeEpParallelConfig:
    ...
    def __post_init__(self) -> None:
        if self.ep_group is not None and not isinstance(
            self.ep_group, dist.ProcessGroup
        ):
            raise ValueError(
                "ep_group must be a torch.distributed.ProcessGroup or None, "
                f"got {type(self.ep_group).__name__}"
            )
        if self.max_tokens_per_rank is not None and (
            isinstance(self.max_tokens_per_rank, bool)
            or not isinstance(self.max_tokens_per_rank, int)
            or self.max_tokens_per_rank < 0
        ):
            raise ValueError(
    ...
    def __post_init__(self) -> None:
        for name in ("output_format", "combine_format"):
            ...
            if not isinstance(value, MoeFormat):
                raise TypeError(
                    f"{name} must be a MoeFormat, got {type(value).__name__}"
                )
        ...
        if not isinstance(self.fc1_weight_layout, MoeEpFc1WeightLayout):
            raise TypeError(
                ...
                f"got {type(self.fc1_weight_layout).__name__}"
            )
        ...
        if isinstance(value, bool) or not isinstance(value, Real):
            raise ValueError(
                "gate_up_clamp must be a finite non-negative real number or None"
            )
...
class MoeEpConfig:
    ...
    training_forward_tuning: MoeEpTuningConfig = field(
        default_factory=MoeEpTuningConfig
    )
    training_backward_tuning: MoeEpTuningConfig = field(
        default_factory=MoeEpTuningConfig
    )
    ...
    def __post_init__(self) -> None:
        ...
        for name, fmt in (
            ...
            if model.hidden_size % required_multiple:
                raise ValueError(
                    f"hidden_size ({model.hidden_size}) must be divisible by "
        ...
        for name, tuning in (
            ...
            if tuning.reduce_topk_in_kernel and (
                ...
                or not data_path.apply_topk_in_fc1
            ):
                raise ValueError(
                    f"{name}.reduce_topk_in_kernel requires BF16 "
                    "combine/output and apply_topk_in_fc1=True"
                )
        backward = self.training_backward_tuning
        if backward.token_back_mode != "epi_warps":
            raise ValueError(
                "training_backward_tuning requires " "token_back_mode='epi_warps'"
            )
        if backward.reduce_topk_in_kernel:
            raise ValueError(
                "training_backward_tuning does not support "
...
def resolve_moe_ep_config(config: MoeEpConfig) -> ResolvedMoeEpConfig:
    ...
    if group is None:
        ...
        ep_global_ranks = tuple(
            dist.get_global_rank(group, group_rank) for group_rank in range(ep_size)
        )
