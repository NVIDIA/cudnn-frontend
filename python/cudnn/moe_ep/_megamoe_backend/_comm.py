...
class _NvshmemMemoryProvider:
    ...
    def _core():
        try:
            ...
            raise RuntimeUnavailableError("symmetric workspace requires nvshmem4py and NVSHMEM libraries") from exc
        return core
    ...
    def allocate(self, nbytes: int, device: torch.device) -> torch.Tensor:
        ...
        try:
            ...
            raise RuntimeUnavailableError(f"failed to allocate {nbytes} bytes from the NVSHMEM symmetric heap") from exc
        _runtime_debug(
    ...
    def free(self, tensor: torch.Tensor) -> None:
        ...
        try:
            ...
            raise RuntimeUnavailableError("failed to free the NVSHMEM symmetric root slab") from exc
        _runtime_debug(
            "symmetric.free.end",
            elapsed_seconds=f"{time.monotonic() - started_at:.3f}",
        )
    ...
    def peer_address(self, tensor: torch.Tensor, peer: int) -> int:
        ...
        try:
            ...
            raise RuntimeUnavailableError(f"failed to map symmetric root slab for peer {peer}") from exc
...
class PeerMapping:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rank < 0 or self.rank >= len(self.offsets):
            raise ValueError(f"peer mapping rank {self.rank} is outside {len(self.offsets)} ranks")
        if self.offsets[self.rank] != 0:
            raise ValueError(f"local peer offset must be zero, got {self.offsets[self.rank]}")
...
class SymmetricSlab:
    ...
    def __init__(
        ...
        *,
        ...
        self._nbytes = nbytes
        self._provider = provider or (_NvshmemMemoryProvider() if runtime.nvshmem_enabled else _TorchMemoryProvider())
    ...
    def ensure_allocated(self) -> None:
        if self._cleanup_required:
            raise RuntimeError("symmetric slab requires cleanup before allocation")
        if self._root is not None and self._mapping is not None:
            return
        if self._root is not None:
            raise RuntimeError("symmetric slab has an allocation pending cleanup")
        ...
        if not isinstance(root, torch.Tensor):
            raise TypeError("symmetric memory provider must return a torch.Tensor")
        self._root = root
        try:
            if root.dtype is not torch.uint8 or root.numel() < self._nbytes:
                raise ValueError("symmetric root must be a uint8 tensor with at least " f"{self._nbytes} elements")
            if root.device != self._runtime.device:
                raise ValueError("symmetric root device does not match runtime device: " f"root={root.device}, runtime={self._runtime.device}")
        ...
        try:
            ...
            for peer in range(self._runtime.world_size):
                if peer == self._runtime.rank:
                    offsets.append(0)
                    continue
                offsets.append(self._provider.peer_address(root, peer) - base_address)
            mapping = PeerMapping(
                base_address=base_address,
                offsets=tuple(offsets),
                rank=self._runtime.rank,
            )
            if mapping.world_size != self._runtime.world_size or mapping.rank != self._runtime.rank:
                raise RuntimeError("symmetric peer mapping does not match the EP subgroup")
        except Exception:
            self._cleanup_required = True
            raise
    ...
    def allocated(self) -> bool:
        return not self._cleanup_required and self._root is not None and self._mapping is not None
    ...
    def byte_view(self, offset: int, nbytes: int) -> torch.Tensor:
        if offset < 0 or nbytes < 0 or offset + nbytes > self._nbytes:
            raise ValueError(f"byte view [{offset}, {offset + nbytes}) exceeds " f"symmetric slab size {self._nbytes}")
