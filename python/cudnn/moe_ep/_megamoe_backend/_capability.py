...
def _validate_wgrad_config(config: ForwardConfig) -> None:
    if config.backward_wgrad_mode not in ("none", "operands"):
        raise ValueError("unsupported backward_wgrad_mode " f"{config.backward_wgrad_mode!r}")
    if config.backward_wgrad_mode == "operands":
        ...
        if config.token_padding_size != 128:
            raise ValueError("backward_wgrad_mode='operands' requires " "token_padding_size=128")
        if config.sf_padding_size != 128:
            raise ValueError("backward_wgrad_mode='operands' requires " "sf_padding_size=128")
...
def validate_config(config: ForwardConfig) -> None:
    ...
    if config.hidden_size % 128:
        raise NotImplementedError("MoeEp SM107 MXFP8 kernel currently requires hidden_size " f"to be divisible by 128, got {config.hidden_size}")
    if config.intermediate_size % 256:
        raise NotImplementedError("MoeEp SM107 MXFP8 kernel currently requires intermediate_size " f"to be divisible by 256, got {config.intermediate_size}")
    if config.top_k > 32:
