...
class Mxfp8TrainingStager:
    ...
    def _validate(
        ...
        if source.dtype not in (torch.bfloat16, torch.float32):
            raise TypeError(
                "training staging source must be BF16 or FP32, "
                f"got {source.dtype}"
            )
        if source.ndim != 2 or source.shape[1] != self.hidden:
            raise ValueError(
                f"training staging source must have shape (T, {self.hidden})"
            )
        ...
        if (
            topk_weights.dtype is not torch.float32
            or not topk_weights.is_contiguous()
        ):
            raise TypeError(
                "training staging topk_weights must be contiguous FP32"
            )
        if (
            output.dtype is not torch.float8_e4m3fn
            or output.ndim != 2
            or output.shape[1] != self.hidden
            or not output.is_contiguous()
        ):
            raise ValueError(
                "training staging output must be contiguous E4M3 "
                f"(capacity, {self.hidden})"
            )
        if token_count > output.shape[0]:
            raise ValueError(
                f"token count {token_count} exceeds capacity {output.shape[0]}"
            )
        ...
        for name, tensor, dtype in (
            ...
            if (
                tensor.shape != (output.shape[0], self.top_k)
    ...
    def stage(
        ...
        output_sf.zero_()
        routing_in_place = (
            topk_idx.data_ptr() == output_topk_idx.data_ptr()
            and topk_weights.data_ptr() == output_topk_weights.data_ptr()
        )
        routing_partially_aliased = (
            topk_idx.data_ptr() == output_topk_idx.data_ptr()
        ) != (
            topk_weights.data_ptr() == output_topk_weights.data_ptr()
        )
        if routing_partially_aliased:
            raise ValueError(
                "training staging routing inputs must either both alias "
                "their outputs or neither alias"
            )
        ...
        with self._lock:
            ...
            if compiled is None:
                if torch.cuda.is_current_stream_capturing():
                    raise RuntimeError(
                        "MXFP8 training stager must be compiled before "
                        "CUDA graph capture"
                    )
