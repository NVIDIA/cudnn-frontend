...
"""PyTorch custom operation and public API for cuDNN GAT attention."""
...
def _require_backend() -> None:
    require_backend_symbols("gnn_mha_gat_forward", "gnn_mha_gat_backward")
...
def _backward(
    ...
    with torch.cuda.device(offsets.device):
        cudnn.gnn_mha_gat_backward(
            ...
            TORCH_DTYPE_TO_CUDNN[feature_grad_dtype],
            TORCH_DTYPE_TO_CUDNN[weight_grad_dtype],
        )
    return grad_src, grad_dst, grad_edge, grad_weights
...
_lib.define(
    "gnn_mha_gat_fwd(Tensor offsets, Tensor indices, Tensor? map_csc_to_coo, Tensor src_features, Tensor dst_features, "
    ...
    "Tensor? csc_rev_offsets, Tensor? map_rev_to_coo, bool deterministic, ScalarType feature_grad_dtype, "
    "ScalarType weight_grad_dtype) -> (Tensor, Tensor, Tensor)"
)
_lib.define(
    "gnn_mha_gat_bwd(Tensor? grad_output, Tensor? grad_attention, Tensor offsets, Tensor indices, "
    "Tensor? map_csc_to_coo, Tensor src_features, Tensor dst_features, Tensor? edge_features, Tensor attn_weights, "
    "Tensor sm_scores, Tensor? dropout_mask, int num_src_nodes, int num_heads, bool concat_heads, "
    "str activation, float activation_alpha, Tensor? csc_rev_offsets, Tensor? map_rev_to_coo, "
    "bool deterministic, ScalarType feature_grad_dtype, ScalarType weight_grad_dtype) -> (Tensor, Tensor, Tensor, Tensor)"
)
_lib.impl("gnn_mha_gat_fwd", _forward, "CUDA")
_lib.impl("gnn_mha_gat_bwd", _backward, "CUDA")
...
def _fake_forward(
    ...
    del feature_grad_dtype, weight_grad_dtype
    return _fake_forward_outputs(offsets, indices, src_features, num_heads, concat_heads, return_attention_weights)


@torch.library.register_fake("cudnn::gnn_mha_gat_bwd")
def _fake_backward(
    grad_output,
    grad_attention,
    offsets,
    ...
    del grad_output, grad_attention, offsets, indices, map_csc_to_coo, sm_scores, dropout_mask
    del num_src_nodes, num_heads, concat_heads, activation, activation_alpha
    del csc_rev_offsets, map_rev_to_coo, deterministic
    return _fake_backward_outputs(src_features, dst_features, edge_features, attn_weights, feature_grad_dtype, weight_grad_dtype)
...
def _setup_context(ctx, inputs, output) -> None:
    ...
    if return_attention_weights:
        ctx.mark_non_differentiable(sm_scores)
    else:
        ctx.mark_non_differentiable(attention, sm_scores)
    ...
    ctx.feature_grad_dtype = feature_grad_dtype
    ctx.weight_grad_dtype = weight_grad_dtype


@torch.compiler.allow_in_graph
def _autograd_backward(ctx, grad_output, grad_attention, grad_sm_scores):
    del grad_sm_scores
    ...
    grad_src, grad_dst, grad_edge, grad_weights = torch.ops.cudnn.gnn_mha_gat_bwd(
        grad_output,
        grad_attention,
        offsets,
        indices,
        ...
        map_rev_to_coo,
        ctx.deterministic,
        ctx.feature_grad_dtype,
        ctx.weight_grad_dtype,
    )
    return _autograd_input_gradients(ctx, grad_src, grad_dst, grad_edge, grad_weights)


torch.library.register_autograd("cudnn::gnn_mha_gat_fwd", _autograd_backward, setup_context=_setup_context)
...
def mha_gat(
    ...
    *,
    ...
    Args:
        ...
        attn_weights (torch.Tensor): Flat source, destination, and optional
            edge weights with length 2 * dim_node + dim_edge.
        ...
        num_heads (int): Number of attention heads. Feature dimensions must be
            divisible by this value. Default: 1.
        ...
        high_precision_dgrad (bool): Compute source, destination, and edge
            feature gradients in FP32 for FP16 or BF16 inputs. Requires
            high_precision_wgrad=True. Default: False.
        high_precision_wgrad (bool): Compute attention-weight gradients in FP32
            for FP16 or BF16 inputs. Default: False.
    ...
    Note:
        PyTorch separately controls leaf-gradient accumulation. Set each
        applicable input tensor's grad_dtype to torch.float32 to retain a
