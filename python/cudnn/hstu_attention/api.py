...
_ARBITRARY_FWD_MAX_SEQLEN_K = 64 * 1024
_ARBITRARY_BWD_MAX_SEQLEN_Q = 32 * 1024
...
def _require_disjoint_writes(
    ...
    for write_name, write_tensor in writes:
        ...
        for other_name, other_tensor in comparisons:
            ...
            if spans_overlap and not _are_disjoint_fused_thd_slices(
                write_tensor, other_tensor
            ):
                raise ValueError(
...
def _validate_page_metadata(
    ...
    if (
        page_indptrs.ndim != 1
        or page_indptrs.dtype != torch.int32
        or page_indptrs.shape[0] != batch_size + 1
    ):
        raise ValueError(
            f"page_indptrs_tensor must be torch.int32 with shape "
            f"({batch_size + 1},)"
        )
...
class _HSTUBase(APIBase):
    ...
    def _init_common(
        ...
        *,
        ...
        self.cu_seqlens_q_desc = self._make_tensor_desc(
            sample_cu_seqlens_q, name="cu_seqlens_q"
        )
    ...
    def _check_common(self, supported_head_dims: Tuple[int, ...]) -> None:
        ...
        for name, tensor in (("q", q), ("k", k), ("v", v)):
            if tensor.ndim != 3:
                raise ValueError(
                    f"{name}_tensor must use packed THD rank-3 layout, got "
        ...
        if v.shape != k.shape:
            raise ValueError(
                f"v_tensor must have the same shape as k_tensor; got "
                f"{tuple(v.shape)} and {tuple(k.shape)}"
            )
        if q_heads != k_heads:
            raise ValueError(
                "HSTU SM100 currently supports MHA only; q, k, and v must "
                f"have the same head count, got {q_heads} and {k_heads}"
            )
        if q_dim != k_dim:
            raise ValueError(
                f"q, k, and v head dimensions must match, got {q_dim} and {k_dim}"
            )
        if q_dim not in supported_head_dims:
            raise ValueError(
                f"head_dim must be one of {supported_head_dims}, got {q_dim}"
            )
        if q.dtype not in (torch.float16, torch.bfloat16):
            raise ValueError(
                f"q_tensor must have dtype torch.float16 or torch.bfloat16, got {q.dtype}"
            )
        if k.dtype != q.dtype or v.dtype != q.dtype:
            raise ValueError(
                "q_tensor, k_tensor, and v_tensor must have the same dtype"
            )
        ...
        if q_batch_size != k_batch_size:
            raise ValueError(
                "cu_seqlens_q_tensor and cu_seqlens_k_tensor must describe the "
                "same batch size"
            )
        self.batch_size = q_batch_size
        self.max_seqlen_q = _resolve_max_seqlen(
            self.max_seqlen_q, total_q, "max_seqlen_q"
        )
        self.max_seqlen_k = _resolve_max_seqlen(
            self.max_seqlen_k, total_k, "max_seqlen_k"
        )
        ...
        if not math.isfinite(self.scaling_seqlen) or self.scaling_seqlen <= 0.0:
            raise ValueError(
                f"scaling_seqlen must be positive and finite, got {self.scaling_seqlen}"
            )

        window_left, window_right = self.window_size
        if window_left < -1 or window_left > self.max_seqlen_k:
            raise ValueError(
                f"window_size[0] must be -1 or in [0, {self.max_seqlen_k}], "
                f"got {window_left}"
            )
        if window_right < -1 or window_right > self.max_seqlen_k:
            raise ValueError(
                f"window_size[1] must be -1 or in [0, {self.max_seqlen_k}], "
                f"got {window_right}"
            )
        ...
        if self._sample_func is not None:
            ...
            if func.ndim != 3:
                raise ValueError(
                    "func_tensor must have shape (1, n_func, total_q + 256)"
                )
            if func.dtype != torch.int32:
                raise ValueError(
                    f"func_tensor must have dtype torch.int32, got {func.dtype}"
                )
            _require_16_byte_alignment(func, "func_tensor")
            if func.shape[0] != 1 or func.shape[1] <= 0 or func.shape[1] % 2 == 0:
                raise ValueError(
                    "func_tensor must have shape (1, n_func, total_q + 256) "
                    "with n_func positive and odd"
                )
        ...
        if major != 10:
            raise RuntimeError(
                "HSTU SM100 requires an SM10x Blackwell GPU; found "
                f"SM{major}{minor} on {q.device}"
            )
        ...
        *,
        ...
        for name, tensor, desc in (
            ...
            if tuple(tensor.shape) != tuple(desc.shape):
                raise ValueError(
                    f"{name} shape must match the compiled shape {desc.shape}, "
            ...
            if (
                ...
                raise ValueError(
                    f"{name} dtype/device/stride must match the compiled descriptor"
                )
            if not _has_non_overlapping_strides(tensor):
        ...
        if (func_tensor is None) != (self.func_desc is None):
            raise ValueError(
                "func_tensor presence must match the compiled configuration"
            )
...
class HSTUFwdSm100(_HSTUBase):
    ...
    def __init__(
        ...
        self.page_indptrs_desc = self._make_tensor_desc(
            sample_page_indptrs, name="page_indptrs"
        )
    ...
    def check_support(self) -> bool:
        ...
        if paged is None:
            if page_ids is not None or page_indptrs is not None:
                raise ValueError(
                    "page_ids_tensor and page_indptrs_tensor require paged_kv_tensor"
                )
        else:
            if not self.is_causal or self.is_local or self._sample_func is not None:
                raise ValueError(
                    "paged KV requires causal attention and cannot be combined "
                    "with local or arbitrary masking"
                )
            if page_ids is None or page_indptrs is None:
                raise ValueError(
                    "paged KV requires page_ids_tensor and page_indptrs_tensor"
                )
            expected_tail = (2, 128, self.num_heads, self.head_dim)
            if (
                paged.ndim != 5
                or paged.shape[0] <= 0
                or tuple(paged.shape[1:]) != expected_tail
            ):
                raise ValueError(
        ...
        if (
            ...
            and self.max_seqlen_k > _ARBITRARY_FWD_MAX_SEQLEN_K
        ):
            raise ValueError(
                "arbitrary-mask forward requires max_seqlen_k <= "
                f"{_ARBITRARY_FWD_MAX_SEQLEN_K}, got {self.max_seqlen_k}"
            )
    ...
    def check_support(self) -> bool:
        ...
        for name, tensor, expected in (
            ...
            if tensor.shape != expected.shape or tensor.dtype != expected.dtype:
                raise ValueError(
                    f"{name} must have shape {tuple(expected.shape)} and dtype "
                    f"{expected.dtype}"
                )
            if not _has_non_overlapping_strides(tensor):
                raise ValueError(f"{name} must have non-overlapping strides")
        ...
        if (
            self._sample_func is not None
            and self.max_seqlen_q > _ARBITRARY_BWD_MAX_SEQLEN_Q
        ):
            raise ValueError(
                "arbitrary-mask backward requires max_seqlen_q <= "
                f"{_ARBITRARY_BWD_MAX_SEQLEN_Q}, got {self.max_seqlen_q}"
            )
        _require_disjoint_writes(
            (
                ("dq_tensor", self._sample_dq),
...
def hstu_attention_forward(
    ...
    _validate_cu_seqlens_metadata(cu_seqlens_q_tensor, "cu_seqlens_q_tensor")
    _validate_cu_seqlens_metadata(cu_seqlens_k_tensor, "cu_seqlens_k_tensor")
    resolved_max_q = _resolve_max_seqlen(
        max_seqlen_q, q_tensor.shape[0], "max_seqlen_q"
    )
    resolved_max_k = _resolve_max_seqlen(
        max_seqlen_k, k_tensor.shape[0], "max_seqlen_k"
    )
    resolved_scaling = float(
        resolved_max_q if scaling_seqlen is None else scaling_seqlen
    )
