...
- **New API modules must be framework-neutral, and that means more than imports.** The package serves multiple frameworks, and a JAX process must be able to import an operation module (e.g. `cudnn.sparse_attention.forward.api`) without torch ever entering `sys.modules` — the lazy table alone is not enough for that: it protects `import cudnn`, not the operation module itself. Concretely:
  - **No module-level `import torch`.** `from __future__ import annotations` (annotations stay strings), `if TYPE_CHECKING:` for type-only imports, function-local `import torch` (or `api_base._torch()`) — the pattern `tensor_adapter.py` and `datatypes.py` already follow.
  - **No torch-typed public API.** Signatures take framework tensors (anything dlpack-compatible — torch, JAX, numpy), annotated `Any`, never `torch.Tensor`. Validation runs on canonical descriptors and adapter helpers, never on torch types: `_make_tensor_desc(..., canonical=True)` (cutlass dtypes, adapter devices), `_check_dtype` against `cutlass.*` dtypes, `tensor_adapter.get_compute_capability()` / `get_device()` / `get_data_ptr()` / `detect_framework()` instead of `torch.cuda.*` / `.data_ptr()`.
  - **Framework support is a backend property, not an API property.** A torch-only kernel or reference implementation gates itself with `detect_framework(...)` and raises `NotImplementedError` (request-or-fail); the contract and validation must accept any framework's tensors.
  - Guard both properties with tests: a subprocess test that blocks torch on `sys.meta_path` and imports the module, and a non-torch-input test proving validation raises the documented error classes rather than tripping on torch attribute access (see `test/python/sparse_attention/test_sparse_attention_forward.py::test_import_without_torch` / `::test_non_torch_inputs_reach_validation_cleanly`). Pre-existing `api.py` files with top-level torch imports or torch-typed signatures are grandfathered; do not add new ones.
