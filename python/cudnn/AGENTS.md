...
- **New API modules must not import torch at module level** — the package serves multiple frameworks, and a JAX process must be able to import an operation module (e.g. `cudnn.sparse_attention.forward.api`) without torch ever entering `sys.modules`. The lazy table alone is not enough for that: it protects `import cudnn`, not the operation module itself. Pattern: `from __future__ import annotations` (annotations stay strings), `if TYPE_CHECKING: import torch` for type hints, and function-local `import torch` (or `api_base._torch()`) on the validate/execute paths only — the same pattern `tensor_adapter.py` and `datatypes.py` already follow. Guard it with a subprocess test that blocks torch on `sys.meta_path` and imports the module (see `test/python/sparse_attention/test_sparse_attention_forward.py::test_import_without_torch`). Pre-existing `api.py` files with top-level torch imports are grandfathered; do not add new ones.
