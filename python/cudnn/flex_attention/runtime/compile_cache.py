...
def _compute_source_fingerprint() -> str:
    ...
    - Any Python source under ``cudnn.flex_attention`` is added, removed,
      renamed, or modified.
...
class FileLock:
    ...
    def __enter__(self) -> "FileLock":
        ...
        if not acquired:
            os.close(self._fd)
            self._fd = None
            raise RuntimeError(f"Timed out after {self.timeout}s waiting for " f"{self._lock_label} lock: {self.lock_path}")
