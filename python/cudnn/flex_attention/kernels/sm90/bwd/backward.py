...
class FlexAttentionBackwardSm90:
    ...
    def _get_shared_storage_cls(self):
        ...
        sdKVaccum_size_bytes = self.tile_n * max(self.tile_hdim, self.tile_hdimv) * Float32.width // 8
        mainloop_or_dkv_size_bytes = max(SharedStorageMainloop.__sizeof__(), sdKVaccum_size_bytes)
    ...
    def __call__(
        ...
        if const_expr(self.deterministic and self.qhead_per_kvhead > 1):
            ...
            mdK_semaphore, mdV_semaphore = [layout_utils.select(t, mode=[2, 3, 1, 0]) for t in (mdK_semaphore, mdV_semaphore)]
        else:
            mdK_semaphore = None
            mdV_semaphore = None
        ...
        num_n_blocks = cute.ceil_div(max_seqlen_k, self.tile_n) if const_expr(self.varlen_k) else cute.ceil_div(cute.size(mK.shape[0]), self.tile_n)
        seqlen_q_for_l2 = max_seqlen_q if const_expr(self.is_varlen_q) else cute.size(mQ.shape[0])
        tile_sched_args = TileSchedulerArguments(
            num_n_blocks,
            cute.size(mQ.shape[2]),
            cute.size(mK.shape[3]) if const_expr(mCuSeqlensK is None) else cute.size(mCuSeqlensK.shape[0] - 1),  # num_batch
            seqlen_q_for_l2,
            mQ.shape[1],  # headdim
            mV.shape[1],  # headdim_v
            total_q=cute.size(mK.shape[0]) if const_expr(mCuSeqlensK is not None) else cute.size(mK.shape[0]) * cute.size(mK.shape[3]),
    ...
    def mma(
        ...
        acc_dV, tdVrPt, tdVrdOt = sm90_utils.partition_fragment_ABC(wg_mma_dV, shape_mnk_dV, sPt, sdOt, swap_AB=self.dKV_swapAB)
        if const_expr(not self.mma_dkv_is_rs):
            mma_pdo_fn = partial(gemm_w_idx, tiled_mma_dV, acc_dV, tdVrPt, tdVrdOt, swap_AB=self.dKV_swapAB)
        else:
        ...
        acc_dK, tdKrdSt, tdKrQt = sm90_utils.partition_fragment_ABC(wg_mma_dK, shape_mnk_dK, sdSt, sQt, swap_AB=self.dKV_swapAB)
        if const_expr(not self.mma_dkv_is_rs):
            mma_dsq_fn = partial(gemm_w_idx, tiled_mma_dK, acc_dK, tdKrdSt, tdKrQt, swap_AB=self.dKV_swapAB)
        else:
        ...
        while work_tile.is_valid_tile:
            ...
            seqlen = SeqlenInfoCls(batch_idx)
            m_block_max = cute.ceil_div(seqlen.seqlen_q, self.tile_m)
            consumer_state_Q, consumer_state_dO, process_tile = consume_block_sparse_mma_bwd_sm90(
                blocksparse_tensors,
                batch_idx,
                head_idx,
                n_block,
    ...
    def mma_one_m_block(
        ...
        # (2) [GEMM 2] dP = dO @ V.T
        pipeline_dO.consumer_wait(consumer_state_dO_cur, pipeline_dO.consumer_try_wait(consumer_state_dO_cur))
    ...
    def epilogue_dKV(
        ...
        epi_barrier = cutlass.pipeline.NamedBarrier(barrier_id=int(NamedBarrierBwd.Epilogue), num_threads=self.num_mma_threads)
        warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx())
        ...
        if const_expr(self.qhead_per_kvhead == 1):
            ...
            store_dK, _, _ = copy_utils.tma_get_copy_fn(tma_atom_dK, 0, cute.make_layout(1), sK, gdK, single_stage=True)
            store_dV, _, _ = copy_utils.tma_get_copy_fn(tma_atom_dV, 0, cute.make_layout(1), sV, gdV, single_stage=True)
            ...
            mdKaccum_cur = seqlen.offset_batch_K(mdK, batch_idx, dim=2, padded=True, multiple=self.tile_hdim)[None, head_idx_kv]
            mdVaccum_cur = seqlen.offset_batch_K(mdV, batch_idx, dim=2, padded=True, multiple=self.tile_hdimv)[None, head_idx_kv]
