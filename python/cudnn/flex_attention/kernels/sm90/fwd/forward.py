...
from cudnn.flex_attention.kernels.common.pack_gqa import make_packgqa_tiled_tma_atom, pack_gqa_layout
...
class FlexAttentionForwardSm90(FlexAttentionForwardBase):
    ...
    def _get_shared_storage_cls(self, return_max_logit: bool):
        ...
        sMaxLogit_size = self.qhead_per_kvhead if return_max_logit and self.pack_gqa else int(return_max_logit)
        sMaxLogit_struct = cute.struct.Align[cute.struct.MemRange[Float32, sMaxLogit_size], 16]
    ...
    def __call__(
        ...
        self.rescale_O_before_gemm = self.tile_hdimv > 128
        ...
        if const_expr(self.varlen_q):
            mO_tma = copy_utils.create_ragged_tensor_for_tma(mO_tma)
    ...
    def mma(
        ...
        # ///////////////////////////////////////////////////////////////////////////////
        smem_copy_atom_P = utils.get_smem_store_atom(self.dtype)
        ...
        while work_tile.is_valid_tile:
            ...
            mma_one_n_block = partial(mma_one_n_block_all, softmax=softmax)
    ...
    def first_half_block(
        ...
        """Process the first half block of the overlapped K/V pipeline."""
    ...
    def last_half_block(
        ...
        """Process the final PV GEMM of the overlapped K/V pipeline."""
    ...
    def mma_one_n_block(
        ...
        if const_expr(mask_fn is not None):
            if const_expr(mask_prefetch_fn is not None):
                mask_fn(acc_S=acc_S, r_bitmask=r_bitmask)
            else:
                mask_fn(acc_S=acc_S)
