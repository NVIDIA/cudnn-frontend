...
class FlexAttentionBackwardPostprocess:
    ...
    def _setup_attributes(self):
        ...
        if const_expr(self.arch == 90):
            ...
            self.sdQaccum_layout = cute.make_layout((self.tile_m * self.tile_hdim // dQaccum_reduce_stage, dQaccum_reduce_stage))

        num_copy_elems = 128 // self.dtype.width
        threads_per_row = math.gcd(128, self.tile_hdim) // num_copy_elems
        self.gmem_tiled_copy_dQ = copy_utils.tiled_copy_2d(self.dtype, threads_per_row, self.num_threads, num_copy_elems)
        # ///////////////////////////////////////////////////////////////////////////////
        ...
        if const_expr(self.arch == 90):
            ...
            self.sdQ_layout = sm100_utils_basic.make_smem_layout_epi(self.dtype, LayoutEnum.ROW_MAJOR, (self.tile_m, self.tile_hdim), 1)
    ...
    def kernel_row_major(
        ...
        if work_tile.is_valid_tile:
            ...
            assert values_per_thread * self.num_threads == self.tile_m * self.tile_hdim
            for value_idx in cutlass.range_constexpr(values_per_thread):
                ...
                if row_idx < seqlen.seqlen_q - m_block * self.tile_m and col_idx < head_dim:
                    gdQ[row_idx, col_idx] = (gdQaccum[linear_idx] * scale).to(self.dtype)
