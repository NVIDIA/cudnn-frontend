...
class StaticPersistentTileScheduler:
    ...
    class Params(ParamsBase):
        ...
        def create(args: TileSchedulerArguments, *, loc=None, ip=None) -> "StaticPersistentTileScheduler.Params":
            ...
            tile_divisor = args.num_head * args.num_batch if const_expr(args.persistent_block_major) else num_block_cluster
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        assert scheduling_mode == SchedulingMode.STATIC, f"StaticPersistentTileScheduler only supports STATIC, got {scheduling_mode!r}"
        return StaticPersistentTileScheduler.Params.create(args, loc=loc, ip=ip)

    @staticmethod
    def create(params: Params, clc: ClcState | None = None, *, loc=None, ip=None) -> "StaticPersistentTileScheduler":
...
class SingleTileLPTScheduler:
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        return SingleTileLPTScheduler.Params.create(args, scheduling_mode=scheduling_mode, loc=loc, ip=ip)
    ...
    @staticmethod
    @cute.jit
    def create(params: Params, clc: ClcState | None = None, *, loc=None, ip=None) -> "SingleTileLPTScheduler":
        if const_expr(params.scheduling_mode == SchedulingMode.CLC):
            return SingleTileLPTScheduler(params, cute.arch.block_idx()[0], clc, loc=loc, ip=ip)
        if const_expr(params.use_cluster_idx):
            tile_idx = cute.arch.cluster_idx()[0]
...
class SingleTileLPTBwdScheduler:
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        assert scheduling_mode == SchedulingMode.STATIC, f"SingleTileLPTBwdScheduler only supports STATIC, got {scheduling_mode!r}"
    ...
    class Params(ParamsBase):
        ...
        def create(
            ...
            *,
            ...
            assert scheduling_mode != SchedulingMode.CLC or args.cluster_shape_mn[0] == 1, "Varlen CLC currently requires cluster_shape_mn[0] == 1"
...
class Sm100FmhaStaticTileSchedulerParams:
    ...
    def __new_from_mlir_values__(self, values):
        ...
        for obj, n_items in zip([self.problem_shape_mbh], self._values_pos):
            ...
            values = values[n_items:]
        return Sm100FmhaStaticTileSchedulerParams(self.is_persistent, *(tuple(obj_list)), loc=self._loc)
...
class Sm100FmhaStaticTileScheduler:
    ...
    def __new_from_mlir_values__(self, values):
        ...
        scheduler = Sm100FmhaStaticTileScheduler(new_params, new_current_work_linear_idx, new_blk_coord, new_grid_shape)
        scheduler._is_first_block = self._is_first_block
        return scheduler
