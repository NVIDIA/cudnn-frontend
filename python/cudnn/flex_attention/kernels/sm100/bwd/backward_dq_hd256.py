...
class BlackwellFusedMultiHeadAttentionBackwardDQKernel:
    ...
    def __call__(
        ...
        if cutlass.const_expr(cum_seqlen_q is not None):
            assert block_sparse_tensors.cu_total_m_blocks is not None, "SM100 hd256 arbitrary varlen dQ requires compact Q256 row prefixes"
        assert len(block_sparse_tensors.mask_block_cnt.shape) == 2, "SM100 hd256 arbitrary dQ requires compact rank-2 Q2K counts"
        assert block_sparse_tensors.mask_block_masks is not None
        assert len(block_sparse_tensors.mask_block_masks.shape) == 4, "SM100 hd256 arbitrary dQ payload must be rank 4"
        ...
        if warp_idx >= self.epilogue_warp_ids[0] and warp_idx <= self.epilogue_warp_ids[-1]:
            ...
            while work_tile.is_valid_tile:
                ...
                if cutlass.const_expr(not varlen):
                    gdQ_tma_qdl = cute.flat_divide(mdQ_tma, cute.select(self.dsk_block_tiler, mode=[0, 1]))
                    gdQ_tma_staged = gdQ_tma_qdl[None, None, curr_block_coord[0], None, curr_block_coord[2]]
    ...
    def dQ_epilogue_write_zero(
        ...
        tiled_copy_r2g = fa_copy_utils.tiled_copy_2d(self.dq_dtype, cute.size(gdQ_staged.shape[1]), num_epi_threads)
